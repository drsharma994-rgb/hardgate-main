...
export const RECENT_DAY_BARS = 48;
export const RECENT_DAY_MIN_N = 4;
...
export const DESK_ANALOGUES = {
  ...
  reversalsniper: [
    { kind: 'PIN-REJECT', baked: 'suppress' },
...
export function mergeAsideKinds(primary, extra){
...
export function buildNightlyApply(inp){
  ...
  const recentBags = inp && inp.omnirouteRecentBags ? inp.omnirouteRecentBags : null;
  const orAsideBase = dayAsideKinds(orBags);
  const orAsideRecent = recentBags ? dayAsideKinds(recentBags, RECENT_DAY_MIN_N) : [];
  const orAside = mergeAsideKinds(orAsideBase, orAsideRecent);
  ...
  return {
    ...
    omniroute: {
      dayBags: orBags,
      recentDayBags: recentBags || null,
      recentDayAside: orAsideRecent,
      dayAside: orAside,
      dayPrefer: orPrefer
    },
    omnipresent: {
    ...
    omnigold1: og1,
    desk,
    note: 'Rolling 40-day book + last ' + RECENT_DAY_BARS + 'h overlay retunes which setups form tomorrow across OMNIROUTE / OMNIPRESENT / OMNIGOLD 1 and the 19 desk tabs. Baked toxic kinds stay stood aside. Never invents tickets.'
  };
}
