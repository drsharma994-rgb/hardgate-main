...
import {
  deliverLpDigest, digestChannelsReady, digestTelegramReady, digestEmailConfigured,
  digestCronAuthOk, loadDigestState, saveDigestState,
  ...
  return async function paperbookHandler(req, res){
    try{
      ...
      if (method === 'GET' && (u.pathname === '/api/book' || u.pathname === '/api/book/report')){
        ...
        var report = { summary: pbSummary(ctx.book), attribution: pbAttribution(ctx.book) };
      ...
      if (method === 'POST' && u.pathname === '/api/book/intent'){
        ...
        var result = pbAddIntent(ctx.book, body);
        if (!result.ok){
        ...
        return sendJson(res, 200, {
          ...
          summary: pbSummary(finIntent.book),
          check: result.check,
        });
      }
      ...
      if (method === 'POST' && u.pathname === '/api/book/live'){
        ...
        var finLive = persist(ctx, pbPushBlotter(ctx.book, {
      ...
      if (method === 'POST' && u.pathname === '/api/book/digest/send'){
        ...
        var delivered = await deliverLpDigest(finDig.book, sendPeriod);
        if (delivered.ok){
      ...
      if (method === 'POST' && u.pathname === '/api/book/close'){
        ...
        return sendJson(res, 200, { ok: true, fundId: finC.fundId, position: closed.position, summary: pbSummary(finC.book) });
      }
      ...
      if (method === 'POST' && u.pathname === '/api/book/scale'){
        ...
        return sendJson(res, 200, { ok: true, fundId: finS.fundId, position: scaled.position, summary: pbSummary(finS.book) });
      }
      ...
      if (method === 'POST' && u.pathname === '/api/book/stop'){
        ...
        return sendJson(res, 200, { ok: true, fundId: finSt.fundId, position: moved.position, summary: pbSummary(finSt.book) });
      }
