...
import {
  pbNewBook, pbAddIntent, pbClosePosition, pbMarkBook, pbSummary,
...
export function createPaperbookApi(rootDir){
  ...
  return async function paperbookHandler(req, res){
    try{
      ...
      if (method === 'GET' && u.pathname === '/api/book'){
        return sendJson(res, 200, { ok: true, book: book, summary: pbSummary(book) });
      }
      ...
      if (method === 'POST' && u.pathname === '/api/book/reset'){
        var fresh = pbNewBook();
        save(fresh);
        return sendJson(res, 200, { ok: true, book: fresh, summary: pbSummary(fresh) });
      }
