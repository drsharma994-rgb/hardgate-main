...
import {
  ...
  pbBuildLiveOrder, pbPushBlotter, pbApplyExecuteFill, pbPositionsNeedingFillPoll,
...
function executeFillAuthOk(req){
  var secret = process.env.BOOK_EXECUTE_FILL_SECRET || '';
  if (!secret) return true;
  var hdr = (req.headers && (req.headers['x-book-fill-secret'] || req.headers['authorization'])) || '';
  return hdr === secret || hdr === ('Bearer ' + secret);
}
        ...
        for (var pi = 0; pi < pollIds.length; pi++){
          ...
          for (var ni = 0; ni < needs.length; ni++){
            ...
            if (pollRes.ok && pollRes.fill){
              var applyRes = pbApplyExecuteFill(pollBook, Object.assign({
                positionId: pos.id,
                note: pollRes.fill.note || 'broker fill poll',
              }, pollRes.fill));
              if (applyRes.ok){
                pollBook = applyRes.book;
                bookChanged = true;
                filled++;
                row.fillPct = applyRes.position && applyRes.position.brokerFillPct;
              } else {
                row.ok = false;
                row.reason = applyRes.reason || 'apply failed';
              }
            }
