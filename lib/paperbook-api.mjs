    ...
    try{
      ...
      if (method === 'GET' && u.pathname === '/api/book/capabilities'){
        return sendJson(res, 200, {
          ...
          autoRules: true,
          digestWebhook: !!digestWebhookUrl(),
        });
      }
      ...
      if (method === 'GET' && (u.pathname === '/api/book' || u.pathname === '/api/book/report')){
        ...
        return sendJson(res, 200, { ok: true, book: book, summary: summary, attribution: report.attribution, navHistory: book.navHistory || [], blotter: book.blotter || [], capabilities: { liveExecute: !!liveWebhookUrl(), digestWebhook: !!digestWebhookUrl() } });
      }
      ...
      if (method === 'POST' && u.pathname === '/api/book/digest/send'){
        ...
        var digestHook = digestWebhookUrl();
        if (!digestHook) return sendJson(res, 503, { ok: false, reason: 'Set LP_DIGEST_WEBHOOK_URL on Render to enable digest webhook' });
        book = finalize(book);
        save(book);
        var sendPeriod = (digestSendBody && digestSendBody.period) || 'week';
        var sendDigest = pbWeeklyDigest(book, sendPeriod);
        var sendPayload = {
          type: 'hardgate_lp_digest',
          period: sendDigest.period,
          digest: sendDigest,
          text: pbDigestText(sendDigest),
          html: pbDigestHtml(sendDigest),
        };
        var digestRes = await fetch(digestHook, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(sendPayload),
        });
        var digestText = '';
        try{ digestText = await digestRes.text(); }catch(e){}
        return sendJson(res, digestRes.ok ? 200 : 502, {
          ok: digestRes.ok,
          status: digestRes.status,
          digest: sendDigest,
          response: digestText.slice(0, 500),
        });
      }
