...
import { pbCluster, pbClusterCheck, pbClusterHeat } from './clusters.mjs';
import { rfRealisticPnl } from './inc34-core.mjs';
import {
  intentToCandidate, attachFingerprint, formationCfgFromEnv,
  fqsGate, edgeGate, buildEdgeTable, ledgerClosedRows, fpClass,
...
/** Parse BOOK_MAX_DAILY_LOSS_PCT — accepts 0.02 or 2 (percent). */
export function parseMaxDailyLossPct(raw){
  if (raw == null || raw === '') return PB_DEFAULTS.maxDailyLossPct;
  var n = +raw;
  if (!(isFinite(n) && n > 0)) return PB_DEFAULTS.maxDailyLossPct;
  if (n > 1) n = n / 100;
  return Math.min(n, 0.5);
}

export function pbBookCfgFromEnv(env){
  env = env || {};
  return { maxDailyLossPct: parseMaxDailyLossPct(env.BOOK_MAX_DAILY_LOSS_PCT) };
}
...
export function pbRiskCheck(book, intent, cfg, ctx){
  ctx = ctx || {};
  cfg = Object.assign({}, PB_DEFAULTS, cfg || {});
  var reasons = [];
  var nav = (+book.navUsd > 0) ? +book.navUsd : cfg.navUsd;
  var dl = pbDailyLossState(book, cfg);
  if (dl.halted){
    reasons.push('daily loss halt — day P&L ' + (dl.dayPnlUsd >= 0 ? '+' : '') + dl.dayPnlUsd.toFixed(0)
      + ' USD vs limit -' + dl.limitUsd.toFixed(0) + ' USD (' + Math.round(dl.pct * 100) + '% of day start)');
  }
  ...
  var clChk = pbClusterCheck(
    { sym: intent.sym, klass: intent.klass, dir: intent.dir, riskUsd: riskUsd },
    book && book.positions ? book.positions : [],
    nav,
    { maxClusterHeatPct: cfg.maxClusterHeatPct, maxNetBetaHeatPct: cfg.maxNetBetaHeatPct }
  );
  if (clChk.veto){
    veto = true;
    reasons = reasons.concat(clChk.reasons);
  }
  if (ctx && ctx.gates && typeof ctx.gates.record === 'function'){
    ctx.gates.record('cluster', !clChk.veto, { symbol: intent.sym, reason: clChk.reasons && clChk.reasons[0] });
  }
  return { ok: !veto, veto: veto, reasons: reasons, notionalUsd: notional,
           riskUsd: riskUsd, bucket: bucket, cluster: clChk.cluster };
}

export function pbFormationCheck(book, intent, ctx){
  ctx = ctx || {};
  var cfg = ctx.cfg || formationCfgFromEnv(ctx.env || {});
  var cand = intentToCandidate(intent);
  if (!cand) return { ok: true, cand: null, skipped: false };
  attachFingerprint(cand, cand);
  var gates = ctx.gates || null;
  var ledgerRows = ctx.ledgerRows || ledgerClosedRows(book, ctx.outcomes);
  var edgeTbl = ctx.edgeTbl || buildEdgeTable(ledgerRows, { priorWeight: cfg.priorWeight });
  ...
  if (gates) gates.record('fqs', q.ok || !cfg.fqsGate, { symbol: cand.sym, reason: q.reason, fqs: cand.fqs });
  if (cfg.fqsGate && !q.ok){
    return { ok: false, veto: true, reason: q.reason, gate: 'fqs', cand: cand, quality: q.quality };
  }

  var eg = edgeGate(cand, edgeTbl, { minN: cfg.edgeMinN, blockBelowExpLB: cfg.edgeBlockLB });
  if (gates) gates.record('edge', eg.ok || !cfg.edgeGate, { symbol: cand.sym, reason: eg.reason });
  if (cfg.edgeGate && !eg.ok){
    return { ok: false, veto: true, reason: eg.reason, gate: 'edge', cand: cand, edgeRow: eg.row };
  }
  cand.sizeMult = eg.mult || 1;
  return { ok: true, veto: false, cand: cand, quality: q.quality, edgeRow: eg.row, edgeTbl: edgeTbl };
}

export function pbAddIntent(book, intent, cfg, ctx){
  ctx = ctx || {};
  var form = pbFormationCheck(book, intent, ctx);
  if (!form.ok){
    return { ok: false, book: book, check: { ok: false, veto: true, reasons: [form.reason || 'formation gate'], formation: form } };
  }
  if (form.cand){
    intent = Object.assign({}, intent, {
      fpKey: form.cand.fpKey,
      fpParts: form.cand.fpParts,
      fqs: form.cand.fqs,
      fqsGrade: form.cand.fqsGrade,
      fqsPillars: form.cand.fqsPillars,
      sizeMult: form.cand.sizeMult,
    });
    if (form.cand.sizeMult && form.cand.sizeMult !== 1 && intent.riskPct){
      intent = Object.assign({}, intent, { riskPct: intent.riskPct * form.cand.sizeMult });
    }
  }
  if (intent.riskScale != null && isFinite(+intent.riskScale) && +intent.riskScale > 0 && +intent.riskScale < 1){
    var baseRp = intent.riskPct || cfg.defaultRiskPct;
    intent = Object.assign({}, intent, { riskPct: baseRp * +intent.riskScale });
  }
  var chk = pbRiskCheck(book, intent, cfg, ctx);
  ...
  var pos = {
    ...
    bucket: chk.bucket,
    cluster: chk.cluster || pbCluster(intent.sym, intent.klass),
    entry: +intent.entry,
    stop: +intent.stop,
    ...
    fpKey: intent.fpKey || null,
    fpParts: intent.fpParts || null,
    fqs: intent.fqs != null ? intent.fqs : null,
    fqsGrade: intent.fqsGrade || null,
    fqsPillars: intent.fqsPillars || null,
    ...
    rr: intent.rr,
    res: intent.res || intent.tf || '4h',
    setupKind: intent.setupKind || intent.strategy || null,
    timeStopBars: isFinite(intent.timeStopBars) ? +intent.timeStopBars : null,
...
/** Record broker fill qty against an open paper position (webhook / manual reconcile). */
export function pbApplyExecuteFill(book, fill){
  fill = fill || {};
  var id = fill.positionId;
  if (!id) return { ok: false, book: book, reason: 'positionId required' };
  var positions = [];
  var updated = null;
  for (var i = 0; i < (book.positions || []).length; i++){
    var p = book.positions[i];
    if (p.id !== id){ positions.push(p); continue; }
    var mark = isFinite(p.mark) && p.mark > 0 ? p.mark : p.entry;
    var orderQty = isFinite(fill.qty) && fill.qty > 0 ? +fill.qty : (p.notionalUsd / mark);
    var filledQty = Math.max(0, Math.min(+fill.filledQty || 0, orderQty));
    if (!(filledQty > 0)) return { ok: false, book: book, reason: 'filledQty required' };
    var fillPct = orderQty > 0 ? filledQty / orderQty : 0;
    updated = Object.assign({}, p, {
      brokerOrderQty: orderQty,
      brokerFilledQty: filledQty,
      brokerFillPct: fillPct,
      brokerFillAt: Date.now(),
      brokerAvgPrice: isFinite(fill.avgPrice) ? +fill.avgPrice : mark,
    });
    positions.push(updated);
  }
  if (!updated) return { ok: false, book: book, reason: 'position not found' };
  var next = Object.assign({}, book, { positions: positions, at: Date.now() });
  next = pbPushBlotter(next, {
    type: 'execute_fill',
    positionId: id,
    sym: updated.sym,
    dir: updated.dir,
    ...
    filledQty: filledQty,
    fillPct: fillPct,
    avgPrice: updated.brokerAvgPrice,
    note: String(fill.note || '').slice(0, 200),
  });
  return { ok: true, book: next, position: updated };
}

/** Latest execute_ok / execute_fail / live_send blotter row for a paper position id. */
export function pbLatestExecForPosition(blotter, positionId){
...
export function pbBracketSentForPosition(blotter, positionId){
  var evt = pbLatestExecForPosition(blotter, positionId);
  if (!evt) return false;
...
/** Open rows with bracket sent but broker fill missing or partial. */
export function pbPositionsNeedingFillPoll(book){
  book = book || {};
  ...
  for (var i = 0; i < (book.positions || []).length; i++){
    var p = book.positions[i];
    if (!p || !p.id) continue;
    if (!pbBracketSentForPosition(blotter, p.id)) continue;
    var pct = p.brokerFillPct;
    if (pct >= 0.999) continue;
    out.push({ position: p, execEvent: pbLatestExecForPosition(blotter, p.id) });
  }
  return out;
}

/** CSV/UI label for broker fill state on a position row. */
export function pbFillExportLabel(position, blotter){
  var p = position;
  if (!p) return '';
  var pct = p.brokerFillPct;
  if (pct >= 0.999) return 'FILL OK';
  if (pct > 0) return 'FILL ' + Math.round(pct * 100) + '%';
  if (p.id && pbBracketSentForPosition(blotter, p.id)) return 'UNFILLED';
  return '';
}

export function pbFillBacklogSummary(book){
  book = book || {};
  var blotter = book.blotter || [];
  ...
  for (var i = 0; i < positions.length; i++){
    ...
    var pct = p.brokerFillPct;
    if (!(pct > 0)) unfilled++;
    else if (pct < 0.999) partial++;
  }
  return { unfilled: unfilled, partial: partial, total: unfilled + partial };
}

/** Rolling execute bracket stats for digest (events since startMs + open without any bracket). */
export function pbDigestExecuteSummary(book, startMs){
  book = book || {};
  ...
  for (var p = 0; p < positions.length; p++){
    var pos = positions[p];
    if (pos && pos.id && !pbLatestExecForPosition(blotter, pos.id)) pending++;
  }
  return { ok: ok, fail: fail, pending: pending, total: ok + fail, liveOk: liveOk, liveFail: liveFail };
}

export function pbMonthStartMs(at){
  var d = new Date(isFinite(at) ? at : Date.now());
...
export function pbClosePosition(book, id, mark){
  ...
  for (var i = 0; i < (book.positions || []).length; i++){
    ...
    if (p.id === id){
      ...
      var risk = pbUnitRisk(hit);
      if (risk > 0 && isFinite(hit.mark)){
        var move = hit.dir === 'short' ? (hit.entry - hit.mark) : (hit.mark - hit.entry);
        hit.r = move / risk;
        hit.realizedR = hit.r;
        var rf = rfRealisticPnl(hit, { atr: hit.atr, feeBps: 4 });
        hit.idealRealizedUsd = hit.realizedUsd;
        hit.realisticRealizedUsd = rf.realisticUsd;
        hit.executionDragUsd = rf.dragUsd;
        hit.idealR = hit.r;
        hit.realisticR = rf.realisticR;
      }
      closed.unshift(hit);
...
/** UTC-day loss vs day-start equity; halts new risk when beyond maxDailyLossPct. */
export function pbDailyLossState(book, cfg){
  cfg = Object.assign({}, PB_DEFAULTS, cfg || {});
  book = pbRollDay(book || {});
  var dayStart = isFinite(book.dayStartEquityUsd) ? book.dayStartEquityUsd : pbEquity(book);
  var equity = pbEquity(book);
  var dayPnl = equity - dayStart;
  var pct = isFinite(cfg.maxDailyLossPct) ? +cfg.maxDailyLossPct : PB_DEFAULTS.maxDailyLossPct;
  var limitUsd = dayStart * pct;
  var halted = pct > 0 && limitUsd > 0 && dayPnl <= -limitUsd;
  return {
    halted: halted,
    dayPnlUsd: dayPnl,
...
export function pbSummary(book, cfg){
  ...
  var clusterHeat = pbClusterHeat(book.positions, nav);
  return {
    navUsd: nav,
    equityUsd: equity,
    ...
    dayStartEquityUsd: dayStart,
    dayKey: book.dayKey || null,
    dailyLossHalt: dl.halted,
    dailyLossLimitUsd: dl.limitUsd,
    maxDailyLossPct: dl.pct,
    heatUsd: exp.heat,
...
const PB_SEC_PER_RES = { '15m': 900, '1h': 3600, '4h': 14400, '1d': 86400 };

function pbTimeStopDue(p, nowMs){
  if (!p || !p.openedAt || !p.timeStopBars) return false;
  var sec = PB_SEC_PER_RES[p.res || '4h'] || 14400;
  var elapsed = ((nowMs || Date.now()) - p.openedAt) / 1000;
  return elapsed >= (+p.timeStopBars) * sec;
}
...
export function pbApplyAutoRules(book, rules){
  ...
  for (var iter = 0; iter < 10; iter++){
    ...
    for (var i = 0; i < positions.length; i++){
      var p = positions[i];
      if (rules.timeStop && pbTimeStopDue(p)){
        var clTs = pbClosePosition(b, p.id, p.mark);
        if (clTs.ok){
          b = clTs.book;
          actions.push({ action: 'time_stop', sym: p.sym, dir: p.dir, mark: p.mark, bars: p.timeStopBars });
          acted = true;
          break;
        }
      }
...
export function pbWeeklyDigest(book, period){
  ...
  if (period === 'month'){
    ...
    return Object.assign({}, lp, {
      ...
      execute: pbDigestExecuteSummary(book, pbMonthStartMs()),
      fill: pbFillBacklogSummary(book),
    });
  }
  var now = Date.now();
  ...
  return {
    ...
    execute: pbDigestExecuteSummary(book, startMs),
    fill: pbFillBacklogSummary(book),
    at: now,
  };
}
...
export function pbDigestText(d){
  ...
  if (d.execute && (d.execute.ok || d.execute.fail || d.execute.pending)){
    lines.push('Brackets: ' + d.execute.ok + ' OK · ' + d.execute.fail + ' fail · '
      + d.execute.pending + ' open without bracket');
  }
  if (d.execute && (d.execute.liveOk || d.execute.liveFail)){
    lines.push('LIVE webhook: ' + d.execute.liveOk + ' OK · ' + d.execute.liveFail + ' fail');
  }
  if (d.fill && (d.fill.unfilled || d.fill.partial)){
    lines.push('Broker fills: ' + d.fill.unfilled + ' unfilled · ' + d.fill.partial + ' partial');
  }
...
export function pbDigestHtml(d){
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + title + '</title>'
    ...
    + '<tr><th>Open</th><td>' + (d.openCount || 0) + '</td></tr>'
    + (d.execute && (d.execute.ok || d.execute.fail || d.execute.pending)
      ? '<tr><th>Brackets</th><td>' + d.execute.ok + ' OK · ' + d.execute.fail + ' fail · '
        + d.execute.pending + ' open w/o bracket</td></tr>' : '')
    + (d.execute && (d.execute.liveOk || d.execute.liveFail)
      ? '<tr><th>LIVE webhook</th><td>' + d.execute.liveOk + ' OK · ' + d.execute.liveFail + ' fail</td></tr>' : '')
    + (d.fill && (d.fill.unfilled || d.fill.partial)
      ? '<tr><th>Broker fills</th><td>' + d.fill.unfilled + ' unfilled · ' + d.fill.partial + ' partial</td></tr>' : '')
    + '</table>'
