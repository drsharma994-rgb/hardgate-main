  ...
  return async function notifyHandler(req, res){
    try{
      ...
      if (method === 'POST' && u.pathname === '/api/notify'){
        /* Public-URL guard: anyone hitting a published pplx.app URL could burn the
           bot's rate limit / send arbitrary alerts. Require the same shared secret
           the mutating routes use — fails closed if HARDGATE_API_SECRET is unset. */
