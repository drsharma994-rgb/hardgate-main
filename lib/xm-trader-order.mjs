...
export async function xmPlaceOrder(order, opts){
  ...
  /* THE RATE LIMIT COUNTS LIVE ORDERS, SO IT IS SPENT WHEN ONE IS SENT.

     xmOrderRateAllow() records a hit as it answers, and it used to be called
     BEFORE the route probe. A bridge that was unreachable therefore burned the
     budget: measured against a dead bridge, six of eight attempts spent a slot
     and the last two were refused "rate limit" — with not one order placed.
     The cap on live orders had become a cap on failures, and it locked the
     desk out for five minutes at the moment the bridge came back.

     The probe goes first now. Nothing is charged until there is a route to
     post to. */
  ...
  for (i = 0; i < paths.length; i++){
    ...
    /* A 404 MEANS THE ROUTE IS ABSENT. EVERYTHING ELSE MEANS SOMETHING WENT
       WRONG, WHICH IS NOT THE SAME THING.

       This loop used to fall through to the next route name on ANY failure,
       so a bridge answering 500 took the SAME order six times — /order,
       /trade, /api/order, /api/trade, /order/send, /api/v1/order — measured,
       all six. Most bridges map several of those names onto one handler, so a
       transient 5xx after the broker had accepted could leave several live
       gold positions from one ticket. The idempotency map in
       omnigold-xm-bot.mjs sits above this and only records on success, so it
       cannot suppress them either.

       Route discovery is what the 404 branch above is for, and the probe has
       already done it. Past that, a POST that placed an order is not retried
       on a different name just because the reply was unhappy. */
    return last;
  }
  return last;
}
