...
async function fetchCoindcxJson(url, ttlMs){
  var up = await coindcxFetchCached(url, ttlMs);
  if (!up.ok) return null;
  try{ return JSON.parse(up.text); }catch(e){ return null; }
}
...
export async function fetchDualUniverse(){
  ...
  var cdcxRaw = await fetchCoindcxJson(coindcxInstrumentsUrl(), 15 * 60 * 1000);
  var marksRaw = await fetchCoindcxJson(coindcxMarksUrl(), 90 * 1000);
...
export async function fetchVenueCandles(item, tf, count){
  ...
  if (item.exchange === 'delta'){
    ...
    var to = now * 1000;
    var cj = await fetchCoindcxJson(coindcxCandlesUrl(item.sym, from, to, cres), 45 * 1000);
...
export function venueUniverse(items, venue){
  return (items || [])
...
export function topByTurnover(items, venue, n){
  var list = venueUniverse(items, venue);
