...
export function edgeStats(a, prior, priorWeight) {
  ...
  const se = a.n ? Math.max(sd / Math.sqrt(a.n), 1 / Math.sqrt(a.n + priorWeight)) : 0;
  return {
    n: a.n,
...
function stats(a, prior, priorWeight) {
  return edgeStats(a, prior, priorWeight);
}
