/* HARDGATE — /api/chart-vision/* OHLCV chart reading + next-move analysis. */
import {
  ...
  return async function chartVisionHandler(req, res){
    try{
      ...
      if (method === 'GET' && u.pathname === '/api/chart-vision/capabilities'){
        return sendJson(res, 200, chartVisionCapabilities(process.env));
      }
      ...
      if (method === 'POST' && u.pathname === '/api/chart-vision/analyze'){
        ...
        if (!force && __cache.has(key)){
          ...
          if (now - hit.at < CACHE_MS){
            return sendJson(res, 200, { ok: true, cached: true, key: key, analysis: hit.analysis, svg: hit.svg, ms: 0 });
          }
        }
        var t0 = Date.now();
        var out = await chartVisionAnalyze(input, process.env, chartVisionGeminiCall);
        __cache.set(key, { at: now, analysis: out.analysis, svg: out.svg });
