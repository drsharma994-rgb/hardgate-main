...
/* HOW FAR PRICE TRADED THROUGH A RESTING ORDER, in price units.
 *
 * ogXmBarTouchesEntry above fills a pending order the instant the bar's
 * extreme reaches it. That is the standard backtest convention and it is
 * the most optimistic one available: a bar whose low is EXACTLY the limit
 * price fills every single time in the walk, while in a real book that is
 * one print at the extreme tick with a queue in front of it. Nothing in
 * this repo has ever measured how much of the fill rate rests on that.
 *
 * The depth is what distinguishes the two cases:
 *
 *   0          the bar's extreme merely kissed the order — least credible
 *   large      price traded well through it — the fill is not in doubt
 *
 * Returned in price units; the caller normalises (by risk for an R figure,
 * by the bar range for a within-bar one). A market order returns null
 * rather than 0: it fills at the open with no queue at all, so "depth"
 * is not a question that applies to it, and 0 would read as the worst
 * possible fill instead of no reading.
 *
 * Measurement only. No walk behaviour depends on this — it exists so the
 * question can be answered from an artifact instead of argued about. */
export function ogXmFillDepth(typeName, dir, bar, entry){
  if (!bar) return null;
  var e = num(entry);
  if (!(e > 0)) return null;
  var name = String(typeName || '');
  if (name === 'BUY' || name === 'SELL') return null;   /* fills at the open */
  var h = num(bar.h), l = num(bar.l);
  if (!isFinite(h) || !isFinite(l)) return null;

  var through;
  if (name === 'BUY_LIMIT' || name === 'SELL_STOP') through = e - l;
  else if (name === 'SELL_LIMIT' || name === 'BUY_STOP') through = h - e;
  else through = (dir === 'short') ? (h - e) : (e - l);

  /* negative means the bar never reached the order — not a fill at all */
  return (isFinite(through) && through >= 0) ? through : null;
}
