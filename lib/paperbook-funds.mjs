...
import { pbNewBook, pbSummary, PB_DEFAULTS, pbLpReport, pbWeeklyDigest, pbAttribution, pbDigestExecuteSummary, pbMonthStartMs, pbFillBacklogSummary } from './paperbook-core.mjs';
...
export function pbConsolidatedDesk(store, cfg){
  store = pbNormalizeStore(store);
  var ids = Object.keys(store.funds || {}).sort();
  ...
  for (var i = 0; i < ids.length; i++){
    ...
    var fill = pbFillBacklogSummary(book);
    execOk += ex.ok || 0;
    execFail += ex.fail || 0;
    execPending += ex.pending || 0;
    execLiveOk += ex.liveOk || 0;
    execLiveFail += ex.liveFail || 0;
    fillUnfilled += fill.unfilled || 0;
    fillPartial += fill.partial || 0;
  ...
  var lossPct = cfg.maxDailyLossPct;
  ...
  return {
    ...
    execute: { ok: execOk, fail: execFail, pending: execPending, total: execOk + execFail,
      liveOk: execLiveOk, liveFail: execLiveFail, period: '7d' },
    fill: { unfilled: fillUnfilled, partial: fillPartial, total: fillUnfilled + fillPartial },
    at: Date.now(),
  };
}
...
export function pbConsolidatedLp(store, period, monthKey){
  ...
  return {
    ...
    byFund: byFund,
    execute: { ok: execOk, fail: execFail, pending: execPending, total: execOk + execFail,
      liveOk: execLiveOk, liveFail: execLiveFail },
    fill: { unfilled: fillUnfilled, partial: fillPartial, total: fillUnfilled + fillPartial },
    at: Date.now(),
  };
}
...
export function pbConsolidatedDigestText(c){
  ...
  if (c.execute && (c.execute.liveOk || c.execute.liveFail)){
    lines.push('LIVE webhook: ' + c.execute.liveOk + ' OK · ' + c.execute.liveFail + ' fail');
  }
  if (c.fill && (c.fill.unfilled || c.fill.partial)){
    lines.push('Broker fills: ' + c.fill.unfilled + ' unfilled · ' + c.fill.partial + ' partial');
  }
  var funds = c.byFund || [];
  if (funds.length){
    lines.push('By fund:');
...
export function pbConsolidatedHtml(c){
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>HARDGATE Consolidated LP</title>'
    ...
    + (c.execute && (c.execute.liveOk || c.execute.liveFail)
      ? '<tr><th>LIVE webhook</th><td>' + c.execute.liveOk + ' OK · ' + c.execute.liveFail + ' fail</td></tr>' : '')
    + (c.fill && (c.fill.unfilled || c.fill.partial)
      ? '<tr><th>Broker fills</th><td>' + c.fill.unfilled + ' unfilled · ' + c.fill.partial + ' partial</td></tr>' : '')
    + '</table>'
