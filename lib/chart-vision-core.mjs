/* HARDGATE — chart vision core: OHLCV → SVG + structured next-move analysis.
   Heuristic path works offline (tests/CI); GEMINI_API_KEY enables LLM refinement. */
...
export function chartVisionCapabilities(env){
  ...
  return {
    ...
    vetoEnabled: env.CHART_VISION_VETO === '1' || env.CHART_VISION_VETO === 'true',
  };
}
...
export function chartVisionShouldVeto(dir, analysis, env){
  ...
  if (!(env.CHART_VISION_VETO === '1' || env.CHART_VISION_VETO === 'true')) return false;
...
export async function chartVisionAnalyze(input, env, geminiFn){
  ...
  var base = chartVisionHeuristic(rows, dir, Object.assign({}, input.context || {}, plan));
  var stats = base.indicators || {};

  if (env.GEMINI_API_KEY && String(env.GEMINI_API_KEY).trim() && typeof geminiFn === 'function'){
    try{
      var prompt = chartVisionBuildPrompt(input, Object.assign({}, stats, {
        heuristicBias: base.bias, heuristicPattern: base.pattern, heuristicConfidence: base.confidence,
      }));
      var gem = await geminiFn({
        ...
        prompt: prompt,
        svg: svg,
        pngBase64: input.pngBase64 || null,
      });
      ...
      if (parsed){
        ...
        return { ok: true, analysis: parsed, svg: svg, ms: gem.ms || 0 };
      }
    }catch(e){
  ...
  return { ok: true, analysis: base, svg: svg, ms: 0 };
}
