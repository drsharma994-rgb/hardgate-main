...
import {
  chartVisionTaStats,
  chartVisionRenderProSvg,
  chartVisionSkillPrompt,
  CHART_VISION_SKILL_REF,
} from './chart-vision-skill.mjs';
...
export function chartVisionCapabilities(env){
  ...
  return {
    ...
    clientPngAccepted: true,
    skillRef: CHART_VISION_SKILL_REF,
...
export function chartVisionRenderSvg(rows, plan){
  ...
  var stats = chartVisionTaStats(rows, plan || {});
  return chartVisionRenderProSvg(rows, plan || {}, stats);
}
...
export function chartVisionBuildPrompt(input, stats, heuristic){
  return chartVisionSkillPrompt(input, stats, heuristic);
}
...
export async function chartVisionAnalyze(input, env, geminiFn){
  ...
  var taStats = chartVisionTaStats(rows, plan);
  var svg = chartVisionRenderSvg(rows, plan);
  var base = chartVisionHeuristic(rows, dir, ctx);
  var stats = Object.assign({}, base.indicators || {}, taStats);
  ...
  if (useGemini && !pngBase64 && svg){
    try{
      var envPng = Object.assign({}, env);
      pngBase64 = await chartVisionSvgToPng(svg, envPng);
    }catch(e){}
  }

  if (useGemini){
    try{
      var prompt = chartVisionBuildPrompt(input, stats, {
        bias: base.bias,
        pattern: base.pattern,
        confidence: base.confidence,
        outcomeProb: base.outcomeProb,
        predictedPath: base.predictedPath,
      });
      ...
      geminiModel = gem.model || null;
      var parsed = chartVisionParseModelText(gem && gem.text);
      if (parsed){
        parsed.note = (parsed.note || base.note) + (geminiModel ? ' · gemini:' + geminiModel : '');
        parsed.source = pngBase64 ? 'gemini-png' : 'gemini-svg';
        if (!parsed.nextBarOpinion && base.nextBarOpinion) parsed.nextBarOpinion = base.nextBarOpinion;
      ...
      geminiError = 'parse failed';
    }catch(e){
      geminiError = (e && e.message) || String(e);
      base.note = (base.note || '') + ' · gemini: ' + geminiError;
    }
  }
