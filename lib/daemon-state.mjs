...
import { hgReadStateFile, hgWriteStateFile, hgStateEncryptionEnabled } from './daemon-state-crypto.mjs';
...
export class StateDatabase {
  ...
  _read(){
    try{
      ...
      if (hgStateEncryptionEnabled()){
        var dec = hgReadStateFile(this.filePath);
        if (dec && typeof dec === 'object'){
          this.state = {
            version: dec.version || 1,
            convictions: Array.isArray(dec.convictions) ? dec.convictions : [],
            orders: Array.isArray(dec.orders) ? dec.orders : [],
            outcomes: Array.isArray(dec.outcomes) ? dec.outcomes : [],
  ...
  _write(){
    try{
      ...
      if (hgStateEncryptionEnabled()){
        hgWriteStateFile(this.filePath, this.state);
        return;
      }
