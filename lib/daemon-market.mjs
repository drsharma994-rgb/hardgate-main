...
import { hgTimeBarrierExpired } from './triple-barrier.mjs';
...
export async function evaluateActiveConvictions(ctx){
  ...
  for (var i = 0; i < ids.length; i++){
    ...
    if (!candle) continue;
    var status = null;
    if (hgTimeBarrierExpired(setup, ctx.barIndex)){
      status = 'TIME STOP';
    } else {
      var is15 = is15mBarClose(candle);
      status = mgr.evaluateSetup(setup, candle, is15, false, now, ctx.barIndex);
    }
    if (status){
      setup.status = status;
