...
export function filterExecutableBrainRows(rows, tiers, ctx){
  ...
  if (liveMode){
    return filterDaemonBrainRows(rows, { liveMode: true, tiers: tiers || ['PRIME'] })
      .filter(function(r){ return !cooldownCheck(r.sym, outcomes, cdCfg, now).blocked; });
  }
  ...
  for (var i = 0; i < rows.length; i++){
    ...
    if (cd.blocked){
      if (ctx.onSkip) ctx.onSkip(row.sym, cd.reason);
      continue;
    }
    out.push(row);
  }
...
export async function runMarketScan(ctx){
  ...
  var executable = filterExecutableBrainRows(brain.rows, undefined, {
    outcomes: db && typeof db.recentOutcomes === 'function' ? db.recentOutcomes(100) : [],
    onSkip: function(sym, reason){ log('[COOLDOWN SKIP] ' + sym + ' — ' + reason); },
  });
