...
export function composeAtomicDesk(pipeline){
  ...
  return {
    ...
    delta: {
      count: (delta.setups || []).length,
      scanned: delta.scanned || 0,
      universe: delta.universe || delta.scanned || 0,
      ms: delta.ms || 0,
    },
    coindcx: {
      count: (cdcx.setups || []).length,
      scanned: cdcx.scanned || 0,
      universe: cdcx.universe || cdcx.scanned || 0,
      ms: cdcx.ms || 0,
    },
    bestSetups: ranked.slice(0, 40),
    topFindings: ranked.slice(0, 48).map(function(s){
      return {
        sym: s.sym,
        dir: s.dir,
