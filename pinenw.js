/* HARDGATE — pinenw.js
   PINE NW tab: NW Envelope mean-reversion snap-back on 7-gate universe. */
(function(){
'use strict';
...
function buildPlan(dir, price, rows, res){
  var mean = res && fin(+res.meanTarget) ? +res.meanTarget : null;
  var bandStop = null;
  if (res){
      ...
      if (st && fin(+st.stop)){
        ...
        if (risk > 0){
          ...
          return {
            ...
            t1: t1,
            t2: dir === 'long' ? t1 + ext * 0.5 : t1 - ext * 0.5,
  ...
  var t1F = mean !== null ? mean : (dir === 'long' ? price + 2 * riskF : price - 2 * riskF);
  return {
    entry: price, stop: stopF,
    t1: t1F,
    t2: dir === 'long' ? t1F + riskF : t1F - riskF,
    planSrc: mean !== null ? 'NW mean target' : '1.5×ATR fallback'
  };
}

function runNw(rows){
  try{
    var fn = W[NW_SCRIPT.fn];
    if (typeof fn !== 'function') return null;
    return fn(rows, NW_SCRIPT.opts || {});
  }catch(e){ return null; }
}
...
function signalFromResult(item, res, rows){
  ...
  var sig = {
    sym: item.sym,
    dir: item.dir,
    scriptId: NW_SCRIPT.id,
    scriptLabel: NW_SCRIPT.label,
    meanTarget: res.meanTarget,
    nwCenter: res.nwCenter,
    upper: res.upper,
    lower: res.lower,
    atr: res.atr,
...
function cardHTML(sig){
  ...
  var revLabel = sig.dir === 'long' ? 'SNAP BACK ↑' : 'SNAP BACK ↓';
  ...
  return '<div class="panel ' + cls + '" style="margin-bottom:12px">'
    + '<h2>' + esc(sig.sym) + ' <span>' + esc(sig.dir.toUpperCase()) + ' · NW Envelope' + badge + '</span></h2>'
    + '<div class="note">Mean <b>' + pxF(sig.meanTarget) + '</b> · upper ' + pxF(sig.upper) + ' · lower ' + pxF(sig.lower)
    + ' · wick pierce + close inside band'
    + ' · mark ' + pxF(sig.price)
    + (gateNote ? ' · ' + gateNote : '')
    + '</div>'
    ...
    + (typeof W.hgBookBtn === 'function'
      ? W.hgBookBtn(sig.sym, sig.dir, sig.entry, sig.stop, sig.t1, { scanner: 'pine-nw', strategy: sig.scriptId, t2: sig.t2 })
      : '')
    + '</div>';
}

var __pineNwSnap = null;
var __pineNwTab = { busy: false, hasRun: false, run: null };

function mount(el){
  el.innerHTML =
    '<div class="panel">'
    + '<h2>PINE NW <span>Nadaraya-Watson kernel mean · ATR(100)×2.5 envelope · snap-back</span></h2>'
    + '<div class="note">50-bar Gaussian kernel regression centerline with ATR envelope. '
    + '<b>Long</b> when wick pierces lower band but close reclaims inside (bullish reversion). '
    + '<b>Short</b> when wick pierces upper band but close fails back inside. '
    + 'Target = NW mean. Same 7-gate intersection as other Pine tabs.</div>'
    + '<div class="row" style="margin-top:10px">'
    + '<button class="btn" id="pineNwRun">RUN NW SCAN</button>'
    + '<span class="note" id="pineNwStat">Run gate tabs first, then scan.</span>'
    + '</div>'
    + '<div class="prog" id="pineNwProg"><i></i></div>'
    + '<div id="pineNwFunnel" style="margin-top:8px"></div>'
    + '<div id="pineNwOut" style="margin-top:12px"><div class="empty">Press RUN NW SCAN after gate tabs have run.</div></div>'
    + '</div>';

  var btn = el.querySelector('#pineNwRun');
  var stat = el.querySelector('#pineNwStat');
  var prog = el.querySelector('#pineNwProg');
  var out = el.querySelector('#pineNwOut');
  var funnelEl = el.querySelector('#pineNwFunnel');
  ...
  async function runScan(opts){
    opts = opts || {};
    if (__pineNwTab.busy) return 'busy';
    __pineNwTab.busy = true;
    __pineNwTab.hasRun = true;
    ...
    try{
      ...
      if (funnelEl && typeof W.hgFunnelPanelHTML === 'function' && typeof W.pineFunnelRows === 'function'){
        funnelEl.innerHTML = W.hgFunnelPanelHTML('NW gate funnel (all tabs must agree on sym+dir)',
          W.pineFunnelRows(gate.funnel), 'pineNwGateFunnel');
      }
      if (!gate.eligible || !gate.eligible.length){
        ...
        __pineNwSnap = { at: Date.now(), signals: [], gate: gate, stat: stat ? stat.textContent : '' };
        return status;
      }
      ...
      for (var ci = 0; ci < eligible.length; ci += CHUNK){
        ...
        await Promise.all(chunk.map(async function(item, ix){
          var n = ci + ix + 1;
          if (stat) stat.textContent = 'NW ' + n + '/' + eligible.length + ' · ' + item.sym + ' ' + item.dir.toUpperCase();
      ...
      signals.sort(function(a, b){
        ...
        return Math.abs(b.entry - b.meanTarget) - Math.abs(a.entry - a.meanTarget);
      });

      var freshNew = signals.filter(function(s){ return s.isNew; });
      if (!opts.quiet && freshNew.length && typeof W.pineFireAlerts === 'function'){
        try{ await W.pineFireAlerts(freshNew); }catch(eAl){ console.warn('pine nw alert', eAl); }
      }

      __pineNwSnap = { at: Date.now(), signals: signals, gate: gate, stat: '' };

      if (!signals.length){
        if (out) out.innerHTML = '<div class="empty">' + eligible.length + ' gated contracts — no NW envelope snap-back on latest bar.</div>';
      } else {
      ...
      if (stat) stat.textContent = 'done · ' + eligible.length + ' gated · ' + signals.length + ' NW signal(s)'
        + (newN ? (' · ' + newN + ' NEW alerted') : '') + ' · failed ' + failed + ' · ' + dt + 's';
      __pineNwSnap.stat = stat ? stat.textContent : '';
    }catch(e){
      status = 'error: ' + ((e && e.message) || e);
      if (stat) stat.textContent = status;
      if (out) out.innerHTML = '<div class="empty">NW scan failed: ' + esc(status) + '</div>';
    }finally{
      if (btn) btn.disabled = false;
      setProg(null);
      __pineNwTab.busy = false;
    }
    return status;
  }

  if (btn) btn.addEventListener('click', function(){ runScan(); });
  __pineNwTab.run = runScan;
}

async function pineNwRefresh(){
  try{
    if (__pineNwTab.busy) return 'busy';
    if (!__pineNwTab.hasRun || typeof __pineNwTab.run !== 'function') return 'skipped: not run yet';
    return await __pineNwTab.run({ quiet: false });
  }catch(e){
    return 'error: ' + ((e && e.message) || e);
  }
}

W.pineNwScan = function(){ try{ return __pineNwSnap; }catch(e){ return null; } };
W.HG_tabs = W.HG_tabs || [];
