...
var HG_BUILD = {
  version: 'hg-v976',
  pack: 'STAR TRADER GOLD LANE NAMES ITS FEED AND TAKES THE SHARED LIVE FEED; GOLD SWING FEED LABEL IS ORNAMENTAL AND IS SAID TO BE. hg-v974 fed this lane news, macro, the 1h / 4h legs and the 15m signal bar, and stopped at no feed label and no live feed. Both were reachable: the rows come through startraderCandles, whose gold branch walks the XM bridge, getXAUCandles, the macro chain and Yahoo, and every leg knows its source; and stWarmContext is async, so the one shared loader (hg-v971 / v972) rides on the context the lane already receives. Unfed, the mint TRUSTED proxy volume the GOLD SCALP desk distrusts, and the spread lock, the DOM rule and the OI / funding reads were unchecked on the vote this desk casts. The candle chain records the gold source per timeframe, where the rows are fetched and only when rows came back: the XM bridge is the broker feed, getXAUCandles reads the shell per-timeframe record (the same record the DATA chip and GOLD SCALP read, never the last-fetched-TF label), the macro chain names its payload, Yahoo is named as macro.js names it; a crypto fetch never touches it; a 4h fetch leaves the 15m record alone; nothing fetched reads null. The lane hands the 15m label to the mint and applies ctx.goldLive through the ONE applier, which fills only what the input carries none of; stWarmContext loads the feed once, null on every failure. Driven on the real mint through the real lane: seed 102 casts a GOLD SCALP short vote; a wide unnamed quote on the feed LOCKS the mint and the vote is gone; the same width naming delta-xaut, with the book beside it, keeps the vote (reported, not gated); seed 120 votes unnamed, named binance-paxg the volume votes are withheld and the vote is gone, the broker label keeps it - parity with GOLD SCALP, which has always named its feed. GOLD SWING: this file said the desk passes candleSource for volume trust. The swing mint reads that field nowhere - the board is byte-identical with and without a PAXG label on every tape - so the claim is corrected, the field is asserted unread at its two SET sites, and NOT wired: a swing volume-trust rule would be a new gate on a desk with no measurement of what it removes (the hg-v966 trap). The swing replay walk hands no label either, and that changes nothing for the same reason. One vacuous fixture in this pack own guard, caught by its own mutation pass: the crypto case named a contract the catalog does not carry, so the branch never ran and a mutation recording a gold source there survived; reachability is asserted first now. No gate is loosened, no threshold moves, no new writer of any global; one module-private record and its reader are added. Tests: tests/test-startrader-gold-lane-fed.mjs, 37 assertions, with 12/12 behavioural mutations caught across startrader.js and startradertab.js, baseline verified green before every pass',
