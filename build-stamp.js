...
var HG_BUILD = {
  version: 'hg-v930',
  pack: 'ONE POSITION AT A TIME, ON GOLD SCALP TOO - closing an asymmetry that ran for four packs. hg-v926 measured the only read in this desk evidence base that is positive at BOTH fill bounds: taken one at a time the SCALP ticket book is +0.1484R (n=73, conservative bound) and +0.4155R (n=54, the other), against -0.1160R all at once (n=8132 at XM costs). The walk publishes 59.4 plans a day on ONE instrument and holds 57 at once, which is not 57 bets but one bet on gold at 57x size. THE FIX WENT IN ON OMNIGOLD AND STOPPED THERE. hgOgOneAtATimeGate reads BOTH booking stores, so OMNIGOLD held itself back while GOLD SCALP was in a trade and GOLD SCALP went on stacking - the desk whose ticket book supplied the measurement was the one still exposed to it. WHAT HOLDING MEANS HERE: this desk has no gate ledger to push a row through, so a held card is DEMOTED, which is already understood everywhere - it still paints, keeps its levels and its whole tally, and can never lead. Every leader picker (goldPickSpotAlignedBest, the naive scan, the display fallback) already skips demoted rows, so MOST PROBABLE empties with no second code path. AND THE HANDOFF IS THE ACTUAL ENFORCEMENT: a demote alone does not remove ADD TO BOOK or SEND TO TRADE PLAN - hg-v611 had to add a suppress verdict for exactly that, after a demote stamp left four stop-outs bookable - so cardHTML drops both CTAs on a held card and says why in their place rather than going blank. TWO THINGS IT DOES NOT DO. The position you are ALREADY IN keeps running: a row carrying c.locked is skipped, never demoted, and can still lead, which is why the hold is applied AFTER applyConviction stamps that flag and BEFORE the leader is picked. And it FAILS OPEN - gsOpenGoldConvictions DELEGATES to OMNIGOLD hgOgOpenGoldConvictions so one definition of an open gold conviction serves both desks rather than a second parser that would drift the first time either store changed shape; an absent or throwing reader is 0 open and every setup stands, because refusing to trade over a file that did not load is a gate nobody chose. THE LIMIT IS ON THE PANEL, NOT JUST IN A COMMENT: n=73 at +1.16 sigma is positive and NOT SIGNIFICANT, clearing not even an uncorrected single test, and SWING disagrees between the bounds (-0.240 / +0.317) so it carries no verdict at all. What is NOT statistical is the concentration - 57 simultaneous positions on one metal is arithmetic - and that is the half this acts on. Reversible: gsSetOneAtATime(false) persists, window.HG_GS_ONE_AT_A_TIME overrides, and the switch is resolved per scan so headless warms honour it too. NO THRESHOLD MOVED: G1-G7, the 1.2R scalp floor, the stop floors, the cost ceilings and the hg-v925 / v928 / v929 switches are untouched. Tests: tests/test-gold-one-at-a-time.mjs (59 assertions, asserting against the SHIPPED leader picker rather than a copy of its logic); 19 of 19 mutations caught, after a first run left two survivors - one a real gap (an all-locked board reported a hold of zero) and one a mutation of mine too cosmetic to test anything, which is recorded in the file rather than quietly dropped',
