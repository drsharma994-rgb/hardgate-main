...
var HG_BUILD = {
  version: 'hg-v915',
  pack: 'THE SEARCH THAT CAME BACK EMPTY, RECORDED SO IT IS NOT RE-RUN. Packs 909-914 worked through the gold evidence and shipped what held up. Pack 914 took the obvious next idea and refuted it out of sample. This pack asks whether anything else is findable OFFLINE, since the replays cannot be re-run here: sweep every dimension the two desks OWN trade rows expose, fit a keep-rule on the first part of each book, apply it to the held-out rest, at three splits. TWENTY PRE-REGISTERED RULES - ten on GOLD SCALP (stop distance, stopAtr multiple, planned R:R, UTC hour, order type, direction, grade, confluence tally, bars held, killzone weight) and ten on OMNIGOLD (tier, horizon, direction, ticket flag, order type, checksPass, gateConf, confluence band, kind, UTC hour). A rule ships only if it improves the out-of-sample book at ALL THREE splits, leaves it positive, and clears t>1.96. NOT ONE DOES. The closest is SCALP bars-held, which flips the OOS book positive at every split (+0.049, +0.052, +0.025) and never reaches significance (t = 0.72, 0.69, 0.29) - and it is the one rule in the sweep that CANNOT BE USED, because how long a trade runs is not known when the setup forms. That the only rule to improve everywhere is the one carrying look-ahead is what you would expect if the rest is noise, which is why it is kept in the sweep rather than quietly dropped. AND IT SETTLES THE OMNIGOLD VENUE QUESTION. Re-priced at the XM round trip the desk OWN order path uses, by the repo own exact rule, the OMNIGOLD book is -1.241R (t=-41.29) at the baked PAXG 0.26% and -0.116R (t=-7.55) at XM 0.020%. Switching venue is an ENORMOUS improvement and NOT A FIX: the book is still significantly negative at the cheapest venue the desk can trade, so the twelve net-positive kinds do not carry it. A per-kind count is not a book. That is worth knowing before anyone flips a fail-closed default expecting one, and the default is left untouched - the measurement informs that decision, it does not make it. TWENTY TESTS AT 95% EXPECT ONE FALSE POSITIVE BY CHANCE, so a search like this is only honest if the count is reported and the empty result is recorded. The guard re-runs the whole sweep rather than remembering its conclusion: if a re-bake makes something survive, the suite goes red and someone looks. NOTHING WAS WIRED INTO A DESK on the strength of this search, and a test asserts that too. WHAT THIS MEANS FOR THE TWO TABS: GOLD SCALP is now -0.020R (t=-0.58) with the shipped gates, statistically flat rather than the -0.148R (t=-5.71) it started at; OMNIGOLD is -0.116R (t=-7.55) at its best venue. Neither has an offline-discoverable parameter improvement left. The remaining path is genuinely new data - a re-bake in an environment with market access - not more slicing of these books. G1-G7, the gold stop floors and ceilings, the cost-reject bar, the suppress, demote and prefer bars, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
