...
var HG_BUILD = {
  version: 'hg-v912',
  pack: 'GOLD SCALP COST GATE REJECTS, BECAUSE A DEMOTE NEVER STOPPED A TRADE. The desk demoted any setup whose stop sat under 8x the venue round trip (0.16% of entry at XM 0.020%). The reasoning on the line was that the cohort is GROSS-POSITIVE, so it should paint and simply never lead. The cohort IS gross-positive. MEASURED on the tab OWN 15m replay, live rows only with the harness shadow book excluded, net R at XM: the whole live book is n=2193 at -0.148R (t=-5.71); the rows with a stop under the 0.16% bar are n=671 at -0.363R (t=-7.54); what remains once they are rejected is n=1522 at -0.053R (t=-1.75). SO 671 TRADES - 31% OF THE BOOK - CARRIED -243.3R OF ITS -324.3R TOTAL LOSS. A demote stopped none of them: it only stopped them leading, so they painted and they ran. The setups are not wrong, the geometry cannot pay for them. Rejecting removes three quarters of the bleeding and keeps 69% of the volume. WHAT REMAINS IS STILL NEGATIVE (-0.053R, t=-1.75) - this is where the bleeding stops, not where an edge begins, and the pack says so rather than dressing it up. THE SPLIT IS THE EVIDENCE, NOT A PREFERENCE: SCALP rejects, SWING still demotes. The swing replay has ZERO trades under the bar, so there is nothing measured there to reject on - and rarely is not never: the swing ENGINE can produce sub-bar geometry, which a 4h order-block retest anchored just beyond the OB base does at about 0.03% of entry. test-goldswing.mjs mints exactly that, and it went red when the first cut rejected on both lanes, which is how this pack learned the difference between the replayed book and the engine. Rejecting a swing setup on the scalp lane evidence would be the unearned extrapolation this desk avoids, so swing keeps its demote and the gate says which lane it is judging and why. A test asserts the swing replay really is empty of these, so the moment it is not the split needs re-deciding. WHY THIS WAS NOT OBVIOUS, and a correction to an earlier reading in this session: the gate was ALREADY FIRING - 650 of the 787 rows under the bar carry its demote. But the replay harness recorded c.stamps.slice(0, 6) and COST-HEAVY is the 13th stamp the scalp pipeline can add, so on any card with six earlier stamps it was cut from the record. The evidence showed a working gate firing ZERO times, which is how a live gate reads as a dead one. The cap is now 24 and a test holds it there. A SECOND MEASUREMENT ARTIFACT CORRECTED: the cohort under the bar is 787 rows counting the harness SHADOW book of EDGE-suppressed candidates no reader ever sees, and 671 counting only live ones. Every figure in this pack is the live number; counting shadow rows would have inflated the claim by 116 trades. The call sites now read c.dropped after the gate - without that line a rejection reached the board anyway - and the gate sets ONE verdict rather than two. Two tests that pinned demote-only, on the gross-positive reasoning this measurement overturns, were updated to the new contract with the numbers that changed it. G1-G7, the 1.5xATR stop floor, the 3.5x ceiling, the suppress, demote and prefer bars, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
