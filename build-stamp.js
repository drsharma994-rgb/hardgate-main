...
var HG_BUILD = {
  version: 'hg-v947',
  pack: 'GOLD DIRECTION CROWNS ON A PREFER ROW WHOSE OWN TWIN SAYS THE OPPOSITE. This tab crowns ONLY measured-proven candidates and hg-v702 built that proof at scan time from the live surfaces -- but branch (a) admitted EVERY HG_GOLD_SETUP_EDGE prefer row without asking whether that row exact OMNIGOLD twin is past the measured-failure bar. hg-v943 withholds the rank boost on GOLD SCALP and GOLD SWING for exactly that, hg-v946 withholds the crown on GOLD ULTRA, and this desk went on crowning. MEASURED, and it is half the book: of the FOUR prefer rows the live table carries, TWO are p9volbar -- SCALP and SWING -- and its exact twin P9-VOLBAR reads minus 0.499R net at XM over 274 gate-clear trades at minus 2.97 sigma, past the bar at which OMNIGOLD refuses to ticket a mechanic at all. The other two are p6fail, whose twin is NOT past the bar at plus 0.73 sigma, and weekly, which has no twin. So the rule bites on exactly the rows it should and no others, and that is asserted from the live table rather than claimed. IT WITHHOLDS THE CROWN AND NOTHING ELSE: the candidate still forms, keeps its ENTRY STOP and T1, keeps its prefer tier, still paints as a full card and is neither demoted, suppressed, held nor rejected. TWO SEAMS, BECAUSE ONE WOULD HAVE BEEN A SILENT NO-OP. provenOf short-circuits on the candidate own prefer row and NEVER consults the proven set, so fixing buildProvenSet alone would have changed nothing for most candidates -- the hg-v945 trap, and a mutation says so out loud. Both carry the rule and agree by construction. THE SET IS NEVER ALLOWED TO SHRINK IN SILENCE (hg-v940): the withheld rows are counted, named with BOTH samples, printed on the stat line and published on the snapshot beside the set. The snapshot reason and the NOT CROWNED header now tell a candidate nothing has measured apart from one carrying a prefer row its own twin contradicts -- one message for two causes is the v940 failure, in the header this time. SCOPED ON PURPOSE: the omnigold-prefer branch IS the OMNIGOLD record and has nothing to disagree with, and the live-paid branch is this tab own forward ledger on its own gates and horizons -- a third population, measured live, which a stale twin does not speak for. Both are proved untouched. ONE LEVER, NOT A THIRD SWITCH: hgGoldSetTwinCheck(false) reverses it here too, read through hgGoldTwinCheckOn. FAILS OPEN at every seam -- no OMNIGOLD, no verdict, or a throwing reader and the row crowns as before. The disclosure comes from goldind ONE definition rather than a second copy: hgGoldTwinConflictNote takes what the caller withholds, so this desk says THE CROWN where the others say a rank boost, and the two existing callers get byte-identical text. No gate, threshold or verdict moves on any desk, and no setup leaves any board. Tests: tests/test-gold-direction-twin-crown.mjs, 51 assertions, 12 of 12 behavioural mutations caught across two files with the baseline verified green first',
