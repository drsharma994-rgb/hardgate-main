...
var HG_BUILD = {
  version: 'hg-v951',
  pack: 'THE DESK CALLED PLAIN GOLD IS INLINE IN THE SHELL, AND THAT IS WHY NOTHING FOUND IT. Every other gold desk is a module file, so every file-level survey this session ran could enumerate them - and so could hg-v949 coverage reporter, which finds a desk by probing a module global. runGold is about 200 lines inside index.html, registered as id gold in HG_NAV_GROUPS beside the other sixteen. It had NO probe and therefore NO BUCKET in that reporter: not covered, not uncovered, ABSENT. A reporter built so that gaps name themselves had a blind spot, and the tab hiding in it was the one being asked about by name. WHAT IT DOES: reads getXAUCandles, whose chain falls back to Delta XAUTUSD and Binance PAXG, both 24/7. Runs TWO gated lanes - swing on 4h, scalp on 15m - and EACH calls logSetup and renders SEND TO TRADE PLAN and ADD TO BOOK. So a setup formed on a Saturday bar was directly bookable from this tab, with none of the gold calendar the module desks now carry. PER LANE, because the lanes read different series: judging the 15m scalp by the 4h bar clock would mislabel it, and a mutation that makes the scalp lane read h4 is caught. BOTH HANDOFFS GO, TOGETHER, AND NOTHING ELSE DOES. hg-v930 is explicit that a demote alone does NOT remove ADD TO BOOK or SEND TO TRADE PLAN - hg-v611 needed a suppress verdict for exactly that, after a demote stamp left four stop-outs bookable. So when gold was shut the two CTAs are REPLACED by the reason, while the ledger, the evidence and the levels stay exactly where they were. Fails OPEN at every seam: no calendar, an unreadable instant or an empty series and nothing is withheld. THE REPORTER CAN NO LONGER BE BLIND TO AN INLINE DESK: the inline tab is listed and probes the function the shell defines, the buckets still partition the list with it included, and with the shell absent it reports NOT LOADED rather than silently covered. THE GUARD EXECUTES THE SHIPPED CODE rather than grepping for it. The first cut of three sections grepped index.html for the branch - the brittle shape this session has corrected four times, and one that cannot see whether a branch is reachable at all. Each lane real assignment statement is now LIFTED OUT OF THE SHELL and RUN under stubs, and the HTML it produces is read. Two fixture defects fell out of doing that honestly: the lift began below the verdict declaration the statement closes over, so it could not run; and an assertion searched the output for the words SEND TO TRADE PLAN, which the withholding notice itself contains because it NAMES what it removes - it failed against correct code until it asserted on the button markup instead. No gate, threshold or verdict moves on any other desk, and no setup leaves any board. Tests: tests/test-gold-inline-weekend.mjs, 29 assertions, 8 of 8 behavioural mutations caught across index.html and gold-formation.js with the baseline verified green first',
