...
var HG_BUILD = {
  version: 'hg-v993',
  pack: 'THE REGIME LAYER RUNS TWO HARD RULES AND NEITHER WAS RECORDED BESIDE AN OUTCOME ANYWHERE. The playbook bias (LONG-ONLY / SHORT-ONLY / BOTH / STAND-ASIDE) is a hard block on PINE, an against on the FTS setup stack, a +2 vote on STAR TRADER and a BRAIN layer; the per-symbol tape regime (detectRegime: VOLATILE kills trend continuation, COMPRESSION kills swing) is a hard veto through hgRegimeAllowsSetup on SWING, SCALP, BEST, SUPER SETUP, CHART VISION, CONTRACT REPORT and the FTS stack, and asked by none of the crypto desks that record most. No forward record carried the bias, the verdict, the snapshot age (regimeState().at was read by nothing, so a snapshot hours old is applied as current) or the tape key. The bias rule lives once now, hgRegimeBiasBlocks in regime.js, and pineRegimeAllows delegates to it (fails open and says so with the home absent) instead of carrying its own copy of the table; hgRegimeBiasMark reads the live snapshot for a record at fire time (bias, verdict, age in minutes; a gold-lane symbol gets no verdict). hgTapeRegimeMark in plans.js reads the tape key and the veto the gate in force would have applied, and equals hgRegimeAllowsSetup on every readable series (25 of 25 tape x style pairs); a thin series reads NOT RECORDED where the gate fails open. The forward ledger records regimeBias, regimeAgainst, regimeAgeMin, tapeRegime and tapeVeto on every record at hgFwdRecordScan, the tape from the first candle series the candidate carries under the writers own names (rows, rows4h, rows1h) and judged under the style the record timeframe implies; a valid desk hand-in wins, junk is never coerced, the direct door refuses the same junk; nw-style folds (ra / ro / tp / tv / tw) outlive the live cap; REGIME PLAYBOOK SPLIT and TAPE REGIME SPLIT print on the shared forward panel, nothing where no record carries a mark. Nothing is gated on the marks; the PINE block produces exactly the counts it always did; no threshold moves; no setup leaves any board. Tests: tests/test-regime-tape-marks.mjs (new); tests/test-pinegate.mjs boots regime.js as index.html does',
