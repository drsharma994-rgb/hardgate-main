...
var HG_BUILD = {
  version: 'hg-v968',
  pack: 'THE 3-MINUTE GOLD SCALP REFRESH WAS INVISIBLE, AND THE GOLD SPREAD LOCK HAD NEVER FIRED. ASKED THREE TIMES FOR AN AUTO-UPDATING GOLD SCALP TAB. hg-v965 put the desk on a 3-minute clock; hg-v967 fixed the resolution that stopped it firing; and this tab showed NO LAST-SCAN TIME ANYWHERE, so a working refresh and a dead one looked identical. Two answers changed a timer the desk owner had no way to observe - the mechanism was delivered and the observable outcome was not, which is the actual reason it kept coming back. The tab now carries a live line: AUTO 3m, last scan HH:MM:SS, next in M:SS. It is stamped in the runScan FINALLY, so a FAILED scan stamps too - a frozen clock is not information - and the countdown is read from the shell timer rather than recomputed, so it cannot drift from the clock it describes; with the shell absent it omits the countdown instead of inventing one. One page, one ticker. SEPARATELY, THE GOLD SPREAD LOCK HAD NEVER FIRED EITHER. AGENTS.md documents it as a live hard gate - live bid/ask wider than 250 points / 2.5 pips kills the entry - and the globals it reads (__hgGoldQuote, __hgGoldSpreadUsd, __hgGoldL2Book) have 37 read sites across four desks and ZERO WRITERS anywhere in the repo, so it returned unchecked on every scan since it shipped. THE QUOTE WAS ALREADY ON THE WIRE: Delta /v2/tickers returns a quotes object and parseDeltaTicker reached into it for mark_price while dropping best_bid and best_ask - the exact two fields the lock reads. That is the hg-v932 work-done-and-dropped-on-the-way-out finding, in the gold quote path. Both parser copies now keep them, and the guard drives BOTH because the server uses the .cjs while the test imports the .mjs - patching only the tested copy would have left production still dropping the quote, which is the hg-v967 trap one file along. AND THE BAR IS VENUE-SPECIFIC: 0.25 dollars is a broker figure, while the feed chain on these desks ends in Delta XAUTUSD and Binance PAXG, crypto-settled gold PROXIES. hg-v919 measured that exact asymmetry on the cost ceiling - 96.1 percent of the scalp book vetoed at PAXG against 16.4 percent at XM - so a quote naming a non-broker venue is REPORTED and never dropped. NOTHING LEAVES ANY BOARD THAT WAS NOT ALREADY LEAVING IT, because today the lock drops nothing at all; a quote naming no venue behaves exactly as before, which is what every existing caller and guard sees. TWO DEFECTS IN THE GUARD THIS PACK SHIPS: the repo sweep regex over-escaped and matched nothing, so the zero-writers assertion passed VACUOUSLY on an empty sweep - caught only because the reads-first assertion demanded a non-zero count; and the half-quote case was driven on one parser copy while the two were compared only on a FULL quote, leaving a mjs-only fabricated-spread mutation alive. AND A THIRD, WHICH HUNG THE WHOLE SUITE: the countdown ticker was armed at MODULE LOAD. Harmless in a browser; in Node it installs a real 1-second setInterval that is never cleared, so the event loop never drains and the process never exits - the full run stalled on test-conviction-orphan-expiry for over three minutes with no output. It is armed from the MOUNT now, which is also the correct lifecycle since a countdown for a tab nobody has opened has nothing to paint, and the guard asserts that LOADING the file installs no timer at all. A fourth, in the assertion written for that: it read a fixed 4,000 characters after the function name to find the arm site and missed it - the fixed-width-slice brittleness this repo has corrected five times - so it mounts the real tab and watches the timer appear instead. AND A FIFTH, caught by the full suite after the hang fix - the hg-v967 trap two lines below the comment warning against it: exporting the two stamp helpers took goldscalp.js from 19 to 21 module-scope functions and turned test-gold-render-integrity red; the guard is not relaxed, both helpers ride the HG_tabs registration and the globals are asserted absent. NO GATE IS LOOSENED and no threshold moves - the 0.25 bar is unchanged and asserted unchanged. Tests: tests/test-gold-spread-lock-fed.mjs, 76 assertions, with 30 of 30 behavioural mutations caught across goldscalp.js, goldswing.js, goldind.js and both delta-perp-history copies, baseline verified green before every pass',
