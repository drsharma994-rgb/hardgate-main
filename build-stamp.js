...
var HG_BUILD = {
  version: 'hg-v979',
  pack: 'THE FORWARD LEDGER SETTLED A GOLD RECORD ON WHICHEVER DESK NEXT HANDED IT BARS, AND THE GOLD DESKS DO NOT READ ONE FEED. Every gold desk records sym XAUUSD and hgFwdSettle matched on symbol and timeframe only. GOLD SCALP and GOLD SWING price their levels on the XM bridge first; GOLD ULTRA, GOLD DIRECTION, GOLD PRO and OPTI GOLD have no XM leg and read Binance XAUUSDT, then PAXG; OMNIGOLD settles every open XAUUSD record with whatever bars it fetched, by design. macro.js puts PAXG within ~0.5% of spot and goldscalp.js records XAUT ~1.5% under the XM print; in GOLD SCALP committed walk (2,605 trades) the median stop is 0.219% of entry and 91.4% of stops are narrower than 0.5%, so a record priced on one feed and settled on another was decided on its first bar by the basis and the direction of the trade, not by the tape. The rule lives once: a record carries the feed its levels were priced on, the resolver names the feed of the bars it hands over, and a KNOWN mismatch is held for its own feed; unknown on either side fails open, exactly as the timeframe rule beside it. Thirteen gold files record and nine resolve, each named as its own feed chain names it (per timeframe where the legs can differ; GOLD DIRECTION names one feed only when every leg agrees). The desk note says what is waiting and for which feed. Driven end to end: seven XM-priced GOLD SCALP records under PAXG bars 1.5% under the print settle NOTHING and are counted held, settle on the tape under XM bars, and stripped of their feed (the pre-pack record) 4 of 7 resolve on the FIRST bar, every short a win. Found on the way: TAURIC read rows.length on the PACK hgOgFetchRows returns and so never priced a plan in production (its guard stubbed a bare array); and hg-v978 census read the entry point by name and could not see GOLD PINE, which records through a local alias, so that desk carried neither bar nor feed - both fixed; the census here is derived by call shape, aliases included. Five textual guards pinned the old call text and match the rule now. No gate is loosened, no threshold moves. The crypto resolvers name no feed and settle as before. Tests: tests/test-gold-ledger-same-feed.mjs, 97 assertions, with 44/44 behavioural mutations caught across hg-forward.js, gold-forward-read.js, goldscalp.js, goldswing.js, goldultra.js, golddirection.js, tauric.js, optigold.js, eightypercent.js, omnigold.js, omnigold1.js, newgold.js and goldpine.js, baseline verified green before every pass',
