...
var HG_BUILD = {
  version: 'hg-v982',
  pack: 'OMNIROUTE OUT-OF-SAMPLE JUDGE COUNTED EVERY RESTING ORDER AS IF IT HAD OPENED; THE FILL-AWARE RULE OMNIGOLD CARRIED INLINE IS NOW ONE RULE IN HG-FORWARD.JS AND BOTH DESKS READ IT. OMNIROUTE measured-edge gate vetoes a mechanic when the tickets this ledger cleared read at or under -2 sigma against breakeven out of sample. Every order this desk records rests at the setup level (hg-v424); its committed replay never fills on the signal bar and leaves 34.3% of what it opens unfilled, and the judge read the actual tally, which settles a resting order the tape never reached as a target hit or a stop. Since hg-v981 the records carry a mark so the fill-aware tally exists; nothing on OMNIROUTE read it. OMNIGOLD judge preferred it in an inline block. hgFwdJudgeSample(stats, minN) in hg-forward.js: reads the chosen population actual tally and prefers its fill-aware tally only when that clears the same floor on its own, so a legacy log decides exactly as before; OMNIGOLD inline block replaced by the call (its labels unchanged), OMNIROUTE ticket-only judge reads it and labels FILLED TICKETS with the never-filled count excluded. Neither desk reads fillSamples itself any more. Driven on both real gates: 30 cleared tickets at 50% pass; the same 30 of which 22 opened and lost VETO, naming 22 FILLED TICKETS at 9% and 8 never filled excluded; 19 fill-aware records under the floor leave the actual tally deciding; a fill-aware tally that pays passes labelled FILLED. The solidity grader G6/G7 judges expectancy, for which no fill-aware figure exists in hgFwdStats; reported, not wired. No threshold moves; the only verdict that can change is one a fill-aware sample at or over FWD_MIN_JUDGE overturns, and no crypto record written before hg-v981 carries one. Tests: tests/test-omniroute-fill-aware-judge.mjs, 30 assertions, with 13/13 behavioural mutations caught across hg-forward.js, omniroute.js and omnigold.js, baseline verified green before every pass',
