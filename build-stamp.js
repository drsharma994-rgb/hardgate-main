...
var HG_BUILD = {
  version: 'hg-v885',
  pack: 'CRYPTO SCAN\'s empty state said “The other 0 were never read”. Pack 869 built that state so a broken scan could not be reported as a quiet market, and its rule is: nothing is claimed about a contract that was never read. Pack 882 then added a counter for contracts that WERE read and voted on and whose scoring - this app\'s own code, not the feed - then threw, and folded it into `partial` without reaching this sentence, while `partial` was what chose the wording. So a run where every contract was read and two crashed in scoring rendered “300 of 300 contracts reached the engine and none produced a directional signal. The other 0 were never read, so nothing is claimed about them” - directly above a COVERAGE line naming the two crashes. Not a sentence, and the two lines contradicted each other on one card. The deeper error is the claim itself: a contract whose scoring threw might well have produced a signal, so “none produced a directional signal” can no more be said of it than of one that was never fetched. The wording is now driven by the counts rather than the flag - the claim covers the contracts actually JUDGED (read minus crashed, floored at zero), each silence is named only when its own count is non-zero, singular and plural follow the count, “either” appears only when something came before it, and a run where everything read then crashed gets its own branch saying the engine ran and nothing was judged, which the old code had no wording for at all. csCoverage and `partial` are unchanged; only the sentence moved',
