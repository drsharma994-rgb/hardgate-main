...
var HG_BUILD = {
  version: 'hg-v903',
  pack: 'GOLD SCALP COMPLETES ITS OWN ACCOUNTING: what never reached a gate is now counted, not dropped in silence. hg-v900 gave the tab a ranked funnel - \'N setups held back across M gates\' - which ranks every NAMED rejection, meaning every setup that formed and was then stopped. But a strategy attempt can end BEFORE any gate runs, and __gsCand did that with a bare return null. MEASURED over 600 synthetic scans: 2,667 attempts into __gsCand, of which 937 (35.1%) ended there - 378 with fewer than two agreeing reads, and 559 OUTVOTED, meaning the reads existed and the desk\'s own evidence book went against them. Those are not the same thing. The thin ones are honestly silent: no setup formed, and naming every strategy that did not trigger would bury the ones that did. The outvoted ones are a desk that HAD a directional read and whose own ledger voted it down - the single commonest way a strategy produces no card - and the funnel\'s headline gave no hint that a set larger than the one it ranked never got that far. __gsCand now tallies both halves on a per-scan bundle (reset every scan, so it cannot accumulate), the tally rides out beside .rejected on the same side-channel, goldscalp sums it across venue legs, and the WHY NOTHING LED panel prints the split under the ranked gates. A tally of zeroes is not a finding and renders nothing; outvoted attempts alone still open the panel, because that IS the answer to why the board is empty; and the two halves are never merged into one number. THIS COUNTS; IT DOES NOT GATE. The condition in __gsCand is unchanged, it still returns null, and it does not fabricate a named rejection for a setup that never formed. Nothing is held back or released by any of it. ONE HYPOTHESIS CHECKED AND REJECTED: the tab promises \'every reason named (never silently dropped)\', so these bare returns looked like a contract violation. They are not - that promise covers setups that FORMED and reached the rejection channel, and an attempt whose evidence never met the bar has no levels and no setup to show. The fix is therefore to complete the accounting, not to manufacture rejections. Two assertions in the hg-v900 funnel test were updated for the new two-argument call, and its harness now lifts gsPreGateLine alongside the renderer that calls it. G1-G7, the 1.5xATR scalp stop floor, the 3.5x ceiling, the baked edge table, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent ALSO FIXED, CAUGHT BY THIS PACK\'S OWN SUITE RUN: test-cryptoscan-rule-applied.mjs is wall-clock dependent. Its tie sweep anchors 400 tapes to Date.now() and passes now: Date.now() into an engine whose session gates are time-of-day dependent, so whether ANY tape lands on an exact L == S tie moved with the hour - the file passed in isolation and failed inside a suite run fourteen minutes earlier, reporting \'0 of them were exact ties\'. Same defect and same fix as test-cryptoscan-rank-stamp.mjs in hg-v894: the clock is pinned to 12:00 UTC and the tapes anchored to it, so the sweep asks the same question every run. Nothing it asserts changed. Unrelated to this pack\'s gold changes - reported rather than waved through as a flake',
