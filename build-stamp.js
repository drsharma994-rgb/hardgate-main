...
var HG_BUILD = {
  version: 'hg-v990',
  pack: 'CRYPTO ULTRA GATED ITSELF MEASURED NOT TRADABLE ON A WALK THAT MEASURED NOTHING: EVERY REPLAY HARNESS HANDED THE ENGINE A RULE WITHOUT GEOMETRY, EVERY PLAN WAS PRICED AT STOP NULL AND TARGET NULL, AND ZERO SETTLED TRADES IN EVERY CELL WAS TYPED UP AS AN ARCHITECTURAL VERDICT. The committed artifact (scripts/backtest-cryptoultra-results.json, 2026-08-22 to 2026-09-12, 1,999 x 15m BTCUSDT bars) has 14 grid cells, fires 1,628 times at its loosest cell, settles ZERO trades in every cell, and carries chosen null and evidence null. The hand-typed HG_CRYPTO_ULTRA_EVIDENCE literal said measured true and diagnosed architectural failure, indicator votes too correlated, long and short canceling through merge logic. The cause is the harness seam, reproduced on the real engine: every replay (backtest-cryptoultra, backtest-crypto-multisymbol, backtest-cryptoscan-improved) passes rule minPct 0, minAvail 0, regimeGate false, and the engine read inp.rule OR RULE, so that object REPLACED the rule: stopAtr, costFloorMult, t1R, t2R and timeoutBars were undefined, rawStop and the cost floor were NaN, and every plan carried stop null and target null. A trade with no stop and no target never exits on price, times out with NaN R, and every aggregate silently dropped it; the 1,512 merged of 1,628 fired is exactly what one un-exitable trade per side produces. 40 of 40 synthetic tapes priced a NaN stop under the override; 0 of 40 with the rule merged. The engine merges an override over RULE (an override tunes the fields it names and inherits the rest; minPct 0.99 still gates, stopAtr 3.0 still moves the stop) and REFUSES a plan whose stop distance or target ladder is not a positive finite number as a named gate with plan null (stopAtr null reads a zero stop distance, +null is 0; t1R null is not a ladder) rather than handing back a plan whose stop is null. The walk exports its functions, counts unpriced plans (READS.unpriced, recorded in meta), keeps them out of the trade walk and REFUSES to write an artifact while any exist (assertPriced, fatal and named); its per-bar counter reassignment used to wipe the count every bar and now mutates. Driven on synthetic bars through the real engine: every kept plan has finite geometry, trades settle with finite net R and every settled trade counts; a stub engine pricing NaN stops is counted 30 of 30 unpriced and the write is refused. HG_CRYPTO_ULTRA_EVIDENCE is GENERATED (scripts/cryptoultra-evidence-literal.mjs, markers, --write, missing marker fatal, npm run cu:evidence the drift check): with no rule chosen it writes measured false and a note derived from the artifact (fired, cells, settled 0, a defect of the walk not a verdict on the strategy); with a chosen rule it writes measured true from the artifact figures and verdict; tradable is the boolean or false. The desk now reads NOT YET MEASURED, which is record-only exactly as MEASURED NOT TRADABLE was: no fire, no ticket, nothing loosens; no hand-typed diagnosis survives anywhere. Nothing is re-baked here (403 CONNECT on every route; no bar cache), so what ships is the seam, the refusal and the honest literal. Tests: tests/test-cryptoultra-evidence.mjs (new); tests/test-cryptoultra.mjs follows NOT YET MEASURED',
