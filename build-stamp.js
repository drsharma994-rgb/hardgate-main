...
var HG_BUILD = {
  version: 'hg-v985',
  pack: 'THE DIRECTIONAL FUNDING RULE IS APPLIED BY THE SWING AND SCALP MATRICES, ASKED BY NONE OF THE CRYPTO DESKS THAT RECORD MOST, AND MEASURED NOWHERE; EVERY CRYPTO RECORD THAT HAD FUNDING IN HAND NOW CARRIES THE RATE AND THE RULE VERDICT, THE TWO PURE-PRICE DESKS SAY SO ON THE CARD, AND TWO DESKS WHOSE FORWARD LOG NEVER WORKED ARE REPAIRED. fundingGateDirectional (hg-setup-core.js) vetoes a plan whose funding runs against the trade at or beyond 0.04% per interval; OMNIROUTE own soft gate reads the same figure at 0.05 (crowded). CRYPTOVERSE and 90PERCENT had the venue rate on every universe item and read price alone; OMNIROUTE, SQUEEZE, OI FLOW, EDGE, CRYPTO SCAN and the GATES tab had it in hand at the record site and recorded none of it; no artifact records funding beside an outcome. One rule, three states: hgFundingAgainstMark says whether the rule in force would have VETOED (true), let through (false) or could not say (undefined: no finite rate, no direction, or a feed out of range, which gateResult would otherwise degrade to a veto). The ledger carries fundingPct (a number, never coerced) and fundAgainst (the two booleans), stamps the verdict from the one rule when a desk hands in a rate, folds fa and fw buckets, and the shared forward panel prints FUNDING RULE SPLIT at the rule bar and, from the raw rate on live records, at OMNIROUTE crowded bar too. Nine record sites hand the rate in (CRYPTOVERSE, 90PERCENT, SQUEEZE, OI FLOW, EDGE, OMNIROUTE, CRYPTO SCAN, GATES, and the SWING/SCALP candidate row the BEST publish spreads); the census is derived by call shape and names the ten writers with no figure in reach. CRYPTOVERSE and 90PERCENT print FUNDING AGAINST with the rate and gate on nothing: identical boards under 0.07%, 0.01% and 0.00%, through both real tabs. Found on the way, three defects: SQUEEZE published row carries no candles, so the hg-v981 mark and bar on that desk never landed (a parallel source array now rides beside the published rows; proved through a real fired scan); OI FLOW record block and its FORWARD panel referenced W in a file whose alias is G, so the try threw on its first line from the day it was written (686a2cc) and OI FLOW never wrote one forward record nor painted the panel; and its classifier emits LONG/SHORT where the normaliser accepts only lower case. All three repaired and proved through a real OI FLOW scan, and the panel repaints after every publish instead of at mount only. No gate is loosened, no threshold moves, no setup leaves any board. Tests: tests/test-crypto-funding-mark.mjs, 102 assertions, with 44/44 behavioural mutations caught across hg-setup-core.js, hg-forward.js, cryptoverse.js, ninetypercent.js, squeeze.js, oiflow.js, edge.js, omniroute.js, cryptoscan.js, engine.js and index.html, baseline verified green before every pass',
  built: '2026-09-26T18:00:00Z'
};
