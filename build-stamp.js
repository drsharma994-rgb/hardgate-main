...
var HG_BUILD = {
  version: 'hg-v882',
  pack: 'CRYPTO SCAN counted a contract twice when its scoring crashed, and reported the crash as a fetch error. THE DOUBLE COUNT: the scan loop incremented `scanned` right after the engine call, then ran ~165 lines of scoring - order flow, sentiment, external risk, the three-layer blend, the pro-grade stamp, SMC - and the single catch around the whole body incremented it AGAIN. Driven through the real runScan with layer 2 rigged to throw on one of three contracts: scanned 4 of a 3-contract universe, progress bar 133.3%, status line “scanned 4/3”. None of those scoring calls is individually guarded, so any of them throwing on a malformed row reaches that catch. THE MISREPORT: pack 870 drew a line between `unread` (a fact about the fetch) and `skipped` (a fact about the contract) because a Binance 451 had been reading as “fewer than 230 closed 15m bars”; the same line was never drawn on the other half of the loop, so a crash in this app\'s own scoring landed in `errors` beside a network failure and the reader was told a contract errored when its bars had arrived and its engine had voted. scanned now ticks exactly once per contract - the catch only counts one the body never reached - the scoring block has its own boundary, and COVERAGE reports “read and voted, then scoring threw” separately from “threw before their bars could be read”, with a bounded per-cause label (error name plus 60 characters of collapsed message) so a hundred contracts failing the same way tally as one line rather than a hundred, and a long or hostile message cannot run away with the panel',
