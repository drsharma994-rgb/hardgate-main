...
var HG_BUILD = {
  version: 'hg-v867',
  pack: 'CRYPTO SCAN’s “order flow” layer is candle arithmetic. order-flow.js declared itself “Independent of price technicals — detects whale accumulation, capitulation”, and that claim is the stated reason the tab gives layer 2 35% of its confidence plus a +15% bonus whenever it agrees with layer 1. There is no order book, trade tape, liquidation feed or on-chain data anywhere in the file: hgBidAskImbalance counts a green candle’s whole volume as buying and a red candle’s as selling, hgVWAPDivergence is (close − vwap) / vwap, hgSweepPattern reads bodies, wicks and a volume ratio — all price technicals over the same OHLCV bars layer 1 votes on. Measured against the real engines over 60 synthetic tapes swept across drift and volatility, 56 of which produce a layer-1 direction: correlation(layer-1 direction, layer-2 score) 0.957, and layer 2 agreed with layer 1 on 53 of 56 (95%). The decorrelating layer says what layer 1 already said 95% of the time, and every one of those agreements pays the bonus for one price read twice. The weights and the bonus are NOT changed — that is calibration and the desk’s call — but the reads are named for what they are, so the vote table the tab invites you to audit by eye stops printing “Bid-Ask Imbalance” over a number that has never seen a bid or an ask, and every why line names the feed it lacks. One arithmetic bug ships with it: hgSweepPattern’s volume baseline summed 19 bars and divided by 20, so avgVol was 5% low on every call, volRatio 5.3% high, and the 1.5× gate written two lines below behaved as 1.425×. Over 18,000 rolling windows the gate passed 11.9% of the time as coded against 7.4% with the divisor matched to the count — 38% of the sweeps it reported were under its own threshold. A zero-volume feed also divided by zero and reported Infinity at full confidence; guarded. Recorded and deliberately not changed: the aggregate is the mean of only the reads that clear their own gate, so a second read that AGREES but sits just over its threshold nearly halves the score — vwap15 at 0.099 is excluded and the aggregate is 0.900, at 0.101 it is admitted and the aggregate is 0.501. Real, but every fix re-scales the layer against the 0.2 direction band and the 0.35 weight above it',
