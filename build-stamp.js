...
var HG_BUILD = {
  version: 'hg-v895',
  pack: 'NEW TAB: 90PERCENT, in the CRYPTO group - the four-step method exactly as specified, on Delta India + CoinDCX futures. STEP 1 trend from the MA stack (20/50/200), MACD and Ichimoku, needing a majority of at least two, with SIDEWAYS a real verdict and the commonest one. STEP 2 support and resistance from CONFIRMED pivots clustered into levels (2+ touches within half an ATR - one pivot is not a level) plus least-squares sloping lines through 3+ pivots. STEP 3 momentum through the three specified pairings, all of which must agree: A RSI + Stochastic, B Bollinger + MA, C MACD + Ichimoku. STEP 4 entry at the close, stop beyond the level behind the trade, target a level in front. TWO INDICATORS HAD TO BE BUILT RATHER THAN REUSED. indicators.js ships stochRsi, a stochastic OF RSI; the brief asks for the classic oscillator - close against the high-low RANGE - which is a different instrument on a different input. And indicators2.js ships an ichimoku whose senkou spans are UNSHIFTED (its own comment says so); the standard indicator plots the cloud 26 bars FORWARD, so the cloud under today\'s price was computed 26 bars ago - taking the shift reads OLDER data, never newer. THE TARGET IS THE FIRST LEVEL THAT PAYS, NOT THE NEAREST. Measured over 300 tapes: 43 confirmed pivots and 11 clustered levels apiece, 94 trending tapes with a level on both sides - and a median structural R:R of 0.42, p90 1.43, only 5 of 94 reaching 2R. Adjacent levels sit roughly equidistant, so nearest-to-nearest is a coin flip paying less than evens. The exit walks the levels outward and takes the first that clears the 2R floor; every candidate is a real measured level with its own touch count, and if none pays the card says what the best available was. A BUG FOUND BY THE FUNNEL: the rolling-sum moving average could not cross a NaN. The Stochastic\'s %K is NaN for its first 13 bars by construction, so one `sum += NaN` made %D NaN for every bar, PAIR A reported UNCHECKED on every contract, and the tab ran clean while producing nothing at all - 0 setups in 600 tapes. Summing each window yields NaN only where the window really holds one; the rule now fires on 13 of 600 (2.2%) at 2.02-5.19R. THE TAB AUDITS ITS OWN NAME: it claims no 90%, writes every setup to the shared forward log under NP90-<side>@v1, and prints the MEASURED hit rate beside the 90% the name implies, saying so plainly while nothing has settled. The card states that RSI, the Stochastic, MACD and Bollinger %B are all functions of the same closes, so their agreement is one price series agreeing with itself - what three pairings buy is selectivity, not six independent confirmations. Closed bars only, record only, nothing sent anywhere',
