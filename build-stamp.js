...
var HG_BUILD = {
  version: 'hg-v902',
  pack: 'GOLD SCALP: WHEN THE R:R ON THE CARD IS ARITHMETIC RATHER THAN A READ, THE CARD NOW SAYS SO. __gsLevels builds the scalp ladder, and its own note explains why the old 2xATR stop cap was a defect - truncating a stop does not reduce risk, it relocates it, and the ladder is then measured against a risk that never reached invalidation - then justifies the 3.5xATR ceiling that replaced it by saying that beyond it \'the geometry is not a stop but a different trade, and the R:R gate declines it on its own\'. THAT LAST CLAUSE IS ONLY HALF TRUE. When TP1 SNAPS to an opposing level, rr is a real ratio against a real level - measured 0.629 across the sweep, well under the 1.2R floor, so the gate does decline it, 138 of 138. When nothing snaps, t1 is entry + 1.5*risk BY CONSTRUCTION, so rr is 1.500000 IDENTICALLY whatever the risk, and the floor declines 0 of 138. On the real mint path over 800 tapes it is starker: all 88 ceiling cases were unsnapped, every one carried rr exactly 1.500, and the gate declined none. The stop was never the hidden part - stopNote already said it sits at the ceiling and that structure may be broken. What was hidden is that the number beside it measures nothing. So __gsLevels now returns ceilingBound and t1Snapped, and a ladder that is both bound and unmeasured DEMOTES with an R:R UNMEASURED stamp and a gate note saying the figure is 1.5x the risk by construction, not a reward the market offered. The card still paints with its levels; it simply cannot lead. Both flags are read together: ceilingBound alone would demote a snapped case whose R:R is real, and t1Snapped alone would demote an unsnapped ladder whose stop is genuinely at structure. WHAT THIS PACK DELIBERATELY DID NOT DO. An earlier cut DROPPED these instead, which removed 14% of the board (640 minted to 551) and broke a fixture that documents rsidiv painting. The 3.5x ceiling is a deliberate, already-tested design - test-gold-stop-model.mjs documents 13% of cases reaching it, test-gold-inst-gates pins the ceiling itself - and cutting it back needed evidence this pack does not have. Demote is what the measurement supports; the drop was an argument. The ceiling, the 1.5xATR floor (still a floor, never a cap) and the 1.2R gate are all unchanged, and nothing is dropped that was not dropped before. ONE MEASUREMENT ERROR CAUGHT IN PASSING: a first pass computed ATR as a simple mean of the last 14 true ranges and made 20.8% of stops look like they broke the 1.5x floor. The desk uses Wilder smoothing; with the desk\'s own atr() the floor holds on 640 of 640. G1-G7, the baked edge table, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
