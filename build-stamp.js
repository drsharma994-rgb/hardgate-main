...
var HG_BUILD = {
  version: 'hg-v879',
  pack: 'CRYPTO SCAN\'s closing note named one blocker in five, and asserted an accuracy it had never measured. Pack 868 built csBlockerTally and the ONLY-blocker column to answer “which gate actually closed the block”, and its own comment says “defer to the tally” - the footer deferred for exactly one of five gates. Handing it 25 setups blocked by a single gate, once per gate: confidence blames confluence, regime blames confluence, session NAMED, voting blames confluence, sentiment blames confluence. “Confluence” names nothing in this pipeline (the gates are confidence, regime, session, voting, sentiment) and it is worse than one in five in practice, because the session gate passes 07:00-17:00 UTC so during those ten hours session can never be a blocker, clockSole is 0 by construction, and the note reached the confluence line whatever had happened. csTopBlocker now returns the largest ONLY-blocker, falling back to the pro-grade stamp and then to the commonest gate among setups with several open, each worded differently because they are different claims - a sole blocker is what alone stopped an otherwise pro-ready setup, a multi count is only the commonest gate and not the reason any one of them failed. All five gates are now named; the session gate keeps the one extra sentence that is a fact about the gate rather than a guess. Separately the same paragraph asserted the engine “produces high volume but low accuracy” two sentences above “no win rate is claimed from the scan window” - an accuracy claim beside a promise not to make one, flagged in pack 876 and left standing because nothing could answer it. Pack 878\'s INDEP correction can: csFwdVerdict pools this tab\'s own settled records and corrects the sample for overlap with the same hgFwdOverlap, the same hgOmniPoolRead, the same 1.5R breakeven and the same family bar the panel uses, and the sentence reports what came back - 3,000 settled at 55% T1-first against a 40% breakeven is 5.96 independent observations and “too few to judge”, where the uncorrected count would have said “has paid”. With nothing settled it says that instead of guessing. The verdict is passed into csFooterNote as an argument, so the note itself stays pure and testable',
