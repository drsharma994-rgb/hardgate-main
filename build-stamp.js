...
var HG_BUILD = {
  version: 'hg-v864',
  pack: 'CRYPTO SCAN was most confident when order flow argued against the trade. hgComputeThreeLayerConfidence took layer 2 as Math.abs(l2.score) * 0.35, with direction supposedly carried by the ±15/20% agreement multiplier below. Composed, it is not: the multiplier is far too small to undo a 0.35-weighted magnitude term, so the confidence curve is V-SHAPED in flow agreement with its MINIMUM at flow = 0. One long, pct 0.90, sentiment +0.60, sweeping the flow read: −0.9 hard against → 0.660, −0.5 → 0.548, 0.0 NO OPINION → 0.408 (the lowest point on the curve), +0.5 → 0.788, +0.9 → 0.949. The harder order flow argued AGAINST the trade, the more confident the desk became; over 200 swept configurations that held in every one — 100% non-monotone, and in 100% a neutral flow scored below an opposing one. Two causes, both here. Math.abs() credits conviction-against straight to the base. And ‘neutral’ is a truthy string, so the l2.dir compare read no-opinion as disagreement, charging it the 0.80 penalty and printing “L1/L2 divergence” on the card while denying it the magnitude credit an opposing read collected — a flow reader that RAN and said neutral therefore scored BELOW one never loaded at all, 0.408 against 0.510. Layer 2 is now read relative to the trade, exactly as layer 3 is, and only a real long/short call takes the multiplier. Measured over 6,840 configurations: 2,520 have flow agreeing with the trade and the confidence changed in ZERO of them, so the pro-grade cohort is 694 cells before and 694 after — the HIGH-QUALITY block and the forward log’s ticket split do not move, because that stamp requires layerAgreement === 2, which IS the agreeing case. 166 cells change tier and shouldTrade, all where flow was being paid to disagree, and 1,800 lose a divergence line that was never true. The ±15/20% multiplier is left alone: it now double-counts direction mildly, but removing it would re-tune every number on the tab. The unreferenced hgVotingSummary carried the same neutral-is-dissent line and was corrected with it',
