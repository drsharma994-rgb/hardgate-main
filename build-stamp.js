...
var HG_BUILD = {
  version: 'hg-v871',
  pack: '“SCAN ALL FUTURES” was quoting a universe two filters had already shrunk. Two findings in the layer above the coverage work of packs 869 and 870. ONE: hgDeskLoadUniverse honours opts.includeUnknown on its xuUniverse branch and hardcodes false on its Binance fallback — same call, same options, two different universes. CRYPTO SCAN asks for { minTurnover: 0, includeUnknown: true } under that button, and on the fallback every perp whose ticker carried no turnover was dropped regardless; passing includeUnknown:false gave the identical answer, which is how you know the option was doing nothing. TWO: the scan records universe = items.length, the list it was HANDED after the turnover floor and the venue filter have run, so COVERAGE could honestly say “300 of 300 contracts read (100%)” while 180 were removed before the scan began — measured on a ten-row fixture as rawLen 10, items 4, six gone before coverage could count them. Both close by naming the funnel, not by loosening a filter: the floors are the desk’s and are not moved. The loader reports rawLen → turnoverLen → filteredLen with droppedTurnover, droppedVenue and droppedNoTicker, plus the floor and the includeUnknown it judged by; hgDeskLoadDeltaCoinDCX records the venue drop instead of overwriting filteredLen with it; and the coverage line quotes both universes: “230 of 300 contracts read (77%) · 60 never fetched · 10 skipped · 180 of 480 never offered to the scan: 120 under the $5M turnover floor, 60 on other venues — 48% of the source universe”. filteredLen and rawLen keep their old meanings, and a run recorded before this pack renders without the funnel rather than with zeros',
