...
var HG_BUILD = {
  version: 'hg-v894',
  pack: 'NEW TAB: CRYPTOVERSE, in the CRYPTO group - a three-factor rule on Delta India + CoinDCX futures, built to the brief that a 99% win rate is an artefact of look-ahead, curve-fitting or repainting and that the target is positive expectancy, not accuracy. Three non-correlated gates in the order price meets them: WHERE, price must have swept a CONFIRMED prior swing high or low (wick through, close back inside - a close beyond is a break, not a sweep) with the higher-timeframe untested-FVG check recorded beside it; WHEN, a Lorentzian k-NN fires in the same direction, AFTER the sweep bar and within an 8-bar window, so a signal mid-range cannot fire because without gate 1 there is nothing to trigger on; CONFIRM, VuManChu Cipher B and the Range Filter vote as binary permission and an absent reading grants none. Everything is evaluated on bars that have already CLOSED, which is this file\'s barstate.isconfirmed: cvClosedRows drops the forming bar, absent stamps are dropped rather than read as epoch-zero, and a tape whose every bar looks closed still loses its last. IT CARRIES ITS OWN CLASSIFIER. pinemath.js exports pineLorentzianKernel and seven tabs use it, but its neighbour label reads rows[idx-4].c against rows[idx].c - the move ENDING at the neighbour. Measured over 925 scored bars: corr with the PAST 4-bar return -0.658, corr with the FUTURE -0.057. It describes a move that already happened. cvMlScoreAtBar labels each neighbour with rows[idx+H].c, and the i >= H floor on the walk is what keeps that label in the past relative to the bar being scored. pineLorentzianKernel is NOT changed - re-labelling it would silently move seven desks. NO WIN RATE IS CLAIMED. The tab reports the BREAKEVEN hit rate its R:R implies (33% at the strict 2R target) and writes every setup to the shared forward log under CVERSE-<kind>@v1 so the question is settled out of sample rather than asserted. The MT5/MT4 webhook payload is BUILT AND SHOWN for inspection, stamped with the closed bar so a gateway can reject a duplicate, and never sent: there is no fetch, no XHR, no beacon and no API path in the file. Crypto execution stays disabled. The stop sits BEYOND the extreme the sweep reached, never inside the wick that just traded there, and a stop on the wrong side of entry is REJECTED rather than silently repaired by the cost floor',
