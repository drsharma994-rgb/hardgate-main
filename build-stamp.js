...
var HG_BUILD = {
  version: 'hg-v924',
  pack: 'WHY NOTHING TICKETED - OMNIGOLD NOW NAMES THE GATE THAT EMPTIED ITS OWN BOARD. Asked to make the PAXG case speak up when the cost ceiling empties the scan, then told there are still no trades on OMNIGOLD or GOLD SCALP. Egress is still denied here (403 CONNECT to the production host and both Binance routes), so the empty board cannot be reproduced in this sandbox - the fix is to make the desk diagnose itself on the reader screen. THE GAP WAS REAL AND ONE-SIDED. GOLD SCALP has had gsRejectFunnelHTML since earlier packs: N setups held back across M gates, binding gate X at P%, with the per-gate breakdown under it and a WHY SILENT on the fully-empty branch. OMNIGOLD had NOTHING of the kind. A reader could read every panel on that tab - the ceiling panel, the replay tables, the sequential book, the factor separation - and still not learn which gate emptied the scan, because every one of them describes the RULES or the REPLAY and not one counts the scan in front of them. hgOgBlockerFunnelHtml counts it: plans scanned, every hard gate that blocked with its share, the binding gate named whenever there is a single one, and the caveat that a plan fails several gates so the counts do not sum. THE COST CEILING IS THE SPECIAL CASE IT FOLDS IN. hgOgVenueCostNoteHtml renders only when the venue DIFFERS from the replay, and the replay is PAXG - so on the fail-closed default the desk said nothing at all, which is exactly where the ceiling does the most work: on the replay it alone vetoes 84.5% of all plans and 94.7% of the scalp lane at PAXG, against 13.0% and 16.6% at XM. When cost-drag is the binding gate the funnel now adds the venue paragraph - vetoed N of the M plans this scan could price, and of those the SOLE-BLOCKER subset, and of THOSE how many clear the same ceiling re-priced at the other venue. THE DENOMINATOR IS THE POINT. A plan that also fails confluence is not a ticket the venue is withholding, so the counterfactual is scoped to plans whose ONLY failing hard gate is the ceiling - the same discipline as the crypto sole-blocker column. With no sole blocker it says a cheaper venue would not have ticketed a single extra plan; with some, it says they would still have to pass every gate they already pass. Dominance is MEASURED from the scan own ledger, not asserted, and a TIE is two causes: the funnel says there is no single binding gate and the venue paragraph does not run. The tie-break is by name so the binding gate is the same gate on every render. The dollar reading aid is the median entry of THIS scan own plans; a scan whose plans carry no levels reports no price and prints no dollars rather than borrowing a spot. NOTHING IS GATED ON ANY OF IT. No ceiling moved, no venue default changed, no plan was dropped or passed. G1-G7, COST_VETO_R 0.30, COST_VETO_R_SCALP 0.15 and the XM UI venue default are untouched. Tests: tests/test-omnigold-cost-emptied.mjs (88 assertions), 18 of 18 mutations caught',
