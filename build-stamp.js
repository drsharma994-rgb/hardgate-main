...
var HG_BUILD = {
  version: 'hg-v886',
  pack: 'CRYPTO SCAN tells the same fact on FOUR surfaces - the live status line, the summary block above the cards, the COVERAGE line and the closing footer - and three packs in a row added something to one of them without walking the rest. Pack 882 split a SCORING CRASH out of `errors` (read, voted on, and then our own code threw) and taught COVERAGE to say so; the empty state still rendered “The other 0 were never read”, fixed in 885, and the STATUS LINE still reported “0 errors” for a run where forty contracts crashed, fixed here. Pack 883 taught the summary block that this desk counts LISTINGS rather than assets - BTC on Delta and BTC on CoinDCX are two rows for one asset - and qualified both its numbers with “across N assets”, while the FOOTER restated the same two numbers unqualified, also fixed here. Each was an additive change that improved the surface it was aimed at and left a sibling contradicting it. The durable part of this pack is the guard: tests/test-cryptoscan-surface-parity.mjs drives the real runScan once and renders every surface from that one run, asserting they agree on the setup count, the scanned count, the asset qualifier and how a crash is described, so the next addition has to walk them or fail there. Two of its own first-round mutations survived and both were gaps in the guard itself - the footer is rendered INTO the cards host so a regex over the whole host was satisfied by either surface, and no fixture exercised the outer catch - both now closed. WHY EMPTY deliberately keeps listing counts: a gate blocked that listing, on that venue\'s bars, so per-listing is the right unit there',
