...
var HG_BUILD = {
  version: 'hg-v946',
  pack: 'GOLD ULTRA CROWNED A MECHANIC THE OTHER GOLD DESKS REFUSE TO PROMOTE, ON THE SAME EVIDENCE. Working the desk list again, the hole this time is not a missing mechanic but a DISAGREEMENT. hg-v943 measured that several mechanics on these tabs are wired OMNIGOLD mechanics, so each carries TWO records, and for p9volbar they disagree in sign with the tab holding the smaller sample: the tab row reads plus 0.155R on n=72, its exact twin P9-VOLBAR reads minus 0.499R on n=274 at minus 2.97 sigma, past the bar at which OMNIGOLD refuses to ticket a mechanic at all. v943 withholds the promotion for exactly that on GOLD SCALP and GOLD SWING. GOLD ULTRA went on CROWNING it, because its crownable test requires book equals prefer and its prefer book was a HAND-TYPED copy of goldind table, sitting under a comment naming that table as the source. Two desks, one mechanic, one body of evidence, opposite answers. THE PREFER BOOK IS NOW READ, not transcribed, so a re-bake moves it with no edit here, and it FAILS CLOSED: with the table unreadable there is no prefer book and nothing is crowned, because this tab whole thesis is that its setups ARE the GOLD SCALP prefer rows and crowning from a stale local list with the source gone is worse than crowning nothing. The two shown-but-never-crowned extras are NOT derived and say so, with the guard asserting each still exists and is not suppressed. THE TWIN CHECK NOW RUNS HERE TOO and it withholds the CROWN AND NOTHING ELSE - the setup still forms, keeps its levels, keeps its prefer tier and stays on the board - with the card naming the twin, both samples and the horizon the record belongs to. It fails OPEN, because a missing verdict is not a negative one, and the read is live so a re-bake that turns the twin positive restores the crown with no edit. WHAT IT COSTS, SAID PLAINLY: on the population this desk forms today p9volbar is 50.4% of the prefer book by count, n=63, and on THIS desk record it reads plus 0.2268R net against p6fail plus 0.2436R. So half the crownable book loses its crown and the local record does not say the mechanic is bad. That is the v943 position, not a new one: refuse to ACTIVELY PROMOTE on the smaller of two contradicting records, and change nothing else. p6fail twin is NOT past the bar at plus 0.73 sigma and it still crowns, so the rule is specific rather than blanket. ONE LEVER, NOT TWO: hgGoldSetTwinCheck(false) reverses it here as well, and wiring that surfaced a defect in v943 own switch - there was no READER of it at all, only a RESOLVER, and hgGoldTwinCheckInit re-reads the window override and localStorage and overwrites the value in force, so asking it whether the check is on silently discards a Set(false) wherever localStorage did not take the write. goldind.js is IIFE-wrapped, so that is every headless harness and every browser with site data blocked. hgGoldTwinCheckOn now reads it. No gate, threshold or verdict moves on any other desk, and no setup leaves any board. Tests: tests/test-gold-ultra-twin-crown.mjs, 35 assertions, 10 of 10 behavioural mutations caught across two files with the baseline verified green first',
