...
var HG_BUILD = {
  version: 'hg-v914',
  pack: 'A CHANGE THIS REPO MEASURED AND DID NOT SHIP. Pack 912 made the GOLD SCALP cost gate reject rather than demote, on the finding that a demote never stopped a trade. The obvious next move is to do the same to the ELEVEN kinds the baked edge table demotes, and the in-sample arithmetic is striking: the live book as the replay ran it is n=2193 at -0.148R (t=-5.71); with the pack-912 cost reject n=1522 at -0.053R (t=-1.75); with today EDGE suppressions n=1205 at -0.020R (t=-0.58); and with the demoted kinds rejected as well n=264 at +0.173R (t=+2.19) - a SIGNIFICANTLY POSITIVE scalp book. It is also circular: those eleven kinds are on the demote list BECAUSE they lost in this very replay, so the survivors t is inflated by the selection that produced them. SO IT WAS TESTED OUT OF SAMPLE, which needs no network - refit the negative-kind set on the first part of the book and apply it to the rest. At 50/50 the OOS book is -0.090R as-is and -0.115R with the cut, WORSE. At 60/40 it is -0.069R against -0.037R. At 70/30 -0.114R against -0.046R. IT DOES NOT SURVIVE: at the even split the cut makes the book worse, at no split does it produce a positive one, the best is -0.037R, and no split leaves the cut book significantly anything (t = -1.92, -0.48, -0.52). In sample it cleared 95% at t=+2.19; out of sample the best is t=-0.48, and that gap IS the selection. THE CHANGE WAS NOT SHIPPED. It would have cost 88% of the desk volume for a result that does not hold. This pack ships the REFUTATION instead: a guard that re-runs the whole comparison rather than remembering its conclusion, so the idea cannot be re-derived from the in-sample number alone and acted on - and so that if a re-bake makes the cut survive, the suite goes red and someone looks. WHAT IT DOES NOT CLAIM: not that the demoted kinds are fine - the book is still negative everywhere here - only that rejecting them, on this evidence, is not shown to help. TWO THINGS CHECKED AND FOUND ALREADY DONE, so nothing was added twice. OMNIGOLD venue: the re-pricing machinery is complete and correct (hgOgVenueNet subtracts a MEAN cost, exact under scaling, and reproduces every measured avgNetR at ratio 1 on all 54 kinds), both presets resolve (PAXG 0.260%, XM 0.020%), and the replay banner ALREADY prints how many kinds stand aside at the active venue with the other preset count beside it - so switching venue is already never a silent change. The remaining venue question is one default, and flipping a fail-closed default to a cheaper venue is a risk decision that belongs to the desk owner, not to this pack. GOLD SCALP nyexh: the only mechanic still significantly negative after the cost reject (t=-2.53) is already SUPPRESSED in the edge table - the committed bake simply predates that row, so it will clear on a re-bake with nothing to change. G1-G7, the gold stop floors and ceilings, the cost-reject bar, the suppress, demote and prefer bars, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
