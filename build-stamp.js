...
var HG_BUILD = {
  version: 'hg-v969',
  pack: 'THE SPREAD LOCK WAS FED ON TWO OF THE FOUR DESKS THAT READ IT, AND THE ADVISORY IT WROTE WAS READ BY NOTHING. hg-v968 fed the gold spread lock from the Delta perp payload on GOLD SCALP and GOLD SWING and stopped there. OMNIGOLD fetches the SAME payload on every scan and its institutional filter is a HARD ledger row (hg-v551) - and it went on reading bid and ask from __hgGoldQuote alone, the global hg-v968 itself proved has ZERO writers. OMNIGOLD 1 awaits the same payload and did the same, so its DATA LOAD line read no quote on every scan since the tab was built. Two desks fed, two documented harder and left dead: the hg-v967 trap one desk along. AND THE ADVISORY VERDICT hg-v968 WROTE WAS ORNAMENTAL: it wrote cand.spreadLock.advisory so a proxy-venue spread would be reported and never dropped, and NOTHING READ THE FIELD - the hg-v955 finding in my own previous pack. It is read now: GOLD SCALP and GOLD SWING paint it once on c.notes; OMNIGOLD folds it into the inst-filter row beside a PASS, so the ledger says SPREAD WIDE on delta-xaut, a gold proxy, reported not gated - while a broker-venue quote of the same width still reads VETO SPREAD LOCK. The 0.25 bar is unchanged and asserted unchanged. ONE READER, STILL: all four desks call hgGoldQuoteFromPerp and the guard asserts no desk parses best_bid or best_ask itself. A named broker quote on the global still wins; a half quote, a missing payload or an absent goldind leave the context exactly as before, proved by lifting each desk real handler out of the file and running it rather than stubbing the reader. NOTHING LEAVES ANY BOARD: OMNIGOLD feed chain ends in Delta XAUTUSD, a gold proxy, so every quote supplied there names a non-broker venue and is reported, never dropped. On OMNIGOLD 1 the spread veto needs a 20-day hour average NOTHING SUPPLIES, so it stays unchecked with or without the quote - the guard runs the engine both ways and requires the veto state, the clear flag and the matrix score to be IDENTICAL. What changes is a load line that reads partial, measured on delta-xaut (gold proxy, not the broker book) instead of no quote. Two seams are asserted textually and say so (hg-v956). No gate is loosened, no threshold moves, no new writer of the quote globals is added. Tests: tests/test-gold-spread-lock-omnigold.mjs, 62 assertions, with 16/16 behavioural mutations caught across goldind.js, omnigold.js and omnigold1.js, baseline verified green before every pass',
