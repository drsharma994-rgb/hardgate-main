...
var HG_BUILD = {
  version: 'hg-v883',
  pack: 'CRYPTO SCAN\'s headline counted listings, not assets. The desk scans Delta AND CoinDCX, both of which list the majors, and reconciles nothing: BTC on Delta and BTC on CoinDCX are two universe rows, two engine runs, two cards, two forward records - and the summary line “12 HIGH-QUALITY setups” counts listings while a reader counts opportunities. I went looking for the wrong thing first. The case I expected to matter was CONTRADICTION, the same asset LONG on one venue and SHORT on the other, both high-quality. Measured over 250 paired tapes with the second venue quoting the same path plus independent noise: at 2 bps both fired 228 times and disagreed once (0.4%), at 5 bps 227 and zero, at 10 bps 227 and once, at 25 bps 221 and three (1.4%) - and in NONE of them did both sides also clear the 75% agreement bar. Rare, and never high-quality. The finding is the other 99.6%: the second venue agrees, on nearly the same bars, for nearly the same reason, so it is not a second opinion but the same one printed twice, and the headline counts it twice. hg-forward already knows better - two records on the same bar with the same horizon are perfectly concurrent, so pack 878\'s INDEP collapses them; the evidence layer stopped double-counting and the display layer never started. Nothing is deduplicated, because which venue to prefer is a trading decision (Delta 0.15% round-trip against CoinDCX 0.20%, and different liquidity). The counts now say how many distinct assets they cover, printed only when that number differs, each card says when the same name fired on the other venue, and the rare opposite-side case is stamped loudly because rare is exactly when a reader needs telling. Also removes allLongs / allShorts, computed on every render and read by nothing',
