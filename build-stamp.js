...
var HG_BUILD = {
  version: 'hg-v927',
  pack: 'THE AGE OF THE EVIDENCE THAT FORMS EVERY GOLD SETUP, SAID ON THE DESK. Asked to update the setups formed on OMNIGOLD and GOLD SCALP, then to do everything logical. Egress is still denied here - 403 CONNECT on Binance, the Binance mirror, Delta India and the production host - so the replay cannot be refreshed from this sandbox, and rebake-gold-literals reports every baked literal already equals its artifact. There is nothing to re-derive offline. WHAT COULD BE UPDATED IS THE ONE THING NEITHER DESK SAID: HOW OLD ITS EVIDENCE IS. Every suppress, demote and prefer on GOLD SCALP and every mechanic verdict on OMNIGOLD comes from ONE replay, and the only record of when that replay ran was prose in a comment - which no desk can read and no re-bake updates. hgOgEvidenceStaleHtml watches the GATE SET moving under the evidence and hgOgEvidenceHealthHtml watches its thinness; NEITHER watches the calendar. So a reader opening either tab was gated by a walk of entirely unstated age, presented as current. As of this build that is 11 days on OMNIGOLD (walk 2026-03-29 to 2026-09-12, 9,897 plans) and 12 on GOLD SCALP (2026-07-10 to 2026-09-10, 2,605 trades). HG_OG_WALK and HG_GOLD_EDGE_WALK are the machine-readable spans, and the age is computed AT RENDER TIME against the real clock - a baked 11 days would be wrong the next morning, so no day count is written into either string. Whole days, floored, so 11 does not become 12 an hour early; a clock before the walk ended reports nothing rather than a negative. NO STALENESS THRESHOLD IS ATTACHED, and that is deliberate: nothing in this repo measures how fast gold edge decays, so a too-old-to-trade line would be invented rather than measured - the fitted number this desk has refused five times. A 400-day-old walk still just reports 400 days. Nothing gates on the age. THE RE-BAKE KEEPS BOTH SPANS CURRENT: scripts/rebake-gold-literals.mjs now rewrites both blocks from the artifacts meta, so npm run gold:rebake moves them and nobody has to remember. A block it cannot locate is FATAL, never skipped - the hg-v921 rule - and the committed tree round-trips to zero drift, which is what proves the writer reproduces the files rather than merely not crashing. The first cut of that writer doubled the indent because it sliced to the declaration instead of the line start; caught by the round-trip before it could land. NOT DONE, AND SAID SO: the third option offered - hand-tuning the suppress/demote/prefer table against the same 2026-09-12 walk - was left alone. Every version of that tested in this sequence failed out of sample, so it is not the logical thing to do. Tests: tests/test-gold-walk-age.mjs (50 assertions), 17 of 17 mutations caught across both files',
