...
var HG_BUILD = {
  version: 'hg-v898',
  pack: 'THE GOLD DESKS WERE SETTLING EACH OTHER\'S RECORDS ON THE WRONG BARS. hgFwdSettleOne counts bars from the rows it is HANDED and compares that count against rec.horizonBars, which is expressed in the RECORD\'s timeframe - so handing it the wrong candles silently rescales the horizon. And hgFwdResolve(sym, null, rows) settles EVERY open record for a symbol whatever its timeframe, which four gold desks called with whichever candles happened to be in scope: goldscalp records 1h and resolved with rows4h, golddirection records 1h and resolved with rows4h, goldswing resolved with rows4h and goldultra with rows15m - each of them settling the others\' records as well as its own. MEASURED, 600 synthetic tapes per case. A 1h record walked over 4H bars: 5.2% of outcomes flipped and EVERY ONE was a win turned into a loss, because a 4H bar that touches stop and target in the same bar resolves as STOP - candles cannot say which printed first. Hit rate 21.5% to 16.3%, a one-directional 5.2-point understatement. A 4H record walked over 15m bars: 86.3% of outcomes changed, and 518 of 600 real wins and losses were thrown away as premature EXPIRY - twenty 15-minute bars is five hours, not the three days twenty 4-hour bars would have been - and expired records are excluded from the hit rate entirely. That is a large part of why the gold desks could not accumulate settled evidence. THE FIX, IN TWO PARTS. hgFwdBarSecs measures the median spacing of the rows (median, so a weekend gap, a missing bar or a repeated one cannot decide it) and hgFwdSettle refuses a KNOWN mismatch, making the caller\'s mistake impossible rather than merely discouraged. Unknown on either side - a record with no timeframe, one on a timeframe the table does not name, or bars too few or irregular to measure - fails OPEN and settles exactly as before, because stranding records unsettled for ever is the louder failure. Then hgFwdResolveMulti walks each timeframe over its own candles, and the four gold desks pass every row set they hold instead of one. eightypercent.js and newgold.js already passed their timeframe and are untouched; the crypto path passes a matching timeframe and is unaffected. A refused record is not lost - it stays open until its own bars arrive. ALSO MEASURED AND LEFT ALONE: hgApplyGoldBestLevels applies plan metadata to a LOCKED conviction while gating the levels behind if (!gc.locked), which looked like the same mixing defect. Across 3,360 plans over 7 setup kinds and 4 entry offsets the plan never moved the entry, and entryType, entryGuidance, fillProb and fillNote are all functions of that entry, so the metadata is consistent with the levels. No fix shipped for a bug that could not be demonstrated. G1-G7, the Wilson lower bound, the 80% accuracy floor and the effN correction are unchanged. Record only, nothing fetched or sent',
