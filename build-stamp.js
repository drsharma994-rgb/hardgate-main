...
var HG_BUILD = {
  version: 'hg-v887',
  pack: 'CRYPTO SCAN\'s audit table cost 64.5 KB a contract, and most of it was the same five fields repeated on every card. The tab\'s pitch is that every one of the engine\'s 470 reads can be audited by eye, so the reads have to be kept - dropping them removes the feature. What does not have to be kept is 470 eight-field objects PER CONTRACT. Measured with --expose-gc over 200 real engine runs, retaining exactly what __results.setups[].votes retained: 64.5 KB per setup, so 18.9 MB at 300 contracts, 31.5 MB at 500 and 50.4 MB at 800, held for as long as the tab is mounted and rebuilt every ten minutes by the auto-refresh. Five of each read\'s eight fields - id, group, name, kind, why - are the same on every card; checked over 200 engine runs across bar counts from 230 to 500, four price scales and a wide drift/vol sweep, ZERO rows differed index-for-index. They are now stored once per scan and each contract keeps only read, vote and regime. End to end through the real runScan on 120 contracts: 51.5 KB per setup down to 11.2 KB, a 4.6x reduction, so 15.1 MB becomes 3.3 MB at 300 contracts and 40.2 MB becomes 8.8 MB at 800. The guard is absolute: csVotePack compares every constant field against the template and returns null on the slightest mismatch, and the caller then keeps that contract\'s reads whole - a packed card can never show another card\'s reads, and the worst case is that it costs what it cost before. The table is materialised only for a card the reader actually expands, which is where the lazy render already was. This is a cost reduction and not a bug fix; 18.9 MB is affordable on a desktop browser, and it is worth removing because the tab also ships as an Android WebView and because nothing about it was load-bearing. Also removes csCompositionNote, left dead when pack 881 de-duplicated it out of the vote table',
