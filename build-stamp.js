...
var HG_BUILD = {
  version: 'hg-v893',
  pack: 'REPORTED BUG: CRYPTO SCAN was constantly losing setups. It was the universe, and the tab was reporting a clean scan of half of it. This tab\'s identity is "all Delta + CoinDCX futures". The universe comes from xuUniverse, which fetches the two venues as separate legs and is explicit when one dies - it returns the survivor\'s rows plus a note reading "coindcx leg failed: <reason> - Delta India contracts only" - and hgDeskLoadUniverse carries that note straight through on the pack. EDGE, OMNIROUTE, REVERSALSNIPER and cryptogates all read it. This tab never did. So a failed leg silently halved the universe and the tab reported a complete scan of what was left. Driven through the real runScan over a 16-contract universe, eight per venue, failing the CoinDCX leg on the second pass: scan 1 read 16 of 16 (100%) and showed 14 setups, 7 per venue; scan 2 read 8 of 8 (100%) and showed 7, all Delta. Half the cards gone, "100%" both times, and the one sentence that explained it thrown away. On the 10-minute cycle a flaky venue does that over and over, which is exactly what constantly losing setups looks like from the outside. The counts were never wrong - 8 of 8 WERE read - they answered a question nobody asked, because `universe` is whatever the loader handed over rather than what this tab claims to cover. Three things change, none inventing a number: the venue mix is on the COVERAGE line every scan (venues with no contracts omitted, since hgDeskVenueCounts always emits all five of its keys and three dead zeros in front of the two real numbers is how a line stops being read); a venue that shrank since the previous scan is named with BOTH counts and tied to the setups the reader just lost; and the loader\'s own note rides along verbatim. The status line leads with the shrink and is flagged bad, because it is the one thing on that line explaining why the list above is shorter. The previous mix is captured before __results is replaced, in-session only - a reload starts the comparison over rather than claiming a drop it did not witness. A venue that GREW, one appearing for the first time, and the first scan of a session are all correctly not drops',
