...
var HG_BUILD = {
  version: 'hg-v907',
  pack: 'ALL THE GOLD TABS SAY WHEN THEIR TAPE IS NOT MADE OF POSSIBLE CANDLES. Pack 906 gave GOLD SCALP a check for bars a market could not have produced - a high below its own low, a range that does not contain the body, a price that is not a number, a timestamp that repeats or goes backwards. This pack asked what the OTHER fifteen gold tabs do with such a tape. MEASURED by driving the whole GOLD nav group twice, once on a clean tape and once on the same tape with one bar in twenty inverted: EIGHT of the fifteen that can be driven rendered DIFFERENT NUMBERS, and ONE of those eight said so. The eight were omnigold1, optigold, newgold, goldswing, goldscalp, goldultra, goldpine and 80percent; the one that spoke was GOLD SCALP, from pack 906. The same eight move at 2%, 5% and 10% inversion, so the set is a property of what each desk computes rather than of one draw. THE MOVE IS NOT UNIFORM AND IS NOT FEWER SETUPS: GOLD SWING rendered a THIRD LESS than on the clean tape while OPTI GOLD and 80PERCENT roughly DOUBLED. A malformed tape does not fail loudly - it quietly produces a different board. THE RULE NOW HAS ONE HOME. gold-tape-sanity.js holds hgGoldTapeSanity and hgGoldTapeSanityNote, loads before every GOLD-group tab and is cached in the service-worker shell; goldscalp.js delegates to it rather than keeping the second copy. THIRTEEN of the fifteen are wired, on the tapes they actually fetch, with the note carrying each desk own timeframe - OPTI GOLD reports per lane, GOLD PINE per leg, 80PERCENT per rung. THE TWO THAT ARE NOT ARE NAMED RATHER THAN QUIETLY COUNTED AS DONE: super-gold reads other desks published snapshots and goldspot is a spot-price readout, and neither fetches a candle of its own - a test checks that exemption is EARNED, so either one growing a candle fetch fails until it is wired. AFTER THE WIRING, EVERY TAB THAT MOVES ON A MALFORMED TAPE DISCLOSES IT: nine disclose against eight that move, because one desk reports a fault that did not change its own numbers, which is the honest direction to err. No tab prints the banner on a clean tape. IT REPORTS; IT DOES NOT GATE. Not one bar is dropped, rewritten or repaired, the check does not mutate the tape it reads, and the implementation contains no filter or splice for a silent drop to hide in. FOUR WIRED TABS STAY QUIET IN THE HARNESS - omnigold, golddirection, goldpro and tauric - because it reaches their mount shell rather than their scan; they are checked at the SOURCE instead, and the test says so rather than reporting 13 of 15 as disclosure. A REGRESSION THIS PACK CAUGHT IN ITSELF: moving the rule, an early draft accepted anything with a numeric length, and a STRING has one - hgGoldTapeSanity on a string reported one malformed bar and would have painted the banner for something that is not a tape. Array.isArray is restored and pinned; a fabricated fault is the same sin as a fabricated zero. G1-G7, the gold stop floors and ceilings, the baked edge tables, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
