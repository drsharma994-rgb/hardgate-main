...
var HG_BUILD = {
  version: 'hg-v857',
  pack: 'GOLD PINE now reads the gold tape the rest of the family reads. Four gold desks share one side \u2014 hgGoldUniformTape in gold-catalog.js, last close against EMA21 and EMA21 against EMA50 \u2014 and GOLD SCALP, GOLD SWING, OMNIGOLD and GOLD DIRECTION all call it, stamping anything pointing the other way AGAINST GOLD TAPE \u00b7 HELD and never letting it be MOST PROBABLE. GOLD PINE never called it. Found by feeding the whole family one synthetic tape and reading what each of the sixteen tabs renders: on a tape reading SHORT it pinned \u201cMOST PROBABLE SETUP \u00b7 XAUUSD LONG \u00b7 GOOD 66 \u00b7 LEADER \u00b7 Levels are the live ticket\u201d, and on the mirrored bars a SHORT leader on a LONG tape. Not a bias \u2014 there was no tape anywhere in its path, so its leader was whatever scored highest. Same tab that shipped both handoff buttons with no reward floor until pack 848. The verdict now lives in runGoldPineScan, not in the paint, so goldPineScan() hands the same answer to anything that reads it; swing rows judged on the 4H leg, scalp rows on the 15m, the same split hgSmcEnrich already uses there. Measured against the pre-fix build on the same bars: an unread tape renders byte-identical, an up tape stops pinning SHORT, and a down tape stops pinning a live-ticket LONG and pins a watch row the right way instead \u2014 a downgrade, and the honest one. Fail-open throughout: a flat stack, a thin stack or gold-catalog.js absent entirely reads unread and holds nothing, so this can only remove a claim, never invent one',
