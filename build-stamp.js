...
var HG_BUILD = {
  version: 'hg-v869',
  pack: 'CRYPTO SCAN reported a broken scan as a quiet market. The empty state was one sentence for four different outcomes: “No setups found — no contracts generated signals.” It said that whether every contract was read and none had a directional signal, or every contract failed to fetch, or every contract was dropped for having fewer than 230 closed 15m bars (cryptoultra’s own MIN_15M — CoinDCX carries young and thin contracts that never reach it), or any mixture of the three. The first is a finding about the market. The other three are a finding about the scan, and the tab reported them as the market. The counts already existed: runScan tracks scanned, skipped and errors and stores them on __results. They only ever reached the transient status line, which the next scan overwrites and which the card block never mentions — and the card block is where the claim is made. renderCards was not even handed them, on a fresh scan or on tab re-open. The cards now carry a COVERAGE line — how many contracts actually reached the engine, out of how many, and why the rest did not — and the empty state names which of the four outcomes happened, refusing to claim anything about a contract that was never read. csCoverage, csCoverageHTML and csEmptyHTML are pure and exported; read is scanned minus skipped minus errors, clamped at zero, with no percentage when the universe is empty. It matters most when the scan DOES find something: two signals off 120 of 300 contracts is not two signals off 300, and the coverage line is identical either way',
