...
var HG_BUILD = {
  version: 'hg-v911',
  pack: 'THE GOLD DESKS OWN RANKING SIGNALS, MEASURED - AND NOT ONE OF THEM IS DEMONSTRATED. The gold desks order a board and present the top of it. MOST PROBABLE is a claim about which setup is best, lead-eligibility decides which cards may be crowned, and the A/B/C grade is printed on every card as quality. All three are rankings, and a ranking earns its keep only if the cards it puts first do better than the ones it puts last. Nothing had ever checked that. MEASURED on the largest committed replays this repo holds - 2,445 settled GOLD SCALP trades and 244 settled GOLD SWING trades, net R at the desk own XM venue cost, Welch t on the difference of means. SCALP MOST PROBABLE vs the rest: n=111 at -0.231R against n=2334 at -0.154R, t=-0.74. SCALP lead-eligible vs demoted: n=381 at -0.221R against n=2064 at -0.146R, t=-1.15. SCALP grade A vs B and C: n=1463 at -0.165R against n=730 at -0.113R, t=-0.95. SWING grade A vs B and C: n=9 at +0.972R against n=235 at +0.047R, t=+1.58. NOT ONE CLEARS 95% IN EITHER DIRECTION, and the pack is careful about what that does and does not mean. It is NOT evidence the rankings are backwards - four of the six comparisons in the sweep leaned that way and a t of -0.74 is a shrug, not a verdict. It is NOT evidence they work either - nothing shows a card the desk puts first beating one it puts last. The honest reading is the third: on the evidence held here, all three signals are UNDEMONSTRATED, and the only cell leaning positive rests on NINE settled swing trades. WHAT CHANGED: the desks say so, in the MOST PROBABLE banner, immediately after the line that claims WHY THIS ONE LEADS. NOTHING ELSE CHANGED - no board reordered, no card suppressed, no grade altered. There is no better ordering on hand, an unproven ordering beats an arbitrary one, and swapping a ranking nobody has measured for a different ranking nobody has measured would be motion rather than improvement. A test pins that the note never claims a card was suppressed or re-ranked on the strength of a shrug. FOUND ON THE WAY: every one of the 277 settled GOLD SWING trades in its replay carries demoted true - leadEligible n=0, mpOnly n=0. The swing desk never crowned a leader across the whole 140-day walk, so it has no measurement of its own MOST PROBABLE to show, which is why its banner carries the grade row instead and why the module deliberately holds NO swing mp row. RE-DERIVED, NOT REMEMBERED: a browser cannot read those JSONs so the table is transcribed into gold-rank-evidence.js, and the test recomputes every cell from the committed per-trade rows on each run - pack 910 is the reason a transcription is no longer trusted on its own. The verdict function can still say ranks and backwards, and a test proves it, so undemonstrated is a finding rather than the only thing it returns. G1-G7, the gold stop floors and ceilings, the suppress, demote and prefer bars, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
