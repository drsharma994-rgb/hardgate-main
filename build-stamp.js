...
var HG_BUILD = {
  version: 'hg-v939',
  pack: 'MILLI GOLD SHOWED NOTHING BECAUSE OF TWO DEFECTS I SHIPPED IN hg-v938 - BOTH FOUND BY DRIVING THE TAB END TO END. Asked to check whether the tab shows setups, the honest answer needed the real path: mount OMNIGOLD, let it scan, read what MILLI GOLD paints. Doing that found the tab stuck on WAITING FOR OMNIGOLD, and the reason was mine twice over. ONE - NEVER SCANNED AND SCANNED-AND-EMPTY WERE THE SAME STATE. hg-v938 read __og.lastView and returned null without it. But OMNIGOLD sets lastView = null DELIBERATELY on its honest-empty / no-candles branch - the hg-v540 comment says so in as many words - and that means SCANNED, NOTHING TO SHOW, not NEVER SCANNED. Conflating them told a reader to keep waiting for something that had already happened, and made the tab ask OMNIGOLD to refresh on EVERY paint: a scan storm on exactly the quiet tape where an empty board is the normal state. __og.ran is now the authority on whether a scan happened and lastView only supplies the cards, so an empty board returns an EMPTY LIST with ran:true and only a desk that genuinely never ran returns null. The tab reports the two differently: OMNIGOLD SCANNED AND ITS BOARD IS EMPTY is the desk being quiet, not this tab waiting. TWO - THE FALLBACK COULD NOT START A SCAN. hg-v938 called that tabs refresh(), and refreshOmnigold returns skipped: not run yet when the desk has never run - a no-op in exactly the case the fallback exists for. Open MILLI GOLD without ever opening OMNIGOLD and it waited forever on a desk it had politely asked to do nothing. It now calls hgScanOneTab(omnigold), the shells own entry, which runs HG_TAB_AUTO_SCAN and, when a module refresh skips, clicks that tabs real RUN button; tab.refresh() stays as the fallback for an older shell. PROVED END TO END, not asserted: with the RUN path driven, __og.ran true, 3 cards exported, MILLI GOLD read them and reported 3 scanned / 0 on the roster. AND THE hg-v938 FIX IS NOW MEASURED RATHER THAN CLAIMED - the same bars through the OLD starved path give 16.5 UNCHECKED GATES PER CARD (28% of the ledger); through OMNIGOLDs own fully-fed cards, 8.0 per card (14%). The remaining eight are feeds genuinely absent offline (no macro module, no news, no DXY, no live quote), which is what an UNCHECKED gate is supposed to mean. One assertion of mine survived the mutation pass and was replaced: it grepped the source for hgScanOneTab, and the comment explaining WHY that entry is used still contained the name, so a mutation replacing the LOOKUP with null passed while silently restoring the bug. It is now behavioural - the tab is run against a stub shell and the call is observed. Tests: tests/test-milli-gold.mjs (239 assertions); 8 of 8 mutations caught across milligold.js and omnigold.js. No threshold moves, no gate loosens, the roster and the forward result are unchanged. Suite: 591 files, 0 failed',
