...
var HG_BUILD = {
  version: 'hg-v880',
  pack: 'CRYPTO SCAN printed a regime nothing had established, and “470-read vote engine” was the least informative number on the tab. THE REGIME. countRegime filters to four ids - chop, adx, vhf, ht_mode - so 26 of the 30 regime-kind reads the engine emits are computed, rendered in the vote table looking exactly like the four, and never consulted; over 300 random tapes 14 of those 26 carried a non-zero opinion at least once, hurst and killzone on EVERY tape, pfe on 285, fdi on 273, ravi on 250, er on 212. regimeSummaryVote returns -1 / 0 / +1 and the engine folded 0 into TREND via `< 0 ? \'chop\' : \'trend\'`, so an UNDECIDED reading printed as a measured one: over the same 300 tapes, chop 18 (6.0%), trend by a real reading 224 (74.7%), and trend by DEFAULT 58 - 19.3%, about one scan in five. The third state now has its own name. The GATE is unchanged: it tests `=== \'chop\'`, which 0 never was, so an undecided regime passes exactly as before, and which four reads decide is unchanged too because widening that set moves the gate and that is calibration. The card chip says how many reads were consulted, and the vote table marks each regime row counted or not counted. THE COMPOSITION. Counted from the engine\'s own output rather than written down: of 470 reads, 127 can vote, 30 are regime, 35 are prints (“identical information to X - counted once”) and 278 are n/a - 59% of the table, identical on every tape and every symbol, and no feed in this app can answer any of them. That is the engine behaving well, since 274 name the feed they need and refuse to fake it and 4 decline as not being indicators at all, but a reader told “470-read vote engine” and nothing else will not guess it. Every card now carries a composition line, and the placeholder that hard-coded 470 counts the reads the setup actually holds',
