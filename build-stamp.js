...
var HG_BUILD = {
  version: 'hg-v929',
  pack: 'HOW FAR UNDER THE FLOOR, not merely which gate. GOLD SCALP has named its binding gate since hg-v905, and on a quiet tape it is almost always structure too close - R:R insufficient. A reader who got that far had exactly one obvious move available to them - LOWER THE FLOOR - and nothing on the page to argue with. THE ARGUMENT IS ARITHMETIC AND THE PANEL NOW SHOWS IT. Over 120 consecutive scans through the real goldScalpSetups, 294 of 385 rejections were this gate and 94.9% of them capped TP1 BELOW 0.80R against a 1.20R floor: only 2 sat in [1.10,1.20), so dropping the floor to 1.00R gives back SIX of 294 and 0.80R gives back fifteen. The floor is not what empties the board; opposing structure is. THAT MEASUREMENT IS A SCRATCH HARNESS ON SYNTHETIC BARS, so not one of its numbers is baked: gsRrShortfall reads the numeric rr and rrFloor the reject now carries and computes from the scan in front of the reader, and the guard proves it by feeding two opposite tapes and requiring two opposite answers. A PROSE PARSE WAS THE OBVIOUS SHORTCUT AND IS REFUSED - it would bind the readout to the wording of a sentence and go quietly blank the next time that sentence was reworded, so goldind.js and goldswing.js hand over numbers, the swing side from its own floor variable rather than a retyped literal. IT IS ALSO LOSSY, PROVED THE HARD WAY IN THIS PACK: the first pass at the figures above parsed those sentences and reported 92.8% and EIGHT, because the sentence prints lv.rr.toFixed(1) and an 0.96R cap reads as 1.0R. The panel rendered the true floats, disagreed, and was right; the rounded figures were corrected in the comment, the guard, AGENTS.md and here. TWO THINGS IT MUST NOT OVERSTATE, both rendered and both tested. A row dropped at this gate RETURNED BEFORE THE LATER GATES RAN, so every count is the most a lower floor could give back, never what it would - an upper bound, and it says so. And a lower floor does not only add trades, it adds the same stop bought for a smaller first target; counting the trades a threshold returns without saying what they would be worth is how a loosening argument gets made by arithmetic alone. TWO FLOORS ARE NEVER AVERAGED into one distribution - the larger group leads, any other is named separately, and the tie breaks on the floor so the answer is stable across renders. THIS COUNTS; IT DOES NOT GATE. The 1.2R scalp floor, the swing build floor, G1-G7 and every hg-v925 / hg-v926 / hg-v928 switch are unchanged. WHAT PROMPTED IT: four packs of gate-policy work aimed at an empty board, then a headless walk of the real mint - 120 scans, 0 throws, 52-55 candidates, the hg-v928 retune demonstrably live - which cleared the code and pointed at a geometry gate none of those packs had touched. Tests: tests/test-gold-rr-shortfall.mjs (55 assertions, every step re-derived by hand rather than by the same loop); 18 of 18 mutations caught, four of them only after a first run left four survivors',
