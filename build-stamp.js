...
var HG_BUILD = {
  version: 'hg-v960',
  pack: 'THE GOLD EDGE VERDICTS WERE HAND-TYPED ON TOP OF GENERATED EVIDENCE, AND THE RULE THAT PRODUCED THEM WAS NOT COMPUTABLE. HG_GOLD_SETUP_EDGE.scalp decides suppress / demote / prefer for GOLD SCALP, GOLD SWING, GOLD ULTRA, GOLD DIRECTION and MILLI GOLD. Every row carries a live block the generator writes and an action a human typed, and nothing kept the two in step - so a re-bake moved the evidence and left the decision frozen. That is the hg-v946 hand-typed-prefer-book failure, inside the table hg-v928 retuned. AND THE RULE WAS NOT MERELY UNSTATED, IT WAS UNCOMPUTABLE. The bars are: suppress when live n at least 50 and live gross at most 0 and live net at most -0.20; prefer when live n at least 50 and live gross above 0 and live net at least +0.10; demote when live net is below zero. Both the suppress and the prefer bar need a GROSS measured on the live population, and the live block emitted only net. Four rows carried a liveGross recorded BY HAND inside their hg-v928 retune blocks; for the other twenty-one the number existed nowhere in the repo. So every suppress and prefer verdict in force was unverifiable against the evidence shipped beside it. THE GROSS IS NOW COMPUTED, AND VALIDATED RATHER THAN ASSUMED: computed on the live population it reproduces all four hand-recorded values EXACTLY to four decimals - nyexh -0.1235, liqsweep 0.0313, bosalign 0.0654, ribbon 0.1093 - and in goldind.js the generated live gross now sits one line above the hand-recorded liveGross it matches. It is rounded to 4dp rather than the 3dp net uses, because that is the precision those four were recorded at. THE RULE LIVES ONCE, IN CODE: hgGoldEdgeVerdictFromLive. NO VERDICT MOVES - applied to the shipped table it reproduces 23 of the 25 in-force verdicts, and the two it does not are the two hg-v928 deliberately REFUSED, on n=6 and n=1, because the demote bar has no sample floor and tightening on the thinnest rows in the table is not an improvement. Those two refusals are now DECLARED ON THEIR OWN ROWS as actionWhy, as data rather than as prose. AN UNDECLARED DEPARTURE IS FATAL TO THE BAKE, never a warning: hgGoldEdgeVerdictDepartures partitions every row into follows-the-rule, declared departure and undeclared departure, and rebake-gold-literals refuses to bake while the last list is non-empty, naming the row, the verdict in force and what the rule says. That is the hg-v921 rule - a generated thing the generator cannot account for stops the bake - applied to a verdict instead of to a block. TWO MUTATIONS SURVIVED THE FIRST PASS, both the same shape: the two FATAL branches cannot be reached on a clean tree, so a bucket nothing can land in is a rubber stamp. The enforcement is now an exported function driven on injected inputs, and both refusals are proved to fire and to name the offending row. hg-v937 hit this exact shape and the fix is the same. A THIRD DEFECT FOUND BY THE SUITE, not by me: the hg-v921 round-trip fixture corrupted a live block by matching a FIXED FIELD LIST, so adding gross silently stopped it matching and it corrupted nothing - caught only because that test asserts the fixture really changed something first. It now matches by sample to the closing brace. And its corrupted values read prefer against an in-force demote, which the new enforcement correctly refused; the fixture now changes the numbers without contradicting the verdict, because whether a literal round-trips and whether a verdict follows the rule are two different questions. One survivor is an EQUIVALENT MUTANT and is reported as one: removing the null-row guard leaves the function throwing into its own catch and returning null anyway, proved identical on nine inputs. Nothing is re-baked here - every data route is 403 CONNECT - and the committed tree still round-trips to zero drift. No gate, threshold or verdict moves on any desk, and no setup leaves any board. Tests: tests/test-gold-edge-verdict-rule.mjs, 57 assertions, 19 of 20 behavioural mutations caught across goldind.js, scripts/rebake-gold-literals.mjs and scripts/edge-live-population.mjs with the twentieth proved equivalent, baseline verified green before every pass',
