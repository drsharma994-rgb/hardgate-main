...
var HG_BUILD = {
  version: 'hg-v889',
  pack: 'CRYPTO SCAN\'s 1h leg failed in silence, and the silence promoted. Pack 870 moved the 15m leg onto hgDeskFetchKlinesResult because hgDeskFetchKlines swallows the reason and resolves to [] whatever went wrong - a venue outage and a three-bar contract arrived indistinguishable. The 1h leg, fetched one line further down, was left on the swallowing wrapper and never picked the instrument up. It has exactly one consumer: read 4 of hgOrderFlowScore, the 1h candle-volume imbalance, which runs only when handed 30+ bars and below that is not pushed at all - no vote, no value, no note anywhere on the card. And layer 2 aggregates as the MEAN of the reads that cleared their own gates, so dropping one does not pull the score toward zero: it removes a divisor. Swept over 1,500 random tapes, full 1h leg against none: |layer-2 score| ROSE 912 times (60.8%) and FELL 151 (10.1%), the tier was PROMOTED 204 times (13.6%) against a single demotion, and the PROFESSIONAL-GRADE stamp was gained 74 times and lost none. A failed fetch is a one-way promotion - the 1h read carries weight 0.8 where the others carry 1.0, so it dilutes more often than it decides and losing it concentrates whatever is left. The tier is what the card stamps, what renderCards partitions on and what hg-forward pools by, so a contract whose 1h request timed out could be recorded under a stronger mechanic than the same contract on the same bars with its feed up, and nothing on the card, the status line, the coverage line or the log said which had happened. The score is NOT changed - re-weighting a layer whose 0.35 share and 0.2 direction band were fitted together is a calibration decision, the same one order-flow.js leaves alone in its note on the gated-mean cliff. What changes is that the fact stops being invisible: the 1h leg is fetched through the instrumented form, csH1State separates a dead feed (which keeps the fetch\'s own reason) from a young contract (fewer than 30 closed 1h bars, order-flow.js\'s own gate), the tape is trimmed once so the counter and the card cannot describe different tapes, COVERAGE counts it beside the two silences it is not - the contracts were read and shown - and every card scored on three reads says so, with the measured direction of the bias, directly above the Confidence and Tier line it moved',
