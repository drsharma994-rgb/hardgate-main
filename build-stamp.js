...
var HG_BUILD = {
  version: 'hg-v970',
  pack: 'THE GOLD L2 BOOK IS FED, AND THE DOM RULE IT FEEDS IS MADE VENUE-AWARE BEFORE IT CAN EMPTY A BOARD. hg-v968 proved three quote globals had zero writers and fed the bid/ask one; the L2 book still had none, so the gold DOM rule - validateDomLiquidity, reached from GOLD SCALP and GOLD SWING micro vetoes - had NEVER RUN on any desk. The book rides the same Delta payload the quote does: /v2/l2orderbook is a sixth leg of fetchDeltaPerpHistory in BOTH parser copies, driven on both with an injected fetch. One reader, hgGoldL2FromPerp, beside hgGoldQuoteFromPerp. THE TRAP FOUND BEFORE THE WIRE WENT IN IS THE PACK: that rule vetoes unless the book skews 20 percent toward the trade - a balanced book vetoes both sides - and on GOLD SCALP a non-demote micro veto is a DROP. Feeding a thin gold-proxy book to a rule that strict, unmeasured, would have emptied every gold board: the hg-v966 trap exactly. Driven on the real mints, the same balanced book with no venue drops the scalp board and the swing board to ZERO - the pre-existing rule, untouched - while the same book naming delta-xaut removes nothing, leaves entry, stop, T1, direction and demotion identical, and lands an L2 READ note once on every card. So the spread lock venue rule applies here too and it is ONE rule: hgGoldSpreadVenueOk is the basis-venue test for both. A book naming a non-broker venue is reported, never gated; a book naming no venue, or the broker, behaves exactly as before. A named broker book on the global still wins. The advisory is READ: both micro vetoes return advisory plus note with no reason key, because a reason is a veto to every caller; the scalp and swing callers carry it onto the card notes. OMNIGOLD and OMNIGOLD 1 are deliberately not wired - neither has a reader that can decide on the book - and are asserted un-wired. A vacuous fixture in this pack own guard was caught before shipping: on the earlier swing seed both candidates were Part8/Part9 direct mints that never reach the micro veto, so a seed search found one that mints through the path carrying the rule, asserted first. One survivor is an equivalent mutant, proved identical on 34 inputs. NO GATE IS LOOSENED, no threshold moves, no new writer of the globals is added, and nothing leaves any board. Tests: tests/test-gold-l2-book-fed.mjs, 102 assertions, with 28/29 behavioural mutations caught across goldind.js, goldscalp.js, goldswing.js and both delta-perp-history copies, the twenty-ninth proved equivalent, baseline verified green before every pass',
