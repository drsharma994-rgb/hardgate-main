...
var HG_BUILD = {
  version: 'hg-v873',
  pack: 'The forward log stamped every record with the FORMING bar. hgFwdRecordScan derived it as Math.floor((Date.now()/1000)/sec)*sec, which names the bar currently forming — but an engine votes on CLOSED bars, and cryptoultra’s closedRows drops the forming one, so the bar it actually read is always the one before. Measured on 15m: exactly one bar ahead for an instant scan, two when the scan straddles a boundary, and CRYPTO SCAN walks hundreds of contracts with two sequential fetches each and calls the recorder once at the end, so straddles are routine. Three consequences from one line. The record names a bar that had not closed when the setup was made, implying information the engine did not have. Settlement walks rows STRICTLY AFTER barT, so the first bar of the trade is skipped — and run through the real hgFwdSettleOne, a trade entered at the close of the voted bar that hits T1 on the very next bar settles as a WIN with the true bar and NEVER SETTLES AT ALL with the stamped one, while a trade STOPPED OUT on that same next bar settles as a stop with the true bar and as a WIN with the stamped one, because the stop bar is skipped and the target is reached later. And the dedup rule breaks on the case it exists for: two scans that voted on the same closed bar, ending 14:58 and 15:03, get barT 14:45 and 15:00, so one firing becomes two records. CRYPTO SCAN always had the answer on the setup — res.bar.t, the closed bar the card prints as “closed 15m bar … UTC” — and now passes it. hgFwdRecordScan takes a finite c.barT, floors it to the timeframe and refuses one in the future; a caller that passes none keeps the floor-of-now behaviour exactly, so every other desk is untouched. Absent means absent: a setup with no bar carries none rather than a fabricated one',
