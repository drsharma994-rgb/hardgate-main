...
var HG_BUILD = {
  version: 'hg-v983',
  pack: 'THE SOLIDITY GRADER MEASURED-EDGE GATES JUDGED ON THE ACTUAL-TALLY EXPECTANCY, AND NO FILL-AWARE EXPECTANCY EXISTED FOR THEM TO READ. hgSolGateMeasuredEdge (G6, veto under -0.25R) and hgSolGateMeasuredWinning (G7, promotion at or over +0.5R) grade the cards on OMNIROUTE, REVERSAL SNIPER, DEX SCREENER, OMNIGOLD, NEW GOLD and GOLD PINE from hgFwdStats expR, the expectancy of the actual tally, which settles a resting order the tape never reached as a win or a loss. hg-v981 gave those records a mark and hg-v982 gave the two mechanic judges one fill-aware rule; the ledger carried a fill-aware hit and no fill-aware expectancy, so the one gate that judges expectancy had nothing to read (said in hg-v982, wired here). hgFwdStats carries fillExpR and fillAvgRr, mirroring expR term for term (NaN with no fill-settled record, -1 when none won, else hit times avgRr minus 1 minus hit) and summed through the aggregate fold (fillRrSum) so pruned records keep deciding; hgFwdJudgeSample carries the expectancy of whichever sample it chose; G6 and G7 read it through one local seam and label FILLED with the never-filled count excluded; the KIND PERFORMANCE fill line prints it. Neither the grader nor the two desk judges compute a fill expectancy of their own. Driven end to end: 12 OMNIROUTE-shaped resting records, 4 filled and won, 4 filled and lost, 4 never filled and settled as wins by the actual tally, read +1.667R actual against +1.000R on the orders that opened; the grader on 47 settled at +0.30R passes, and the same 47 of which 25 opened at -0.40R VETOES, naming 25 FILLED samples and 12 never filled excluded; 60 settled at +0.72R promote, the 40 that opened at +0.30R do not; 19 fill-aware records under the floor leave the actual tally deciding; with hg-forward.js absent the grader reads the actual tally exactly as before. No threshold moves; the only verdict that can change is one a fill-aware sample at or over the sample floor overturns, and no crypto record written before hg-v981 carries one. Tests: tests/test-crypto-fill-expectancy.mjs, 38 assertions, with 16/16 behavioural mutations caught across hg-forward.js, hg-solidity.js and hg-perf-panel.js, baseline verified green before every pass',
  built: '2026-09-26T00:00:00Z'
};
