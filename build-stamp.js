...
var HG_BUILD = {
  version: 'hg-v972',
  pack: 'THE MACRO SNAPSHOT NEVER REACHED THE THREE DESKS THAT BORROW THE GOLD MINTS, SO THE DXY+TNX GOLD-LONG KILL RAN UNCHECKED THERE ON EVERY SCAN. hg-v971 gave GOLD PINE, GOLD ULTRA and GOLD DIRECTION one shared live feed for the borrowed GOLD SCALP and GOLD SWING mints - the quote, the book and the perp payload - and NOT the macro snapshot. Those three desks handed the mints no macro at all (GOLD PINE fetched one for its own scoring and dropped it at the mint seam), so hgGoldMacroLock - the 20-day DXY+TNX kill, a HARD DROP on both filter paths - and the forming regime dollar and real-yield reads were unchecked on those desks on every scan, while the two desks the mints belong to have applied that lock since hg-v553. THE FEED NOW CARRIES IT: hgGoldLiveFeed reads the SAME getGoldMacro every fed desk reads, under the same timer, falling back to the last cached snapshot on a timeout and to null on every failure - never a snapshot invented; hgGoldApplyLiveFeed fills inp.macro only where the input carries none, so GOLD PINE own fetch - now carried into the mint instead of dropped - wins over the shared feed. THIS PACK CAN REMOVE LONGS FROM THOSE THREE BOARDS AND SAYS SO: proved on the real mints, DXY RISING plus TNX RISING drops every gold long with CONVICTION LOCK through the scalp mint, the swing mint and the applier seam the three desks use, and leaves every short identical; a FLAT snapshot or one bullish leg moves NO board. That is parity with the mints those desks borrow, not a new policy - the rule, its bar and its 20-day band are untouched. WHAT IS DELIBERATELY NOT FED: the 5-bar yield guard (validateYieldCorrelation, demote on scalp, DROP on swing) reads inp.us10yCandles, and macro.js has never written that field - so it has been unfed on EVERY gold desk since it shipped, the two fed desks included, which copy it from a snapshot that never carries it. It is a DIFFERENT rule from the 20-day band, unmeasured, and feeding it because the plumbing is at hand is the hg-v966 trap. The guard proves it is live on the top-level field, unreachable through the snapshot, unsupplied by macro.js, and that the applier writes inp.macro and no sub-field. Reported, not fed. TWO EQUIVALENT MUTANTS IN THE FIRST CUT, both a duplicated check: the macro leg carried its own shape test and catch beside the ones on the raced promise, and removing either changed nothing. The duplicates are gone and the single home is proved. Tests: tests/test-gold-live-feed-macro.mjs, 55 assertions, with 28/28 behavioural mutations caught across goldind.js, goldpine.js, goldswing.js, golddirection.js and goldultra.js, baseline verified green before every pass',
