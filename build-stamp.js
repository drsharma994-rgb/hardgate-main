...
var HG_BUILD = {
  version: 'hg-v904',
  pack: 'A CORRECTION TO hg-v903: the WHY NOTHING LED panel was printing two tallies as if they were two parts of one. It read \'N setups held back across M gates\' and, directly beneath, \'and K strategy attempts never reached a gate\'. The `and` reads as a sum, and it is not one. __gsTally counts entries into __gsCand, the strategy-candidate builder. The ranked gates above ALSO hold back setups minted by the ENGINE DETECTORS - liqsweep, sweepob, p6fail, p8range and the rest - which never pass through that counter at all. MEASURED over 400 scans: 1,962 builder attempts, but candidates plus ranked rejections plus the tally came to 2,306, overshooting the attempt count on 317 of the 400 scans. A reader adding the two printed figures would have got a number the pipeline never produced. The line now names its own denominator - \'separately, of N strategy-builder attempts, K never reached a gate\' - and states outright that the gates above cover setups from the engine detectors which do not pass through this counter, so the two are separate tallies rather than parts of one. The split between outvoted and too-thin is unchanged and still never merged into a single figure; a tally of zeroes still renders nothing. Nothing about what is gated changed, in this pack or the last: __gsCand\'s condition, the ranked funnel, G1-G7, the 1.5xATR scalp stop floor, the 3.5x ceiling, the baked edge table, the Wilson lower bound and the 80% accuracy floor are all untouched. This pack is one sentence of arithmetic honesty about a panel shipped one pack ago. Record only, nothing fetched or sent',
