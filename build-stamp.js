...
var HG_BUILD = {
  version: 'hg-v994',
  pack: 'THE ROTATION SEASON REACHED ONE OF ITS FOUR CONSUMERS. rotation.js snapshots season alt / btc / mixed with the alt share; BRAIN read it. STAR TRADER fed the snapshot back into rotationSignal, which wants the raw market list and so answered mixed every time, and then compared against altseason / btcseason, strings the signal never emits: its ROTATION vote had never fired, and since hg-v991 its vote:ROTATION read mark was always absent. The FTS setup stack read rot.btcSeason, a field the snapshot never carried: its caution had never fired on any desk. AI AGENT read rot.leader, also never carried. The two directional reads live once now in rotation.js: hgRotationAgainst (BTC season is a headwind for an ALT long, never for a BTC long) and hgRotationFavours (alt season favours an alt long, BTC season a BTC long; longs only, as this layer has only ever voted). STAR TRADER casts its vote through the favours rule on the season the snapshot carries; the setup stack cautions through the against rule and stays neutral otherwise; AI AGENT reports the season as a note and invents no finding; with the home absent each casts or bumps nothing. hgRotationMark reads the live snapshot for a record at fire time; the forward ledger records rotSeason, rotAltPct and rotAgainst on every record (valid desk hand-ins win, junk is never coerced, the direct door refuses the same junk), folds rt / rta / rto and prints ROTATION SEASON SPLIT on the shared forward panel, nothing where no record carries a mark. Nothing is gated on the marks; no threshold moves; no setup leaves any board: the only behaviour that changes is that a vote and a caution that were written years ago and never fired now fire on the season that exists. Tests: tests/test-rotation-season-marks.mjs (new)',
  built: '2026-10-03T00:00:00Z'
};
