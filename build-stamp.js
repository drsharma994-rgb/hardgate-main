...
var HG_BUILD = {
  version: 'hg-v884',
  pack: 'Pruning destroyed the evidence it promised to preserve, and CRYPTO SCAN is what made it happen. hg-forward caps its record list and prunes oldest-first with the stated bargain “fold the dropped records\' outcomes into the aggregate first, so pruning costs detail and never evidence”. That holds for a SETTLED record - its wins, losses and rrSum survive uncapped in the aggregate and only the detail is lost. It does not hold for an OPEN one: hgFwdFold folds outcomes, an open record has none, so pruning it does not coarsen the evidence, it destroys it - the trade never settles and never counts anywhere. Oldest-first is exactly the wrong order for that, because the records still waiting are by definition the old ones. CRYPTO SCAN made it concrete: it writes one row per setup per 15m bar across the whole Delta + CoinDCX universe into a cap shared by every instrumented tab - 10 setups/bar is 960 rows/day and a record survives ~100h, 20/bar is 1,920 and ~50h, 50/bar is 4,800 and ~20h, 100/bar is 9,600 and ~10h - against a cap the file\'s own comment sizes for “a conservative 150 records/day across ~20 instrumented tabs”. A 4h/20-bar gold setup needs 80 HOURS of bars before it can settle, a 4h/24 desk 96, a daily desk 240. Driven through the real recorder: one open OMNIGOLD MMOVE record, then 40h of CRYPTO SCAN at 50 setups/bar, and before this change the gold record was gone with the aggregate empty - not coarsened, lost. Pruning now takes what the aggregate can absorb first: rows already past their own horizon, settled ones among them, and only if the cap still is not met an open record still inside its horizon. Oldest-first within each group, as before. No cap, threshold or timeframe moves - only which row is chosen when one has to go',
