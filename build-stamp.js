...
var HG_BUILD = {
  version: 'hg-v866',
  pack: 'CRYPTO SCAN’s external-risk layer had never measured anything. Every leaf reader in liquidation-intelligence.js — funding imbalance, exchange flow, whale tracking, cascade detection — is an unwired integration point returning a neutral placeholder, and the two readers above them inherit it, so hgExternalRiskScore over BTCUSDT, ETHUSDT, DOGEUSDT, B-PEPE_USDT, XAUTUSD, empty string, nonsense, null, undefined and {} — ten inputs — gives ONE distinct verdict: riskScore 0, cascadeImminent false, recommendation OK. That is not a finding of low risk, it is the absence of a measurement, and it was indistinguishable from one. Three things read it and all three were dead: the LIQUIDATION_CASCADE veto (confidence 0, trade blocked), the whale CAUTION_LONGS penalty, and the CASCADE / WHALE / FUNDING card flags, whose empty line read as ‘no risk found’. It also reached the PROFESSIONAL-GRADE stamp — noLiquidationRisk is one of four conditions in hgIsProGradeSetup’s conjunction and has been true on every setup ever stamped. Pack 863 rewrote that header to say what the stamp really requires, listed two things it does not, and did not check this one; corrected here to three. Nothing is invented and no gate is loosened. Each reader reports measured:false, the aggregate names which sources are missing, the card prints EXTERNAL RISK UNCHECKED with the feed names, and the verdict fields stay fail-open — this repo’s rule for a missing feed. Each reader also takes an injected dependency, so the arithmetic above the feed is proven to work the day one is wired rather than discovered broken then; one branch already was, since hgWhaleSentiment’s distribution test read flow.inflow > 0.7 && baseScore < 0 with baseScore initialised to 0 two lines above and so could not fire on live data either. With feeds injected the cascade scores 0.90 / high, the veto returns confidence 0, the whale penalty takes 0.615 to 0.461, and the pro-grade stamp is withdrawn',
