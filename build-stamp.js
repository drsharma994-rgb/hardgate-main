...
var HG_BUILD = {
  version: 'hg-v861',
  pack: 'half a gold quote is no quote, not a $4,300 spread. hgGoldSpreadLock is the institutional filter\u2019s spread gate, and when it locks hgGoldInstFilter sets cand.dropped \u2014 no card, no ticket, nothing downstream. Every guard inside its reader was a bare isFinite, and isFinite(null) is TRUE. Handed the shapes a quote feed actually produces: {spreadUsd:null}, {spread:null} and {bid:null,ask:null} each reported 0.000 with unchecked FALSE \u2014 an absent spread presented as a measured PERFECT ZERO, the same claim-from-nothing pack 858 found in the GOLD PRO ledger. Worse, because +null is 0 inside the subtraction, {bid:4300,ask:null} and {bid:null,ask:4300} each produced a spread of 4300.000 and LOCKED: \u201cSPREAD LOCK \u2014 live bid/ask 4300.000 > 0.25\u201d. Half a quote produced a spread equal to the PRICE and hard-dropped the candidate \u2014 a fail-CLOSED on missing data, on a gate whose written rule is fail-open, and it deletes every gold setup for as long as the feed is one-sided. An L2 book with an empty asks array or a level whose price is null did it too. Both faults come from the same coercion and close with it: a one-sided quote is now no quote and reads unchecked, while a genuine zero spread is still a genuine zero and every real reading \u2014 0.20, 0.35 from bid/ask, 300 points, numeric strings, an L2 book \u2014 is unchanged, ceiling still exclusive at $0.25',
