...
var HG_BUILD = {
  version: 'hg-v991',
  pack: 'STAR TRADER CROWNED PRIME AND HIGH, PRINTED ADD TO BOOK AND SEND TO TRADE PLAN, PINNED MOST PROBABLE, AND WROTE NO FORWARD RECORD OF ANY KIND: THE ONE CRYPTO DESK BUILT ON MULTI-FAMILY CONFLUENCE COULD NEVER BE JUDGED, AND NONE OF THE CONTEXT LAYERS IT VOTES WITH (NEWS, REGIME, SENTIMENT, ROTATION, ON-CHAIN, SMART $, OI FLOW, STRUCTURE, CONTEXT GATES) HAD EVER BEEN ASKED ON THIS DESK WHETHER IT SEPARATES. The hg-v981 census of 28 crypto writers was derived from the files that call the ledger, so a desk that never called it was invisible to it; startradertab.js carried no reference to hg-forward at all. Every crypto synthesis with a plan is recorded now (PRIME, HIGH and WATCH drafts alike, so the pool measures the raw synthesis; `ticket` marks a crowned non-draft), under mechanic ST-<plan family> (SWING, SCALP, EDGE, SMART, NEAR), dated on the last CLOSED 4h bar the desk read (never the clock), carrying the mark, the venue funding the ticker had, and one hg-v989 read mark per vote FAMILY: true when the family agreed with points, false when it voted against or only cautioned (a NEWS caution is a calendar that was not clear), absent when the family cast no vote, silent or unloaded alike, said. The vote-family universe fits under the ledger cap of 16 reads, asserted from the source. Open records settle on the 4h bars each scan fetches, before that bar is recorded; the shared forward panel is painted under the cards; the stat line counts records written. Driven through the real tab: mount, click, two crypto records dated on the closed bar with mark, funding and the SWING / SCALP / REGIME / NEWS / SENTIMENT marks; a second scan on the same bar writes nothing (the ledger dedups on the bar); the next closed bar trading through T1 settles both, and hgFwdReadSplit can ask whether REGIME separates on this desk for the first time. The gold lane and the non-crypto contracts record nothing, by choice and said: an FX or index symbol would be read by the ledger crypto macro mark as an alt long, and a gold row would have to enter the three gold-writer censuses and carry its feed, a pack of its own; the guard asserts both record nothing. The synthesis reads nothing back from the ledger: no gate is fed by the record, no threshold moves, no setup leaves any board. Tests: tests/test-startrader-forward.mjs (new); the crypto writer censuses (fill mark, funding, macro) now count startradertab.js',
