...
var HG_BUILD = {
  version: 'hg-v874',
  pack: 'CRYPTO SCAN’s records could never be settled by the fill model, and once they could, the model called them unprovable. hg-forward runs TWO settlements on the same bars: hgFwdSettleOne walks from barT and tests stop and target immediately with nothing requiring price to have reached entry, and hgFwdSettleFill models the order — the file says why both exist, because the naive walk’s error is not random (a limit records PHANTOM WINS, a stop entry records PHANTOM LOSSES). The fill walk needs one field, `mark`, the price when the plan fired. hgFwdNormalize has accepted it since the model was written, and hgFwdRecordScan — the entry point every batch-recording desk uses — never forwarded it, so hgFwdOrderType returned null on every one of their records and the fill-aware pass stood aside for all of them, silently. CRYPTO SCAN has the mark: cryptoultra prices entry = res.price and stamps the plan BUY / SELL, so this desk enters AT THE CLOSE, a market order with no fill ambiguity at all. Passing it exposed the second defect. A market order has no fill bar — it filled at the close of barT, before the first bar of the walk opened — but the walk treated that first bar as the fill bar, and its rule for “the fill bar itself carrying an exit”, which is right for a resting limit whose intrabar order is unknowable, fired on trades whose position certainly existed. Measured over 4,000 synthetic 15m trades, 7.0% came back unprovable at CRYPTO SCAN’s 1.5R ladder and 8.3% at 1.2R. Not neutral: unprovable rows are resolved at the cautious end (omnigold: delete unprovable wins, keep unprovable losses), so the mislabel deleted real wins and kept real losses on a population that was never ambiguous — of the 330 at 1.2R, 52 would have been targets. Market orders now start the walk filled and take the both-in-one-bar → STOP convention this file already states for “the position certainly exists and only the exit is unknown”; limits and stop entries keep the unprovable rule untouched. Absent still means absent: a desk that passes no mark records none and its fill walk stands aside exactly as before',
