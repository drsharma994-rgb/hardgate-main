...
var HG_BUILD = {
  version: 'hg-v865',
  pack: 'CRYPTO SCAN scored a six-day-old sentiment reading as a live one. Every row scripts/sentiment-engine.py writes carries its own ttl and the engine enforces it in is_cache_fresh (no row → False, unparseable stamp → False, else age < entry.get(‘ttl’, CACHE_TTL)). The browser never implemented its half: sentiment.js read entry.stale and nothing else, and the engine only sets that flag when it finds no source at all, never on a row it wrote successfully. The committed cache is stamped 2026-09-13T04:56Z with ttl 300 on all three symbols — 6.4 days old, 1,839× its own time-to-live, is_cache_fresh False for every one — and it was carrying 25% of the three-layer confidence, worth 0.078 on every BTC/ETH/SOL card, while the badge beside it read “bullish lean” rather than stale. Over 280 swept cells, 27 long setups held the ≥ 0.75 PROFESSIONAL-GRADE bar ONLY on that stale number and 144 (51%) printed a tier they had not earned. It cut the other way too: hgSentimentGate can push a major conflict into the tab’s quality gates, so a cold headline score could also VETO a setup — measured as “SENTIMENT CONFLICT: short signal (60%) vs strong bullish sentiment”. Both directions close. What ships is a port of is_cache_fresh, rule for rule, including its two False-by-default cases; no threshold in it is invented, since the ttl is the data’s own and the default is the engine’s CACHE_TTL used exactly where the engine uses it. A stale row is still printed — it is what was read — but labelled with its age and its ttl, scored at zero, and never gated on; a row with no entry now says ‘no sentiment data’ instead of ‘neutral’. A fresh row behaves exactly as before, boost, penalty and major-conflict veto included. The loader is fixed alongside it, because otherwise the rule would mark everything stale five minutes in and never recover: hgSentimentLoad memoised its promise for the life of the page, so sentiment was fetched once per load and never again on a tab built to stay open across a 10-minute scan cycle. It now dedupes only the in-flight request and refetches once the cache falls out of its own ttl. The unused loadThreshold config is gone — the ttl on the data is the only clock',
