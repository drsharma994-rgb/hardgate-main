...
var HG_BUILD = {
  version: 'hg-v921',
  pack: 'THE RE-BAKE CANNOT FETCH FROM THIS SANDBOX, AND THE TWO REASONS IT WOULD ALSO BE PAINFUL ELSEWHERE ARE FIXED. Asked to re-bake the gold replay on current data. Every outbound route is denied by organization policy - api.binance.com, data-api.binance.vision, api.india.delta.exchange, api.coindcx.com and the production host hardgate-main.onrender.com all get 403 to CONNECT from the egress gateway, confirmed against the proxy status endpoint rather than guessed from a timeout. That is not transient and not retryable, so NO BARS WERE FETCHED and no artifact was refreshed. What shipped instead removes the two things that made a re-bake hard even on a machine with network. ROUTE IS NOW A PARAMETER. Both harnesses hardcoded https://api.binance.com, and AGENTS.md documents that cloud VMs commonly get HTTP 451 from Binance - so the one command that could refresh the evidence was unrunnable on exactly the hosts this project is developed on, and the only workaround was editing the harness. lib/klines-source.mjs takes HG_KLINES_BASE (any origin serving the same /api/v3/klines shape) and HG_KLINES_PROXY (a HARDGATE origin whose /api/proxy fetches it for you - the same escape hatch edge-diagnose.mjs uses via HARDGATE_SITE). The two COMPOSE: a mirror inside a proxy hop. Unset, the URL is byte-identical to before, and the harness now prints which route it used. THE LITERALS WRITE THEMSELVES. Five derived blocks across two browser files - the scalp live blocks in goldind.js, and kinds positions 5-8 plus sequentialCells, sequentialTicketHorizon and stopBandGross in omnigold.js - were every one of them HAND-TRANSCRIBED, including by me, four times this session. That is how hg-v909 shipped a block derived from the wrong analysis file. scripts/rebake-gold-literals.mjs rewrites all five from the committed artifacts; it reports drift by default and only writes with --write. It preserves the FILE key order rather than the artifact order, because the first cut re-sorted the stop bands to >=1.733, <0.5, <0.32, <0.133 - same numbers, unreadable table, and a diff that hid what actually moved. A block it cannot locate is FATAL, never skipped. npm run gold:rebake is the whole chain, ending in npm test so a bad bake cannot land quietly; npm run gold:literals is the read-only drift check. THE PROOF IS A ROUND TRIP: on the committed tree the writer reports zero drift, which means it reproduces every literal that was typed in by hand; and the guard corrupts a copy of both files, requires the drift to be detected, requires a dry run to change nothing, then rebuilds them BYTE-IDENTICAL. It also renames a block to prove the missing-block path is fatal, because every block is present today and that branch would otherwise ship untested. 13 of 13 mutations caught. NOTHING ABOUT THE DESKS CHANGED - no threshold, no action, no verdict, no bar. G1-G7, the lead invariant, the gold stop floors and ceilings, the min-loss vetoes, the cost ceilings, the suppress, demote and prefer bars, the Wilson lower bound and the 80% accuracy floor are unchanged. Nothing fetched or sent',
