...
var HG_BUILD = {
  version: 'hg-v876',
  pack: 'CRYPTO SCAN accumulated forward evidence nobody could see. hg-forward ships hgFwdPanelHTML as “a drop-in panel any tab can render with one line — kept here rather than in each tab so the wording, the thresholds and the honest empty state stay identical everywhere”, and five desks render it (REVERSALSNIPER, SQUEEZE, OIFLOW, OMNIGOLD, OMNIROUTE). This one never did, so everything it recorded went into localStorage and stayed there: no reader of the tab has ever seen whether one of its own setups paid. That is the last leg of a chain — pack 873 gave the records the bar the engine actually read, 874 let the fill model settle them, 875 made the scan settle them at all, and none of it was visible. The panel is judged against this tab’s OWN ladder: cryptoultra prices T1 at RULE.t1R = 1.5, so the breakeven hit rate this pool must beat is 1/(1+1.5) = 40%, and the helper’s default of 2R would have tested a plan the desk does not place. Its mechanics are the versioned vote tiers from pack 872, so the table answers the question that instrumentation was for. It sits OUTSIDE the cards host, where an empty scan cannot hide it, and repaints on mount as well as after every scan because the evidence outlives the session. The closing note said “No win rates claimed” while also asserting the engine “produces high volume but low accuracy” — a claim about accuracy with no evidence shown, beside a promise to show none. It now says no win rate is claimed FROM THE SCAN WINDOW, which is the circular claim worth refusing, and points at the panel, which reports only outcomes settled by bars that had not printed when the setup fired',
