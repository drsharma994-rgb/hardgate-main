...
var HG_BUILD = {
  version: 'hg-v986',
  pack: 'CONTRACT REPORT DATED ITS FORWARD RECORD ON THE CLOCK BAR WITH THE 4H SERIES IN HAND, AND DROPPED THE FUNDING ITS PLAN READ; A BEHAVIOURAL CENSUS DRIVES TEN CRYPTO FORWARD-LOG WRITERS THROUGH THEIR REAL SCANS AND NAMES THE ONE IT CANNOT REACH. hg-v985 found two dead crypto forward logs by DRIVING the scans rather than reading them; this pack drives every crypto writer it can (OMNIROUTE, OMNIPRESENT, SUPER BEST, SUPER SNIPER, REVERSAL SNIPER, TRENDMX, GATES, DEX SCREENER, PINE, CONTRACT REPORT) with hg-forward loaded on a tape whose last closed bar is one bar behind the clock and requires one record dated on that bar with a mark. Nine were sound. CONTRACT REPORT was not: planFrom composes its one plan on the last closed 4h bar it is handed and the record it wrote carried the mark (hg-v981) and nothing that said which bar it was, so hgFwdRecordScan dated it on the floor of the clock, the hg-v978 defect on a crypto desk. Settlement walks strictly after barT, so a T1 bar that closed before the clock could never settle the record (proved through the ledger rule), and a FULL REPORT pressed across a clock boundary on the SAME closed bar wrote a second record. The ticker funding planFrom read never reached the record either; hg-v985 named this desk as having no figure in reach at its record site, true of that site, and the plan one function up had it all along. The plan now carries barT (the last row time, seconds, null when unreadable or zero) and fundingPct (number or nothing, never a coerced zero), the record forwards both, dedup follows the signal bar, and the census guard moves CONTRACT REPORT from the bare list to the carriers. EDGE is NOT driven and the reason is checked rather than remembered: on the seed where edgeSignal fires, edgeAssess declines the tally, so the block that records found only is unreachable on a synthetic tape and stays pinned textually. DEX SCREENER grader and one PINE script are stubbed in the census and the guard says so. No gate is loosened, no threshold moves, no setup leaves any board. Tests: tests/test-contract-report-ledger-bar.mjs (52 assertions, 11/11 behavioural mutations caught on contract-report.js) and tests/test-crypto-forward-census.mjs (71 assertions), baseline verified green before every pass',
  built: '2026-09-26T20:00:00Z'
};
