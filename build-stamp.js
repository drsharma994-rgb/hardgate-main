...
var HG_BUILD = {
  version: 'hg-v988',
  pack: 'ONE OUT-OF-SAMPLE VERDICT ON OMNIPRESENT AND IT RUNS AGAINST THE DESK: THE STRETCHED-FROM-EMA21 EXHAUSTION READ, ONE OF THE FAMILIES ITS OWN HARD GATE COUNTS, IS WORSE IN ALL FOUR DISJOINT WINDOWS; NEITHER HARD GATE SEPARATES; AND THE METHOD NOW HAS ONE HOME FOR BOTH CRYPTO DESKS. hg-v987 asked which signal-time read OMNIROUTE scores by separates winners on its own replay; this pack asks OMNIPRESENT the same question through one shared core (scripts/factor-sep-core.mjs: window rule via v984 separate, lean and degenerate classification, literal writer, marker splice), with the OMNIROUTE generator rebuilt as a config over it at zero drift. OMNIPRESENT tickets only a TRIGGERED zone clearing two HARD gates, confluence at or over 3 sources and at or over 2 exhaustion reads (hg-v421), and its replay stamped both verdicts, the zone sources, the evidence, the distance, the score and the cost on 8,502 walked trades (2026-06-13 to 2026-09-11). 32 reads judged; zone sources and evidence families are DERIVED from the rows, the stretched plus or minus N times ATR strings collapsing to one family. Out of sample: ONE verdict. The stretched exhaustion read (n=836) is WORSE 4/4 on win, gross and net: win 26.4% against 31.0%, gross -0.200R, net -0.369R against -0.215R. It is one of the reads the evidence gate counts toward its two, and hg-v421 own trend-guard already says stretch is the trend. The confluence gate leans better on net alone (2/2/4); the evidence gate is worse in three of four on all three figures (n=596, 28.5% against 30.7%, -0.265R against -0.228R), neither a verdict nor a lean and not the direction the gate assumes; the ticket population that clears both (n=165) reads 25.5% win at -0.334R against -0.228R, also 1/1/1. Nine leans named as leans: the cost ceiling in force (costR at or over 0.12, hg-v607) leans worse on net while better on win and gross in three windows, cost arithmetic; prior-day high as a zone source leans better (3/4/4); a zone at or over 1 ATR away leans better on net while worse on win in every window, the stop-width arithmetic again. A verdict here is a measurement, not a gate: nothing on the desk is wired to it. OMNIPRESENT renders the finding through OMNIROUTE one renderer (hgOmniFactorSepHtml) under its cards on both paint paths, and with omniroute.js absent renders nothing rather than a second renderer; the literal is read by that call and by nothing else. npm run op:factor-sep is the drift check. No gate is loosened, no threshold moves, no setup leaves any board. Tests: tests/test-omnipresent-factor-separation.mjs (40 assertions; 17/17 behavioural mutations caught across scripts/factor-sep-core.mjs, scripts/omnipresent-factor-separation.mjs and omnipresent.js, baseline verified green before every pass); tests/test-omniroute-factor-separation.mjs still green over the core',
