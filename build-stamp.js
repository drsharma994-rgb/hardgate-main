...
var HG_BUILD = {
  version: 'hg-v973',
  pack: 'GOLD PINE SWING LANE TOOK A THIRD ROUTE INTO THE SWING MINT THAT NOTHING FED, AND BOTH ITS BORROWED LANES HANDED THE MINTS LESS THAN THE DESKS THEY BORROW FROM. hg-v971 fed the three borrowing desks through one applier and listed GOLD PINE as fed - it fed the SCALP lane. The SWING lane reaches the mint through goldswingCollectCandidates, which called buildCandidates with NO microOpts (news null, so the news gate failed open - the hg-v963 literal in a seventh place; no quote, book, venue or cost override), passed the raw news snapshot where a caution object was expected, and ranked with NO rows for the confluence scorer, so the CONF NO TRADE demote this desk applies read CONF UNCHECKED there and could not fire (the hg-v700 starvation, one route over). THE ROUTE DELEGATES NOW: one route, one home. Proved on the real mint: the route returns the same board as goldSwingSetups stamp for stamp, CONF NO TRADE lands, the news gate locks through it, a wide unnamed quote locks and a wide proxy quote reports, the L2 book reports, the daily bars reach it and the weekend mark lands. THE SCALP LANE fetched daily bars and never passed them (the MTF matrix Daily leg was unchecked), never named its feed, and judged on the wall clock; it now hands dailyCandles, candleSource and the 15m signal bar, and the swing lane hands the desk news and macro, the shared live feed and the 4h signal bar - each lifted out of the file and RUN. THE FEED LABEL DOES REAL WORK, SAID PLAINLY: the mint distrusts PAXG / XAUT volume only when told which feed it is on, so GOLD PINE, GOLD ULTRA and GOLD DIRECTION - which read PAXG and XAUT bars and never said so - let proxy volume vote. Measured on the real scalp mint: named binance-paxg withholds the squeeze and MFI votes, every candidate loses one agreeing read, two fall from grade A to B on one tape, and on another two candidates leave the board. That is parity with GOLD SCALP, which has always named its feed; the rule and its pattern are unchanged. All three desks name the feed now. THE ADAPTER dropped every demote and every advisory note the mint wrote, so a CONF NO TRADE, EDGE DEMOTE or MTF BIAS verdict never reached a GOLD PINE card (hg-v955). They are carried as a MARK and printed - a MINT DEMOTED chip naming the verdict and the notes once - and the leader rule is asserted NOT to read them; a mutation making the leader skip demoted rows is caught, because that is a policy this pack does not set. TWO GREP-SATISFIABLE ASSERTIONS in this pack own guard, caught by its own mutation pass: matching the chip and notes readers by name was satisfied by their definitions while the card called neither; bounded to the call sites inside cardHTML. Two earlier guards lifted the scalp lane without the new signal-bar helper and went red; they lift the real helper now. Two proposed mutations were on pre-existing seams outside this pack (the hg-v700 scorer rows and a second daily-bars consumer) and are dropped, not counted. Tests: tests/test-gold-pine-lanes-fed.mjs, 69 assertions, with 30/30 behavioural mutations caught across goldswing.js, goldpine.js, goldind.js, golddirection.js and goldultra.js, baseline verified green before every pass',
