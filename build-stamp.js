...
var HG_BUILD = {
  version: 'hg-v941',
  pack: 'THE 6-HOUR CONVICTION TTL WAS UNREACHABLE FOR EXACTLY THE RECORDS THAT NEEDED IT, AND SINCE hg-v930 THAT IS A PERMANENT DESK-WIDE LOCKOUT. Asked again why GOLD SCALP shows no setup, the honest answer was not in the mint: it is in the hold. applyHardgateConvictionLock prices each live record against the latest bar OF ITS OWN VENUE, taken from venueRows, and did this: if there is no bar, continue. evaluateSetup then returned null on a missing candle BEFORE it reached the elapsed / TTL branch. So a record whose venue is absent from the current scan was never evaluated at all - not stopped, not targeted, and NOT EXPIRED. GOLD SCALP labels its venue from gold.source, and gold.source walks a feed chain: xm-xauusd, delta-xaut, binance-paxg, proxy. The moment it resolves to a different label than the one a record was minted under, that record is ORPHANED and outlives its expiry for as long as the feed keeps moving. PROVED, NOT ARGUED: the same record, the same clock, 400 hours old against a 6 hour TTL - with its own venue in the scan it EXPIRES, with a different venue label it SURVIVES UNTOUCHED. That pair is the whole defect. WHY IT MATTERS NOW. Before hg-v930 an orphan was a stale history row. Since hg-v930 and hg-v931 the shared reader counts every key in live across BOTH booking stores, and one open conviction HOLDS EVERY NEW SETUP on GOLD SCALP, GOLD SWING and OMNIGOLD - so one unexpirable record silently demotes every card on three desks, forever, and the only trace is the line ONE GOLD CONVICTION IS LIVE, which is exactly what a position someone just took looks like. THE FIX IS THE TTL AND NOTHING ELSE. The elapsed check is the one invalidation that needs no price, so it now runs BEFORE the candle guard, and the pass no longer skips a record it cannot price. Every price-based check still requires a bar: pricing a record against another venue candles would be a worse repair, not a stricter one, and the guard asserts a stop-breaching bar under the WRONG venue does not stop it. A fresh orphan inside its TTL still stands. PLUS DISCLOSURE, because a bare count is how this hid. hgOgOpenGoldConvictions now returns rows - desk, symbol, direction, venue and AGE - and hgOgHoldingRowsHtml names each holding record on both panels, GOLD SCALP delegating to the one definition. A 9h hold on a 6h TTL is now legible on the panel doing the holding. A record whose store never wrote an issuedAt says AGE NOT RECORDED rather than 0m: the first cut used isFinite of plus null, which is zero, and the guard caught it. NO GATE, THRESHOLD OR VERDICT MOVES. G1-G7, the R:R floors, the one-at-a-time hold itself and the lock verbatim-restore are untouched; the guard pins that a live record still restores its issued levels onto its candidate. Tests: tests/test-conviction-orphan-expiry.mjs (43 assertions), 15 behavioural mutations caught across three files. Suite: 593 files, 0 failed',
