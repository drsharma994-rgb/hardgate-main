...
var HG_BUILD = {
  version: 'hg-v870',
  pack: 'A network outage was being reported as thin contracts — including by the COVERAGE line pack 869 shipped. hgDeskFetchKlines swallowed every failure and returned [], so downstream, where CRYPTO SCAN tests rows15m.length < 230 and counts a miss as “skipped (too few bars)”, five distinct causes arrived identically: xuCandles threw (HTTP 451), the venue returned a non-array error body, no candle source was wired, no symbol could be derived, and a genuinely thin contract. Only the last is a fact about the contract; the other four are facts about the fetch, and a Binance 451 for every symbol read out as “300 skipped, fewer than 230 closed 15m bars”. Pack 869 built honest coverage on a counter that already conflated them. hgDeskFetchKlines keeps its exact contract — it still resolves to an array, always — so every existing caller is untouched, including the fall-through to Binance when a non-binance venue has no xuCandles, and the deliberate non-fall-through when that venue answers badly. hgDeskFetchKlinesResult sits beside it returning { rows, ok, reason, error }. CRYPTO SCAN now counts `unread` apart from `skipped`, records which reason each time, and the coverage line names them biggest first: “30 of 100 contracts read (30%) · 60 never fetched: the request failed (45), no candle source is wired for the venue (10), no symbol could be derived for the venue (5) · 10 skipped, fewer than 230 closed 15m bars”. `errors` now means only a throw inside the scan loop, and the empty state names the fetch cause when nothing could be read',
