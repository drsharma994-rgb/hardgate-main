...
var HG_BUILD = {
  version: 'hg-v984',
  pack: 'THE CRYPTO MACRO ALT FILTER IS APPLIED ON SIX DESKS, ASKED BY NONE OF THE CRYPTO DESKS THAT RECORD MOST, AND MEASURED NOWHERE; THE LEDGER NOW RECORDS ITS VERDICT ON EVERY CRYPTO RECORD, AND THE TWO PURE-PRICE DESKS SAY SO ON THE CARD. hgMacroAllowsCrypto (plans.js) blocks an alt long while BTC.D exceeds 55% or the dollar is rising; SWING, SCALP, EDGE, BEST, SETUP CONFIRM and SUPER SETUP apply it, and OMNIROUTE, OMNIPRESENT, SQUEEZE, OI FLOW, DEX SCREENER, CRYPTO SCAN, CRYPTOVERSE and 90PERCENT do not. No artifact in the repo records its verdict beside an outcome, so nobody can say whether it pays; wiring it onto those desks unmeasured is the hg-v966 trap and leaving it unmeasured is the hg-v955 one. One rule, three states: hgMacroAltMark (plans.js) says whether the filter in force would have BLOCKED a plan (true), let it through (false) or could not say (undefined: no direction, a gold-lane symbol, or a regime snapshot with neither gauge readable; the gate fails open on that snapshot, which is right for a gate and wrong for a measurement). The forward ledger stamps it on every crypto record at hgFwdRecordScan from the one rule, so 26 record maps needed no edit; a desk that read it hands it in and wins; hgFwdNormalize keeps only the two booleans; hgFwdFold carries mb and ma buckets so the split outlives the live cap; hgFwdMacroSplit and hgFwdMacroSplitHtml read it, and the shared forward panel prints MACRO ALT FILTER SPLIT on every desk whose records carry the mark and nothing where none does. CRYPTOVERSE and 90PERCENT, which read price alone, take the read once per setup, print MACRO FILTER WOULD BLOCK on the card with the reason, hand the same read to the record, and gate on nothing: the board is identical under a risk-off read, a benign one and no snapshot at all, proved through both real tabs. Found on the way: hgBtcdPct read a null BTC.D gauge as 0% (+null is 0), harmless to the gate and wrong for anything asking whether the leg was read; null means unread now. hgTicketFinalGates carried the gold-lane regex inline; it reads the one predicate hgIsGoldLaneSym. And the nearest measured crypto-macro gate is measured: OMNIROUTE regime row, whose verdict the v701 replay recorded on all 2,833 walked trades, carries NO verdict on four disjoint windows (adverse-veto n=691: worse in 2 of 4 on win, gross and net; whole book -0.232R inside against -0.211R outside; any adverse read or the label against the trade 3 of 4; symbol-trend misalignment 1/2/3), while the module own baked kind demotion separates 4 of 4 and is flagged in-sample because it was fitted on an overlapping window. That is why no regime or post-gate veto is fed to the desks that lack one. Re-derive: node scripts/omniroute-regime-separation.mjs. No gate is loosened, no threshold moves, no setup leaves any board. Tests: tests/test-crypto-macro-mark.mjs, 109 assertions, with 41/42 behavioural mutations caught across plans.js, hg-forward.js, cryptoverse.js, ninetypercent.js and scripts/omniroute-regime-separation.mjs, the 42nd proved equivalent on 468 inputs, baseline verified green before every pass',
