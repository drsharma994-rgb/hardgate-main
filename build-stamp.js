...
var HG_BUILD = {
  version: 'hg-v938',
  pack: 'MILLI GOLD WAS LESS GATED THAN THE DESK IT CLAIMED TO BE A SUBSET OF - FOUND BY THE DESK OWNER READING A CARD. hg-v936 said, in the tabs own panel and in its file header, that nothing was loosened because the roster only ever REMOVES mechanics and every gate, plan and grade is OMNIGOLDs own code. The first half was true and the second was FALSE IN PRACTICE. That tab called hgOgDetect and hgOgEvaluate ITSELF with an EMPTY context. OMNIGOLD hands its evaluator about twenty fields - daily and 4h bars, macro, DXY and yield rows, ADR, news, live and market price, zone context, PAXG basis, quote, L2, bid, ask, the scan clock, the pooled stats - and with none of them ROUGHLY FOURTEEN GATES PER CARD FAIL OPEN TO UNCHECKED: level-fresh, fill-path, htf-daily, macro-realrate, dxy-inverse, yield-guard, adr-budget, news-window, fill-risk, zone-anchor, weekend-exposure and spot-basis, on every scan, by construction. A tab advertising that it was strictly more selective was in fact strictly less checked. THE FIX IS STRUCTURAL, NOT A LONGER ARGUMENT LIST. Rebuilding that twenty-field context inside milligold.js is exactly the second copy that drifts - the thing the file already warned against while doing something worse. The tab now READS THE CARDS OMNIGOLD HAS ALREADY EVALUATED (new export hgOgLastCards, returned by reference so the gate ledgers survive) and keeps the ones on the roster. There is no second context to keep in step because THERE IS NO SECOND SCAN, so every gate is OMNIGOLDs and fully fed by construction rather than by assertion. When that desk has not scanned, this tab ASKS IT TO and waits; it never falls back to scanning differently on thinner inputs, and the guard asserts it does not reach for the detector even as a fallback NAME, since a gfn lookup is a call a paren-check does not see. AND THE CLAIM IS NOW MEASURED RATHER THAN MADE. The tab COUNTS its own gate ledger and prints PASS / AGAINST / VETO / UNCHECKED with the unchecked share, because an UNCHECKED gate is one that did not run for want of a feed, not one that was satisfied, and a reader should be able to see that instead of trusting a comment. The same count would have made the original defect visible on day one. hgOgLastRows is exported too, so the shared gold tape rule judges the SERIES ON SCREEN rather than a freshly fetched one, with the timeframe derived from the bars instead of assumed. One stale exemption came off a list: test-gold-family-scan carried MILLI GOLD as a tab with no candle path of its own, true while it ran a starved scan in a sweep that does not load omnigold.js, false now that it reads that desks cards - and an exception that stops being true has to come off, or the list starts excusing the next bug. Three weak assertions of mine survived the mutation pass and were tightened: the tally asserted the bare word UNCHECKED while the explanatory sentence also contained it, the wait-dont-rescan promise passed on the excuse alone, and the no-rescan check missed a gfn NAME lookup. Nothing about the roster, the gates, the thresholds or the forward result changes. This pack makes the hg-v936 claim TRUE rather than restating it. Tests: tests/test-milli-gold.mjs (222 assertions), 6 of 6 mutations caught. Suite: 591 files, 0 failed',
