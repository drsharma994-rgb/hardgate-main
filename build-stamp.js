...
var HG_BUILD = {
  version: 'hg-v987',
  pack: 'NONE OF THE 36 SIGNAL-TIME READS OMNIROUTE SCORES BY SEPARATES WINNERS ON ITS OWN 2,833-TRADE REPLAY ACROSS FOUR DISJOINT WINDOWS; THE ONLY UNANIMOUS SEPARATOR IS THE MODULE OWN IN-SAMPLE DEMOTION, AND THE DESK NOW SAYS SO UNDER ITS POOLED TABLE. The standing crypto instruction asks for more indicators and strategies in setup formation; before adding one, this pack asks which of the reads the desk already scores by carry any out-of-sample information on its own book. scripts/omniroute-factor-separation.mjs judges every read the v701 replay stamped on all 2,833 walked trades (2026-06-13 to 2026-09-11) through the one separation method this repo keeps (v984 separate, the hg-v920 rule): 14 solidity pillars the replay could score, the solidity gates (families, live-fresh, tape, agreement count), solidity grade GOOD, the legacy tier, the live-price grade at signal, 1h and daily alignment, same-bar cluster, conviction cert, the 20x stop band and three stop-width bands, six sessions. A VERDICT needs all four windows to agree on win AND gross AND net. Out of sample: ZERO verdicts. The module own baked demotion (kindDemoted, n=1634, net -0.375R against +0.001R) is WORSE 4/4 and is flagged IN-SAMPLE (fitted on this window by the v531 roster bake), offered as no confirmation and as proof the method is not blind on this book. Nine LEANS (net unanimous, win or gross not): momentum convergence at or above half its max (net -0.105R vs -0.309R, better w/g/n 2/3/4), the live-fresh gate and the FRESH live grade (2/3/4 each), ASIA session (3/3/4), the regime pillar at or above half (worse on net 4/4 while better on win 3/4), same-bar cluster (worse, 1/0/0), and the three stop bands, where stop at or over 2% reads better on net in 4/4 and WORSE on win in every window: cost arithmetic, not setup quality. A lean is not a verdict. Four pillars (fvg, atrExpansion, orderFlow, newsCalendar) are constant on every replay row and reported STARVED, not measured; three (riskReward, liquidationRecovery, riskAdjusted) clear half their max on every row and are DEGENERATE, no complement. One fill bound, said on the panel: the artifact carries no same-bar ambiguity flag. The finding is baked as HG_OMNI_FACTOR_SEP in omniroute.js by the generator (--write; the read-only run is the drift check, markers fatal when missing), rendered by hgOmniFactorSepHtml under the pooled table beside the forward panel, branching on the verdict list so a future bake that finds one announces it as a measurement and not a gate. The literal is read by the renderer and by nothing else; no gate is loosened, no threshold moves, no setup leaves any board. npm run omni:factor-sep is the drift check. Tests: tests/test-omniroute-factor-separation.mjs (45 assertions; 16/16 behavioural mutations caught across scripts/omniroute-factor-separation.mjs and omniroute.js, baseline verified green before every pass)',
  built: '2026-09-26T22:00:00Z'
};
