...
var HG_BUILD = {
  version: 'hg-v868',
  pack: 'CRYPTO SCAN blamed the reads when the clock had closed the block. isHighQuality needs every quality gate clear AND the pro-grade stamp, and one of those gates is a clock: (h >= 7 && h < 12) || (h >= 12 && h < 17). The two ranges have no gap, so it is one 10-hour block, 07:00–17:00 UTC — ten of 24 hours, which means that for 14 hours a day (58%) no setup can be HIGH-QUALITY whatever its confluence, while the tab’s closing note explained that emptiness entirely as signal quality: “most signals lack sufficient confluence”. For most of the day that is not what emptied it. The window is also fixed UTC and summer-anchored. Read as local sessions via Intl for 2026 it means London 08:00–18:00 and New York 03:00–13:00 in summer, London 07:00–17:00 and New York 02:00–12:00 in winter; London is off the offset it assumes on 155 days of the year (42%), New York on 127 (35%). Which local hours were meant is not recorded anywhere, so the window is NOT moved — picking an anchor is a trading decision, the same call made for the gold session-weight table in pack 860. It is measured and written into the code instead. What ships is the WHY EMPTY panel the SWING tab has had since Fix Pack 17, with the ONLY-blocker column cgSoleBlocker introduced: per gate, how many setups it blocked and how many it was the ONLY thing blocking — a setup in that last column would have been high-quality if that one gate were relaxed, so it says whether you are waiting on the clock, the regime or the reads. Every gate now carries a stable key, csBlockerTally and csFooterNote are pure and exported, and the closing note defers to the tally: when the clock alone emptied the block it says so and counts the setups that cleared everything else, and keeps the confluence explanation for when the reads really are the reason',
