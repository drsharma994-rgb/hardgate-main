...
var HG_BUILD = {
  version: 'hg-v899',
  pack: 'GOLD SCALP: THE DECISION TABLE IS NOW CHECKED AGAINST THE EVIDENCE IT WAS BAKED FROM, and the desk records itself on the timeframe it is actually made of. scripts/gold-setup-edge.json is what a re-bake writes; HG_GOLD_SETUP_EDGE in goldind.js is a HAND TRANSCRIPTION of it, and the transcription is what runs - hgGoldSetupEdgeApply reads the in-code table, never the JSON, and its verdict decides whether a GOLD SCALP setup is dropped, demoted so it can never be MOST PROBABLE, or preferred. Two copies of the same evidence, and nothing compared them: the existing test spot-checks a handful of rows one at a time, which is exactly the coverage that lets a re-bake flip an action in the JSON, pass every assertion, and leave the desk running the old one. Measured when writing this, the 23 rows present in both agreed exactly on n, gross, net and action - so the new test goes green today; it exists for the NEXT re-bake. It compares every actionable row field by field in both directions, allows a neutral row to be left out (the JSON says so in its own note: swing.ob and swing.ribbon are \'no row in the baked table\'), refuses any in-code row the evidence does not have (the hg-v700 lesson, where an inline bos prefer pseudo-row quoted numbers no baked row backed), and checks that the why prose stamped on a demoted card agrees with the net and n beside it - 18 rows quote a figure and all 18 match. WHY IT MATTERS HERE: over 300 synthetic tapes the desk minted 110 setups, and 92 of them were kinds this table marks demote or suppress (bosalign 56, hvn 19, ribbon 9, rsidiv 8). It is not a side table, it decides what GOLD SCALP may lead with. SECOND, THE TIMEFRAME LABEL. goldScalpSetups reads inp.rows15m, takes its entry from the last 15m close and sizes every stop off ATR14 on 15m - yet the desk recorded GOLDSCALP as 1h, a label contradicting the maths behind it, and since hg-v898 the label also decides which candles settle the record. Now recorded as 15m with horizonBars 96, which is the SAME 24 hours the old 24 x 1h meant; measured over 800 tapes with the desk\'s own 1.5xATR15 stop, settling that same window on 15m bars rather than 1h gave an identical verdict every single time, so the label is corrected without moving any evidence. On those tapes 75% of scalps resolved within six hours and 99% within the day - whether a day is the right horizon for a 15m scalp is a calibration question, left open rather than answered by side effect. MEASURED AND LEFT ALONE: the all-demoted board already has NO MOST PROBABLE banner with a fail-closed lead invariant (hg-v699), so that needed nothing. G1-G7, the 1.5xATR scalp stop floor, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
