...
var HG_BUILD = {
  version: 'hg-v863',
  pack: 'CRYPTO SCAN scored sentiment against the trade instead of with it. hgComputeThreeLayerConfidence blends three layers into the number that sets a card’s tier, its shouldTrade verdict and — through hgIsProGradeSetup — the PROFESSIONAL-GRADE stamp the tab turns into its HIGH-QUALITY block and writes to the forward log as `ticket`. Layer 3 went in as ((l3.sentiment || 0) * 0.25), but sentiment.js scores the SYMBOL on an absolute scale (+1 bullish / −1 bearish) and it is hgSentimentScoreSignal there that turns that into agreement with a trade. Added raw it is right for longs and exactly backwards for shorts. One setup, identical price and flow reads at pct 0.80 and flow 0.5, changing nothing but the market’s mood: sentiment −0.80 (bearish, CONFIRMS the short) scored 0.339 weak / no trade, and +0.80 (bullish, CONTRADICTS it) scored 0.799 professional / trade. The short was blocked when the market agreed with it and promoted when it disagreed; longs were right all along, which is why the fault was invisible on half the book. Not hypothetical on the shipped cache — BTCUSDT, ETHUSDT and SOLUSDT all sit at +0.272. Across a 200-cell grid of shorts, 37 (18.5%) held the ≥ 0.75 pro-grade bar ONLY on the wrong sign and none gained it the other way; 86 changed tier and 70 changed shouldTrade, while not one of the 200 long cells moved. Layer 3 is now taken relative to the trade, and an unknown direction contributes nothing rather than a sign picked at random. Also corrected, because the same stamp claimed them: risk-reward is not a pro-grade standard and cannot be while cryptoultra prices every plan off the constant RULE.t1R = 1.5 (the check is 1.5 ≥ 1.5 on every setup ever built), so it stays reported and stays out of the conjunction rather than being folded in to look like a tightened bar; layerAgreement is price versus ORDER FLOW, not all three layers; and the card’s “HIGH-QUALITY (75%+ confidence, trend, liquid hours)” named three of seven conditions, which read worst at zero',
