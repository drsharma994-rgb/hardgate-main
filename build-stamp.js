...
var HG_BUILD = {
  version: 'hg-v875',
  pack: 'CRYPTO SCAN recorded forward evidence and never settled any of it. hgFwdResolve is keyed by SYMBOL and takes the bars, so a desk can only settle what it has candles for — and this tab has written 15m records since hg-v735 without calling it once. Nothing else covered it either: goldultra resolves XAUUSD, every other resolver runs 4h, and hgFwdSettle refuses a record whose timeframe differs. So every row it has ever written sat state:‘open’ until STALE_HORIZONS (3 horizons — 18 hours at 24 bars of 15m) relabelled it “recorded, then the contract went quiet”. That was never what happened: the bars existed, this scan fetches them every cycle for every contract, and threw them away. hg-forward states the principle itself about recording — “a scan that cannot record looks exactly like a quiet market, which is why this says so instead of staying silent” — and an unsettled log is the same lie one step later, since a desk with no wins and no losses looks like a desk with no edge. The scan now resolves each symbol with the bars it already fetched, and does it BEFORE the 230-bar setup test, because a record from an earlier scan needs bars whether or not the contract fires again. A new pure hgFwdOpenSyms(list, tab, tf) returns the symbols that still owe bars — deduped, scoped to tab and timeframe — so the log is loaded once rather than once per contract across a universe of hundreds, and the status line reports how many of the open records a scan settled',
