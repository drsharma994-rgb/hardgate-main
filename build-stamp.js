...
var HG_BUILD = {
  version: 'hg-v906',
  pack: 'GOLD SCALP SAYS WHEN ITS TAPE IS NOT MADE OF POSSIBLE CANDLES. goldScalpSetups checked that it had at least 30 bars, that ATR came out positive and that the entry was a positive number. It never checked that the BARS were well formed, and a feed can deliver ones that are not: a high below its own low, a range that does not contain the body, a price that is not a number, a timestamp that repeats or goes backwards. Those are not edge cases in the maths - they are impossible candles, and every level drawn from them is drawn from something that never happened. MEASURED. A tape where one bar in twenty arrives with its high and low swapped - a proxy glitch or a bad merge - moved the STOP on 36.7% of 300 scans while moving the ENTRY on only 4.7%. That is the worse half to lose: the stop is the risk distance every size, every R and every target on the ladder is measured against. It also minted MORE setups, 350 against the clean tape\'s 340, and said nothing either way. Wholly malformed input was already safe - all-NaN, all-zero, negative, flat and huge-price tapes each minted nothing, and 15 degenerate inputs produced zero throws - so it is the PARTIAL glitch that gets through. gsTapeSanity counts each fault separately and gsTapeSanityNote names them, riding with the unread-leg and mixed-feed banners so it reaches every render path they do. The walk is bounded: it stops at the first bar to reach the cap, and because one bar can carry two faults the reported count can pass the cap by one - clamping it would be a prettier number and a less true one. IT REPORTS; IT DOES NOT GATE. Not one bar is dropped, rewritten or repaired, the check does not mutate the tape it reads, and the banner says so in as many words - a note claiming the bad bars \'were removed\' would be worse than no note, because the reader would then trust levels drawn from a tape nobody cleaned. A test pins that it never makes that claim. TWO NUMBERS RECONCILED RATHER THAN PICKED. A first draft of the test compared entry alone, got 3%, and contradicted the 36.7% in the pack note. Both were real and they measured different things; the wording \'a different set of setups\' was true of the entry-plus-stop signature but read as if the whole board moved. The claim is now the precise one, and the test asserts both halves so the weaker signature cannot quietly come back. THREE AREAS CHECKED AND FOUND CLEAN first: the FORMING NOW watch rows carry \'watch items, not entries\' on both render paths and show one level rather than a ticket ladder; the strategy engine threw on none of 15 degenerate inputs; and the mint-path dedup keys correctly. G1-G7, the 1.5xATR scalp stop floor, the 3.5x ceiling, the baked edge table, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
