...
var HG_BUILD = {
  version: 'hg-v995',
  pack: 'THE TREND MATRIX COMPOSITE REACHED CONTRACT REPORT AS TWO CANDLE ARRAYS, AND WAS RECORDED BESIDE AN OUTCOME NOWHERE. trendtable.js scores five legs (1D vs EMA200, the EMA50/200 cross, the 4H cascade, the cloud, ADX) into a composite of -5..+5 and four consumers read it: the desk board itself (majority at 2), the PINE universe filter, the FTS setup stack and CONTRACT REPORT. CONTRACT REPORT handed trendmxClassify rows4h and rows1h where it wants a scored row and a direction, so its TREND MATRIX row read idle with no detail on every tape, including one the composite scores +5/5; and once that row fired, the report forward-log lookup named the pool TRENDTABLE, which no writer has ever written, where the desk records under TRENDMX. Neither the composite nor its stance toward a plan had ever been recorded beside an outcome. The stance lives once now in trendtable.js: hgTrendMatrixAlign (with / against / neutral, undefined on a string score or no direction, the bar being the majority tmDirOf already applies and not restated), hgTrendMatrixRowOf (the published row for a contract, matched on its base under every spelling) and hgTrendMatrixMark (score, stance and snapshot age for a record at fire time; an unscanned contract or a gold-lane symbol records nothing). CONTRACT REPORT reads the published row through it, names the legs and the source, reads UNCHECKED naming what it could not read when the desk has not scanned the contract, and looks the desk forward log up under the pool it writes. The forward ledger records tmScore, tmAlign and tmAgeMin on every record (desk hand-ins win field by field, junk is never coerced, the direct door refuses the same junk), folds tm[stance] and prints TREND MATRIX SPLIT on the shared forward panel, nothing where no record carries a mark. Nothing is gated on the marks; no threshold moves; no setup leaves any board. Tests: tests/test-trend-matrix-marks.mjs (new)',
