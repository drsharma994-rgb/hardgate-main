...
var HG_BUILD = {
  version: 'hg-v892',
  pack: 'The engine behind CRYPTO SCAN advertised a rule it never applied. THE RULE, as both of cryptoultra\'s surfaces state it: "When >=N reads are decisive, X% of them agree ... that side fires". Neither N nor X exists. rule.minAvail and rule.minPct are read in four places and appear in neither RULE nor any caller\'s override, so both gates were x < undefined - false for every x - and neither has ever fired. Because both surfaces interpolated the keys straight, the tab rendered "Rule: >=undefined decisive - NaN% agree". Measured over 400 tapes through the real engine: 310 produced a direction AND a priced plan, agreement ran as low as 50%, 45 fired under 60% and 25 under 55%. AND A TIE WAS A LONG: lead = L >= S ? \'long\' : \'short\' handed the long side every exact split, so forty reads long against forty short - the engine saying it does not know - was reported as a LONG signal at "50% agree", priced with a real entry, stop and targets, and written into CRYPTO SCAN\'s forward log with a direction half hg-forward splits on. 3 of those 310 were exact ties. The thresholds are NOT invented here: choosing a decisive-count floor and an agreement floor is a calibration decision for the desk, and guessing one would look like a tightened standard while being a number nobody measured. What changes: ruleThreshold returns null for an absent, null or non-numeric key (zero IS a threshold), each gate applies only when its threshold is configured, out.unapplied names the ones that are not, and one shared ruleVoteFloorText drives both surfaces so they state what is actually enforced and become the plain rule the moment the desk configures it. A tie now produces no side and the tally line says "40 long against 40 short - no side" instead of claiming one agreed. A structural test enumerates every rule.X reference in the engine and fails if its key is absent from RULE, so this class cannot recur. CRYPTO SCAN also stops passing allowUnverified: true, a flag cryptoUltraEngine has never read and the only occurrence of that name in the app - it described a permission the tab did not have and the engine did not grant',
