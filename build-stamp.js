...
var HG_BUILD = {
  version: 'hg-v862',
  pack: 'the gold news gate reads the desk’s calendar, it no longer rewrites it. hgGoldNewsGate is the hard block that stops GOLD SCALP and GOLD SWING minting into CPI / NFP / FOMC / GDP (−30 min / +15 min). It built its event list as hgGoldNewsEvents(news) and then pushed the Fed calendar’s FOMC rows into whatever came back — and hgGoldNewsEvents returned news.events BY REFERENCE, where news is the news.js module singleton NEWS: the same array the NEWS tab renders and newsRiskFromEvents walks for every crypto symbol. So a read-only-looking gate appended the whole Fed calendar to the desk’s live news cache on every call. Measured on one real goldScalpSetups pass against a 16-row fomc list: 13 gate calls, 208 rows appended per scan. At the hardcoded 10-minute sweep that is 1,248 rows an hour from GOLD SCALP alone, and GOLD SWING evaluates the same gate on the same cache; sixty scans (ten hours of cycles) took it from 2 events to 12,482 and the scan itself from 22ms to 55ms, because every call re-walks the whole list running four regexes per row. The appended rows were unit-foreign too: news.js stores t in SECONDS and api/fed-calendar.js emits epoch MS, so after one scan the shared array held both. Nothing misreads them today — all four readers of that array test ev.impact first and Fed rows carry no impact — so that half is latent, and the test pins it rather than assuming it. The gate’s own verdict was never wrong: duplicates lock exactly as a single row does. What was wrong is that a reader wrote. The reader now returns a copy and the gate concats; the cache is byte-identical after thirteen consecutive scans',
