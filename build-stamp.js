...
var HG_BUILD = {
  version: 'hg-v877',
  pack: 'CRYPTO SCAN\'s layer 2 was reading the forming bar. cryptoultra drops it (closedRows) before any of its 470 reads run, so layer 1\'s direction, its pct and res.bar all describe one CLOSED bar; the SMC block already trimmed its tape to that same bar and said why - “so SMC grades the same bar”. Layer 2 did not. hgOrderFlowScore was handed the raw fetch, forming bar and all, and layer 2 carries 35% of the confidence, sets layerAgreement and pays the +15% / -20% multiplier against layer 1. Two consequences, both defects. LOOKAHEAD: the forward record is keyed at res.bar.t (pack 873) and hgFwdSettle walks bars strictly after it, so the forming bar sits inside the trade\'s own settlement window - the tier is the log\'s pooling key, so the one question the log exists to answer was being asked of labels that had read their own future. A CLOCK IS NOT A MARKET FACT: hgSweepPattern compares the forming bar\'s PARTIAL volume against an average of COMPLETE bars, volRatio = f x fullVol / avgVol against a 1.5 gate, so it needs f >= 1.5 x avgVol / fullVol and an ordinary bar needs f >= 1.5, which cannot happen. On one fixed tape with one fixed forming bar, moving only the moment the scan ran: 0-50% in gives layer 2 = +0.7224, 3 reads, no sweep; 75% in gives +0.2821, 4 reads, sweep; 97% in gives +0.2236. Trimmed it is +0.7224 at every one of them. Over 495 tapes (476 with a layer-1 direction): layer-2 score differs 405/495 (81.8%), direction 10/495, voteTier 34/476 (7.1%), shouldTrade 27/476, PROFESSIONAL-GRADE 9/476; the sweep read fired 3/495 raw and all three only at 0.99 elapsed. Both layers are now shown the same bars via csTrimToBar / csClosedRows, which SMC shares. No threshold, weight or multiplier moves - the 0.2 band, the 0.35 weight and the agreement multiplier stay calibration and stay the desk\'s call. CS_LABEL_V goes to 5 so v1-v4 records stop pooling with these, and the label-version hash now covers WHICH BARS each layer is handed: this change moved 7.1% of tiers without touching one hashed byte, because it is entirely upstream of the formula. Also corrects a stale figure inside order-flow.js, which still quoted 0.872 as its correlation with layer 1 - measured before pack 867 fixed hgSweepPattern\'s volume divisor, so it never described the code it sat in; the header was corrected then and this copy was missed. It is 0.957, 53 of 56',
