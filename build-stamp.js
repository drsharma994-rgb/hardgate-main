...
var HG_BUILD = {
  version: 'hg-v897',
  pack: 'ALL THE GOLD TABS ON THE ACCURACY FLOOR - and a correction to what v896 said. v896 left OMNIGOLD off the roster and stated in its commit message that the desk \'records none of its own\'. THAT WAS WRONG. OMNIGOLD records under OMNIGOLD:SCALP and OMNIGOLD:SWING through a captured local - var fwdRecord = gfn(\'hgFwdRecord\') - which an extraction keyed on the literal name at the call site could not see. Following that indirection also found OMNIGOLD 1 (pool omnigold1) and GOLD PINE (GOLDPINE:scalp + GOLDPINE:swing) recording and unjudged. Gold coverage goes from 10 of 16 tabs to 13; the roster from 26 desks to 29. THE GUARD NOW RUNS BOTH WAYS. v896 checked only roster -> sources, which catches a name the sources never write but is blind to a desk that records and is simply ABSENT - the exact shape of the OMNIGOLD error. It now also checks sources -> roster: a file that records AND registers a tab must be on the roster or the suite fails. PREFIX FAMILIES RESOLVE THEMSELVES. Hard-coding NEWGOLD:1H/4H and GOLDPINE:scalp/swing put a second list in a second file to drift out of step with the first, which is how a roster ends up naming a pool nothing writes. New hgFwdTabs(prefix) in hg-forward.js returns the desk names the log actually holds - live records AND the aggregate, because pruning at MAX_RECORDS means the desks with the MOST evidence are exactly the ones whose live rows are gone - and a roster entry may now be { prefix }. OMNIGOLD is deliberately NOT a prefix: OMNIGOLD:P80 and OMNIGOLD:TAURIC are separate desks sharing the stem, and a prefix would swallow both and credit their evidence to a tab that did not earn it. That leaves one hole a static check cannot close - a third OMNIGOLD: pool added tomorrow - so hgAccuracyUnclaimed closes it at run time against the log itself, naming any pool accumulating records that no tab on the board is judged on, and the roster board prints it. Independence across pooled names is still the LARGEST, never the sum: a desk split by horizon trades the same instrument at the same time. GOLD SPOT and GOLD COINT stay off the roster and the test proves why - neither mentions entry, stop or t1 anywhere, so there is nothing to measure and no verdict to give; the inline GOLD tab records under a shared CARD:<scanId> family that is not gold-specific. One dead branch removed rather than tested around: the prefix registration inside hgAccuracyUnclaimed could never change its answer, because hgAccuracyPools resolves prefixes from the same list that function walks. G1-G7, the Wilson lower bound, the 80% floor and the effN correction are unchanged. Record only, nothing fetched or sent',
