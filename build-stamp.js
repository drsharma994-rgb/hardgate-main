...
var HG_BUILD = {
  version: 'hg-v992',
  pack: 'THE NEWS CALENDAR READ CLEAR WHEN IT HAD NOT LOADED, AND NEITHER THE BLACKOUT NOR THE FEAR AND GREED GUARD WAS RECORDED BESIDE AN OUTCOME ANYWHERE. hgNewsRisk answered risk low, blackout false on a cold cache and on a calendar leg that failed while the F&G leg loaded, and nothing in the shape said so: GATES G5 read that as CLEAR (its own not-loaded branch was unreachable, since the function existed), STAR TRADER cast a +1 calendar clear vote on it and since hg-v991 wrote a vote:NEWS mark of true from it, and hgNewsGate on OMNIROUTE / OMNIGOLD told a not-loaded calendar apart from a clear one only by matching the note text. hgNewsRisk carries unchecked true in the three dark cases now (cold, calendar-failed, thrown); G5 reads it as NA with the reason, hgNewsGate reads the flag (and still the note for older shapes), STAR TRADER casts no NEWS vote on an unchecked calendar so the read mark is absent, not fabricated. Two marks, one home each, in news.js: hgNewsMark(sym) is the calendar class in force (blackout, high, med, low) or undefined when unchecked; hgFngExtremeMark(value, dir, sym) is the BIAS S2 guard verdict (F&G at or over 80 blocks a fresh long, at or under 20 a fresh short), undefined on a non-number, a missing direction or a gold-lane symbol the index does not speak for; the bar is HG_FNG_GUARD, read by hgOmniMarketSide and proved equal on 10 F&G values. The forward ledger records newsRisk, fng and fngVeto on every record at hgFwdRecordScan (a desk that hands a valid value in wins; junk is ignored, never coerced; a record fired with the module cold reads NOT RECORDED on all three), folds nw / fv / fo buckets so the split outlives the live cap, and prints NEWS CALENDAR SPLIT and SENTIMENT GUARD SPLIT on the shared forward panel, nothing where no record carries a mark. Nothing is gated on the marks (no gate module reads them, asserted); no threshold moves; G5 and the STAR TRADER vote only stop claiming a clearance they never checked. Tests: tests/test-news-sentiment-marks.mjs (new); tests/test-news.mjs follows the flag',
