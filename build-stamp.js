...
var HG_BUILD = {
  version: 'hg-v967',
  pack: 'THE 3-MINUTE GOLD SCALP REFRESH NEVER RAN ONCE. hg-v965 added a dedicated clock in index.html that reads window.goldscalpRefresh every three minutes, and goldscalp.js NEVER PUT THAT FUNCTION ON WINDOW - it existed only as the .refresh property of its HG_tabs entry. So every tick resolved undefined, returned the string unavailable, and the feature the desk owner asked for by name did nothing at all, on every page load, for the whole life of hg-v965 and hg-v966. THE GUARD MISSED IT FOR A REASON WORTH NAMING: it STUBBED ctx.goldscalpRefresh and then asserted the tick called it. That tests the tick and says nothing about whether anything in the shipped tree PROVIDES the function - the same shape as a grep-satisfiable assertion, one layer out, and the same class hg-v963 and hg-v964 each caught once. A stub can only prove the caller; it can never prove the wiring the caller depends on. THE OBVIOUS FIX IS EXPORTING IT, AND THAT IS REFUSED. goldscalpRefresh STARTS A NETWORK SCAN, so putting it on the global object makes it reachable by anything; and test-gold-render-integrity.mjs fuzzes every module-scope export of the gold tabs precisely because this file keeps a small surface. Exporting it weakened a guard to fix a caller, and the full suite said so - the export tipped goldscalp from 19 module-scope exports to 20 and turned that test red. So the export is DELIBERATELY ABSENT, the file says why, and the guard asserts the absence so a later pack does not helpfully add it back. THE FIX RESOLVES RATHER THAN ASSUMES: hgGoldScalpRefreshFn prefers a global where one exists and otherwise resolves the HG_tabs entry, which is the registration the shell own sweep already uses and therefore the authority. With neither present the tick reports unavailable honestly instead of fabricating a function. WHAT THE NEW ASSERTIONS ACTUALLY DO, since the old ones passed while the feature was dead: the real goldscalp.js is BOOTED and the resolver required to find a refresh in it; the registration is asserted directly, so renaming its id or dropping its refresh is caught; a DECOY gold swing tab is pushed so a resolver matching merely the first tab with any refresh is caught; the global branch is exercised separately so both routes stay live; and the tick is driven with NO global and only the registration present - the exact shape hg-v965 shipped into - so a revert to reading the bare global is caught. Found by driving the real module rather than by reading the code: the probe booted goldscalp.js and asked for window.goldscalpRefresh, and it was not there. NO GATE IS LOOSENED, no threshold moves, no setup leaves any board, and the scan this clock runs is the desk own unchanged scan. Tests: tests/test-gold-news-raw-minters.mjs, 118 assertions, with 12 of 12 behavioural mutations caught across goldscalp.js and index.html, baseline verified green before every pass',
