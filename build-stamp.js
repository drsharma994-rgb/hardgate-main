...
var HG_BUILD = {
  version: 'hg-v702',
  pack: 'Gold formation upgrade — one formation contract across OMNIGOLD / OMNIGOLD 1 / NEW GOLD: >=3 independent confirmations from distinct classes, shared venue stop floor, full measured-evidence parity',
  built: '2026-09-10T06:30:00Z',
  note: 'gold-formation.js: the >=3-DISTINCT-CLASS confluence contract (structure / momentum / participation / session-or-HTF) every gold desk now forms on, each confirmation NAMED on the card and every missing class named on a WATCH row. Venue stop floor (8x the venue round trip) delegated whole to hgOgFormation on all three desks — OMNIGOLD 1 stopped calling its $5 SL floor \"the same 0.125R bar\" and NEW GOLD got a cost model for the first time. Evidence parity: kind demotion, v689 KILL-LIST, v685 measured-edge veto, gold-setup-edge and desk-tab-params now read on every gold tab through the SAME function, never a second copy. NEW GOLD’s fabricated `tape: setup.dir` replaced with a real HTF read (4H VWMA-50 regime / OMNIGOLD desk tape) and given a session read. Session leg confirms only where measured gross >= 0 (ASIA +0.097R n=2082, NY-PM +0.053R n=994; LONDON -0.080R n=1305, NY-OVERLAP -0.061R n=2247, OFF -0.011R n=642). newgold.js added to the sw.js precache. Adversarial-audit corrections in the same ship: `inst-filter` is no longer a participation confirmation (it defaults to pass and stays pass when goldind is absent — "nothing objected" is not evidence, and it was satisfying the class on every card on a volume-less feed); the OMNIGOLD 1 placeability rows (Clean Path, Volatility & Target Realism) are excluded the same way the OMNIGOLD fill-path gate already was; the session leg is read on the CLOSED SIGNAL BAR through one shared helper on all three desks instead of three different clocks (NEW GOLD and OMNIGOLD were on Date.now(), so the same bar flipped FORMED/WATCH between scans); and the collapsed stood-aside wrapper counts WATCH and measured-negative separately instead of calling both measured-negative. No win rates. No invented thresholds.'
};
