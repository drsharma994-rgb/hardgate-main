...
var HG_BUILD = {
  version: 'hg-v917',
  pack: 'OMNIGOLD SHOWED A ROSIER RECORD THAN ITS OWN BAKE HAD ALREADY MEASURED. HG_OG_REPLAY_EVIDENCE.kinds is perKind from scripts/omnigold-replay-evidence.json: EVERY settled firing of a detector across the whole walk, priced at the replay PAXG round trip. hgOgReplayNetAtVenue re-prices that number to the desk venue when it renders, and until now that was the only correction applied. THE SAME COMMITTED FILE carries sequentialBake.formedByKind - the same 54 mechanics scoped to the GATE-CLEAR population, what survived the 35-gate stack its own fingerprint names, with netR_xm already computed. NOTHING READ IT. Re-pricing an unscoped population and measuring the scoped one are different corrections and they do not agree. FOR 42 OF THE 54 THE GATE-CLEAR RECORD IS WORSE than the line the card shows: PIN-REJECT reads -0.21R re-priced and -0.94R gate-clear, THREE-BAR -0.24R against -0.93R, PD-EQUILIBRIUM -0.46R against -0.93R. Net-positive at XM falls from 12 kinds to 9 and 7 change sign. So this is not a softer number shown beside a harsh one - it is the HARSHER one, and it is the population the gates actually let through. Every row now carries both: positions 0-4 the unscoped record unchanged, 5-8 the gate-clear n, win rate, netXm and gross, rendered as a second line under the replay line and labelled gate-clear. NOTHING IS PROMOTED OR VETOED DIFFERENTLY, and that was computed rather than assumed. At naive n - the most generous case, since the real path deflates by overlap - NO kind reaches the family bar on either basis. Exactly one reaches the naive 95% bar as displayed and none does once scoped: P6-FAIL, over by +0.0005 and under by -0.0260. That is the whole behavioural difference and it is recorded, not acted on. A BUG THE GUARD CAUGHT IN THIS PACK OWN FIRST CUT: the new line compared itself to the re-priced figure above without checking one existed, so with no venue preset it printed better than the line above - a comparative claim with nothing on the other side. It now makes no comparison when there is nothing to compare to. EDGE_VETO_SAMPLES IS VESTIGIAL IN omnigold.js: declared, never read, and the value left untouched because gold-forward-read.js borrows 30 from it by name. The gold measured-edge gate vetoes at MIN_SAMPLES (20), which is hg-v420 deliberate no 20-29 free-pass choice; omniroute.js is the file that genuinely reads 30. Only the misreading is fixed, and a test pins that this gate does not read it. THE GUARD DOES NOT TRUST THE LITERALS: scripts/omnigold-formed-population.mjs re-derives all 54 gate-clear blocks, the worse-for-42 count, the 12-to-9 drop and the P6-FAIL margins from the evidence JSON on every run, counts EVERY copy of a figure stated in two comments so a stale one cannot hide behind its twin, and exercises the absent branch on a fixture because every kind in this bake has a gate-clear row and the real file therefore cannot guard it. 26 of 26 mutations caught across both files. G1-G7, the gold stop floors and ceilings, the min-loss vetoes, the suppress, demote and prefer bars, the Wilson lower bound and the 80% accuracy floor are unchanged. Nothing fetched or sent',
