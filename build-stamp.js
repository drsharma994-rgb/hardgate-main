...
var HG_BUILD = {
  version: 'hg-v926',
  pack: 'ONE POSITION AT A TIME - THE ONLY BOTH-BOUNDS POSITIVE IN THE WHOLE EVIDENCE BASE, FINALLY ACTED ON. Asked again for better and more profitable gold setups. Every pack in this sequence has looked for BETTER SETUPS and none of the dials moved: hg-v922 measured that NOTHING either gold desk ranks by - tally, grade, demote, tier, confluence, checksPass - separates outcomes across four disjoint windows, and that the one factor which does, stop width, holds in DIRECTION while naming no value. That is a finding, not a failure to search, and it means there is no setting left that makes these desks profitable. SO THIS IS NOT ABOUT WHICH SETUP. It is about HOW MANY AT ONCE. The walk publishes 59.4 plans a day on ONE instrument and holds a time-weighted 57 at once. Every headline table on the tab describes that book. Nobody can run it, and it is not 57 bets - it is ONE bet on gold at 57x size. That half of the argument needs no statistics at all. THE OTHER HALF IS THE SEQUENTIAL BOOK, baked since hg-v918 and never acted on: walk the plans in time order, take one when flat, hold it to its own exit. SCALP tickets +0.1484R at the lower fill bound (n=73) and +0.4155R at the upper (n=54) - POSITIVE AT BOTH ENDS, which is the hg-v918 bar and the only read in this desk evidence that clears it. Against -0.1160R for the all-at-once book over n=8132 at XM. AND THE LIMIT, PINNED AS HARD AS THE CLAIM: at the conservative end that is n=73 at +1.16 sigma against breakeven. Positive, NOT SIGNIFICANT - it does not clear even an uncorrected single test, let alone the 78-mechanic family bar. It is not offered as proof of an edge. SWING disagrees between the bounds (-0.2399 / +0.3166) so it carries NO VERDICT and nothing is claimed for it. The concentration argument stands without either number, and the panel says so. WHAT IT DOES. While a gold conviction is live - read from both booking desks, hgGoldscalpConviction and hgGoldswingConviction, because a position is a position whichever tab opened it - a new OMNIGOLD ticket is HELD. Pushed as an ordinary hard gate through hgOgOneAtATimeGate so it flows into grading, the blocker funnel and the card like every other constraint instead of being a special case in the grader. Cards keep their levels, gates and reasoning; nothing is hidden and NO THRESHOLD MOVED. FAILS OPEN: a corrupt or unreadable conviction store reads as 0 open and the ticket stands. Refusing to ticket because localStorage is unavailable would be a gate nobody chose. ON BY DEFAULT AND REVERSIBLE: hgOgSetOneAtATime(false) persists, window.HG_OG_ONE_AT_A_TIME overrides, a corrupt stored value falls back to the default, and it initialises at mount beside the venue and edge-proof controls. COST_VETO_R 0.30, COST_VETO_R_SCALP 0.15, EDGE_VETO_Z -2, the hg-v925 relaxation and G1-G7 are all unchanged. Tests: tests/test-omnigold-one-at-a-time.mjs (62 assertions), 20 of 20 mutations caught - the first cut only grepped the source for the gate push, which survived both an if(false) in front of it and the pass flag being inverted, so the decision was extracted into a pure function and is now exercised',
