...
var HG_BUILD = {
  version: 'hg-v901',
  pack: 'GOLD SCALP: THE HIGHER-TIMEFRAME READ IS NO LONGER MADE OF EXECUTION BARS, and an unread leg says so. fetchGoldKlines walks a cascade of providers per timeframe and wraps every leg in its own bare catch, so a 4H fetch that fails leaves rows4h empty and says nothing. gold.src already records the source of each leg that SUCCEEDED, so the absence was knowable - it was simply never reported. THE FIX. hgGoldSweepOb called hgGoldHtfBias(opts.rows4h || rows, opts.rows1h) where rows is the 15m EXECUTION tape. hgGoldHtfBias already falls back correctly on its own - __rows(rows4h) || __rows(rows1h) - so a missing 4H drops to the 1H bars, still a higher timeframe than the 15m the setup executes on. Substituting rows handed it the execution bars, so it saw a non-empty first argument, never reached its own fallback, and returned a \'higher timeframe\' verdict computed from 15m candles. Measured over 800 tapes with the 4H leg dropped, the 15m substitute disagreed with the 1H fallback 122 times (15.3%), including outright direction flips - long where the 1H said short. With both HTF legs absent the function now says no HTF bias, which is the honest answer: an absent read, not one invented from the wrong bars. THE DISCLOSURE. Measured over 600 synthetic scans, dropping the 4H leg moved 46 of 485 setup identities (22 appeared only without it, 24 only with it) and took COUNTER-TREND demotions from 16 to 44 - that gate reads D.stack4 !== \'bull\' and an absent 4H stack counts as not-disagreeing, so it demotes MORE, not less. Live setup count barely moved (485 vs 483) and the direction is NOT one-way, so this is no claim that a missing leg inflates the board; it is a claim that the reader could not tell what the board was scored on. gsFeedLegs splits read from unread and gsFeedLegNote names which feed failed, what the board WAS scored on, and the measured effect - riding with the mixed-feed banner so it reaches every render path that already carries a feed caveat. It reports; it does not gate: nothing in it writes dropped, demoted or vetoed, and it never substitutes a leg it found missing. A missing EXECUTION leg raises no HTF caveat, and no feed object at all raises none either - the scan never ran, and the status line says that. THREE HYPOTHESES KILLED BY MEASUREMENT RATHER THAN SHIPPED. A missing HTF leg is NOT a one-way promotion the way CRYPTO SCAN\'s 1h leg was in hg-v889: 22 setups appeared only without the 4H leg against 24 only with it, near enough a wash. The COUNTER-TREND gate treating an absent 4H stack as not-disagreeing is documented, deliberate and fail-CLOSED, not a defect. And hgApplyGoldBestLevels applying plan metadata to a locked conviction, checked in hg-v898, remains consistent because the plan never moves the entry. G1-G7, the 1.5xATR scalp stop floor, the baked edge table, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
