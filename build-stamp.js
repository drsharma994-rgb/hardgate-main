...
var HG_BUILD = {
  version: 'hg-v881',
  pack: 'CRYPTO SCAN\'s card head printed a constant and hid where the stop came from. R:R WAS THE SAME NUMBER ON EVERY CARD: cryptoultra prices t1 = entry +/- |entry - stop| * RULE.t1R, so the ratio is RULE.t1R exactly, always. Measured through the real engine over 359 plans across five price scales, two venues and a wide volatility sweep: ONE distinct value, 1.500000. The tab\'s own footer already said an R:R test “is true for every setup by construction and filters nothing”, and the head went on printing it in the one row a reader scans across five hundred contracts. WHAT DOES VARY IS WHERE THE STOP CAME FROM. stopDist = max(stopAtr * ATR14, price * rtFrac * costFloorMult), so the fee floor wins exactly when ATR14/price < (costFloorMult / stopAtr) * rtFrac - 0.800% on Delta (rtFrac 0.0015) and 1.067% on CoinDCX and Binance (0.002). A derivation, not a simulation. Below those ratios the invalidation level is eight times the round-trip fee and has nothing to do with the chart; over the sweep the floor bound on 39% of plans and pushed stops to 15.15x ATR at the extreme, median 1.50x, p90 3.67x. THE DRAG IS EXACT TOO: rtFrac * price / stopDist is 1/costFloorMult = 0.125R whenever the floor binds, every venue and every contract, and strictly less when it does not - up to 8.3% of the 1.5R target paid to the exchange before the trade can be right. The head now prints the ATR multiple, whether it came from volatility or the fee floor, and the drag in R; neither percentage above is typed into the code, both are derived from RULE and the venue rate, and no threshold moves. Also removes the duplicate composition line pack 880 added inside the vote table - the card body already prints the same breakdown a few lines above it',
