...
var HG_BUILD = {
  version: 'hg-v940',
  pack: 'GOLD SCALP WAS POPULATED ALL ALONG - THE BANNER ABOVE THE CARDS WAS THE PROBLEM. Asked to populate the tab, the honest first step was to drive the real thing rather than the detector in isolation - mount it, click its RUN button, read the FINAL DOM instead of the accumulated writes. It was already carrying TWO CARDS with full levels (XM XAUUSD SHORT, grade B, EMA RIBBON PULLBACK and S52 RANGE-BAR S0 SWEEP), 31,539 characters of rendered cards, and its no-setups div was display:none. The desk had found setups. What said otherwise was the combined banner directly above them: NO TAPE-ALIGNED ENGINE PLAN ON SCALP - STAND ASIDE. WHY IT SAID THAT. hgGoldUniformCompose rejects candidates through SEVEN silent continue statements, and both setups died at the third - demoted - because the hg-v930 ONE AT A TIME hold had withheld them while two gold convictions were already live (the stat line said so: 2 gold convictions live, 2 new setups held). None of the seven ever reached the reader. The banner then emitted one of two fixed strings depending only on whether a tape had been read, so A DESK THAT COMPOSED TWO SETUPS AND WITHHELD THEM READ EXACTLY LIKE A DESK THAT FOUND NONE. That is how this tab came to look empty while holding two fully-levelled cards. THE FIX IS DISCLOSURE AND NOTHING ELSE. Every rejection rule now counts, a candidate is counted ONCE at the FIRST rule that rejected it so the buckets PARTITION the input (the guard asserts they sum to it), and the message names the largest bucket with the true total: 2 engine candidates on SCALP, all demoted (held back, cannot lead) - stand aside. Ties break from the desk withheld it toward the plan was unusable, so the reason a reader can act on wins over one they cannot. THE TAPE IS NAMED ONLY WHEN THE TAPE DID THE WORK - blaming it for a demote was the old messages actual error. NOT ONE GATE, THRESHOLD OR VERDICT MOVES. The same inputs still produce the same setup, the same held list and the same confirmed flag; the guard asserts a clean tape-aligned grade-A candidate still leads and an against-tape candidate is still HELD and shown under hg-v593. The banner is shared, so GOLD SWING and OMNIGOLD get the same honesty. MEASURED, NOT ASSUMED: the direct mint over 120 scans is 108 candidates with 0 throws across 15 mechanics, and 294 of its 385 rejects are structure too close - the hg-v929 finding that the R:R floor is not what empties this board. Nothing here loosens that floor. One assertion of mine survived the mutation pass: an all-one-bucket fixture cannot tell the total apart from the bucket, since two demoted candidates make both read 2, so a mutation reporting the bucket as the total passed unseen; it is now a MIXED board whose buckets are asserted to sum. Tests: tests/test-gold-stand-aside-why.mjs (43 assertions), 8 of 8 mutations caught. Suite: 592 files, 0 failed',
