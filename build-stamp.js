...
var HG_BUILD = {
  version: 'hg-v905',
  pack: 'GOLD SCALP: A LOCKED CONVICTION CAN FINALLY SAY THAT PRICE WALKED THROUGH IT. gsxGeoLine puts hgPlanMarketGeometry\'s verdict on every card - TARGET BEHIND PRICE, price already through the stop, and the rest. It reads c.mark, and hgPlanGeometryLineHtml deliberately stays silent when the mark is unreachable rather than claiming the plan is fine, which is correct on its own terms. THE GAP: convictionCardFromLiveRec rebuilds a locked card from the stored record with entry, stop, targets, anchor and atr - but NO mark, because the record has no idea what price is now. mergeLiveConvictionCards then pushes it into the display AFTER the venue loop that stamps marks on freshly built candidates. So it arrived with mark === undefined and the geometry line stayed silent on exactly the setups that have survived rescans while price walked away from them. Fresh mints were fine: measured over 400 scans, 470 of 470 carried a finite mark. The locked ones carried none. VERIFIED ON THE REAL MERGE. A stored record for entry 2400, stop 2394, T1 2412 came back with mark undefined and a geometry line of \'\'. Given a mark of 2415 the same plan reads \'TARGET BEHIND PRICE: T1 2412 sits between the market and the entry - the retest crosses TP1 before the fill\'. The warning worked all along; it was only ever missing its input. THE FIX: one named rule, gsDeskMark - live goldspot, else the last CLOSED 15m close, the bar the scalp gates already judged - with two callers, the venue loop that stamps fresh candidates and gsStampMergedMarks after the merge. The venue loop\'s inline copy is gone; two copies of that rule would be two things to drift. IT STAMPS; IT DOES NOT GATE. A card that already carries a mark keeps its own. A card with no finite desk mark is left markless and honestly silent, because an invented mark would be worse than none - the geometry line would then speak with authority about a price nobody read. Neither helper writes dropped, demoted or vetoed, and neither touches a level: the lock restores those verbatim, which is its whole point. THREE THINGS CHECKED AND FOUND CLEAN before this one: the mint-path dedup keys correctly on strategy|direction|price-bucket; rescans are fully deterministic, 300 of 300 byte-identical on identical input, so the leader does not flicker; and fresh candidates carry a mark 100% of the time. G1-G7, the 1.5xATR scalp stop floor, the 3.5x ceiling, the baked edge table, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
