...
var HG_BUILD = {
  version: 'hg-v977',
  pack: 'THE TWO HOME GOLD DESKS JUDGED THEIR OWN SIGNAL ON THE WALL CLOCK, AND SUPER GOLD PER-CANDIDATE INSTANT READER HAD NOTHING TO READ. hg-v952, v963, v973, v974 and v975 each moved a borrowing desk from Date.now() to the signal bar - and GOLD SCALP and GOLD SWING, the desks the mints BELONG to, went on handing Date.now() to their own mint (the news gate and the weekend mark), the killzone stamp, the ranker news caution, the watch list, the best-levels session boost, the Part-engine news gates, the A+ cash-open read and the forming stack. On GOLD SWING the last closed 4h bar can sit four hours behind the clock, which is exactly what hg-v973 fixed on GOLD PINE swing lane. Measured on the shipped tree, driving each desk headless under a patched clock on the same bars: GOLD SCALP read 7 grade-A candidates in the NY session with the clock 5 min past the bar and 7 with two downgraded to B and every one stamped OFF-SESSION with it 6 h past; GOLD SWING read both candidates NY AM then OFF-HOURS; a CPI 10 min after the bar locked both desks under the first clock and neither under the second; a high-impact release that is not tier-1 cautioned (the -2 news part, grade A to B) under the first clock and not the second. Each scan now reads ONE signal-bar instant (the last closed 15m bar on SCALP, 4h on SWING, through the one bar reader) for everything that judges the bar, and keeps the wall clock for what IS wall time: conviction age, the weekend-exposure countdown and demote, the scan stamp, the 7-step feed-staleness read. GOLD SWING re-reads its news caution and killzone label on the bar once the bars arrive. Driven end to end under two clocks 5 min and 6 h past the bar: identical boards, the bar session, the news lock and the caution on the bar, and the scan stamp still the clock. SUPER GOLD: sgCandSec (hg-v952) has read c.signalT on every candidate and fallen back to the wall clock every time, because no mint ever wrote that field. Both mints stamp signalT = the instant they judged on, both publishScan copies carry it across the publish boundary (the hg-v955 seam), and SUPER GOLD hands the best-levels pass the candidate own instant and the ranker the newest signal bar among the candidates it ranks. The OMNIGOLD engines panel forming stack reads the 15m bar. Two survivors in the first mutation pass were fixture-equivalent - a tier-1 lock empties the board before the caution can show - and are caught now on a release that cautions without locking; the display-only killzone read is asserted textually and says so. No gate is loosened, no threshold moves; what changes is WHEN the rules on the two home desks are judged: on the bar, like the seven desks that borrow from them. Tests: tests/test-gold-home-desks-signal-bar.mjs, 39 assertions, with 18/18 behavioural mutations caught across goldscalp.js, goldswing.js, goldind.js, super-gold.js and omnigold.js, baseline verified green before every pass',
