...
var HG_BUILD = {
  version: 'hg-v900',
  pack: 'GOLD SCALP NOW SAYS WHICH GATE IS BINDING, not just which setups fell. Every held-back setup already named its reason and the per-instance list still prints every one of them - nothing is hidden and that stays true. But a reason names an INSTANCE (\'opposing structure caps TP1 at 2403.87\'), and a reader staring at an empty GOLD SCALP wants the GATE. MEASURED over 600 synthetic scans of goldScalpSetups: 1,518 rejections collapse to NINE distinct gates, and two of them account for NINETY PER CENT - structure-too-close / R:R-insufficient at 890 (58.6%) and liquidity-sweep-without-volume-climax at 480 (31.6%). The rest: engine stop below the 1.5xATR scalp floor 46, SCALP VWAP bounce measured-negative at XM 40, FVG without HVN support 29, wrong-side long stop 16, wrong-side short stop 9, SCALP FVG FILL measured-negative 7, five-leg sweep engine 1. 66% of those scans ended with nothing live, so an empty board is the state this tab is in most of the time, and a flat list made the reader work the ranking out for themselves across a thousand rows. gsGateFamily collapses the prices, counts and ratios out of a reason so instances of one gate pool together while genuinely different gates stay apart (a long wrong-side stop is not merged with a short one). gsRejectFunnel ranks them by how often each actually bound and names the setup kinds each stopped. The panel renders ABOVE the unchanged per-instance list, in both render paths - the one with cards and the one without. IT COUNTS; IT DOES NOT GATE: nothing in it writes dropped, demoted or vetoed, and no setup is held back or released by anything here. TWO HYPOTHESES KILLED BY MEASUREMENT RATHER THAN SHIPPED. The sweep->OB generator derives its stop from the sweep extreme and its entry from the order block - two independent references with nothing relating them - and on 2,500 random-walk tapes 80% of its plans had a wrong-side stop while every remaining one sat below the 1.5xATR floor. But a tape built to actually contain sweep-displacement-retrace produced no plans at all: the sweep engine never fired, which means those 179 plans came from it firing on NOISE, where bad geometry is the correct outcome and hgGoldPlanSidesOk correctly rejects it. No fix shipped for a bug that could not be demonstrated on a real setup. Separately, an all-demoted board already has no MOST PROBABLE banner with a fail-closed lead invariant (hg-v699), so that needed nothing. G1-G7, the 1.5xATR scalp stop floor, the baked edge table, the Wilson lower bound and the 80% accuracy floor are unchanged. Record only, nothing fetched or sent',
