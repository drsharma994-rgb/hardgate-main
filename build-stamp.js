...
var HG_BUILD = {
  version: 'hg-v896',
  pack: 'THE 80% ACCURACY FLOOR, on every instrumented gold and crypto desk at once. Asked for: update all the gold and crypto tabs to accuracy of more than 80 percent. Accuracy is not a setting - no edit to this repo makes a rule win more often, the market decides that - so what ships is the mechanism that enforces the intent: a desk may CLAIM to clear 80% only when its own settled forward records say so, and \'say so\' means the WILSON 95% LOWER BOUND, never the point estimate. Three wins from three is 100%, and reading that as clearing 80% is exactly the error that makes an over-fitted backtest look extraordinary; Wilson at 3-of-3 says 43.8%. WHAT THE CLAIM COSTS, computed in the file rather than quoted at it: a flawless record needs 16 independent observations (n=15 bounds at 79.6%, n=16 at 80.6%), 95% needs 28, 90% needs 62, 85% needs 246 and 82% needs 1,537 - and at or below the floor the arithmetic stops rather than promising that patience will fix it. THE COUNT IS INDEPENDENT OBSERVATIONS, not rows: a desk firing on 400 contracts on one bar with one horizon has written 400 records and seen ONE thing happen, so the floor reads effN from hgFwdOverlap and a flawless 400-row burst measures under 2 independent observations and does NOT clear; the same 400 wins spread out do. A missing overlap reading is thin, never a fallback to the raw count. A desk pooling under several names sums the record but takes the LARGEST independent count, never the sum - NEWGOLD:1H and NEWGOLD:4H are the same instrument at the same time. THE ROSTER IS CHECKED AGAINST THE SOURCE, NOT TRUSTED. hgFwdPool matches rec.tab with !==, so a plausible-but-wrong name pools nothing and the desk reads NOT YET MEASURED for ever - a wrong verdict wearing the clothes of a patient one, invisible because that is what a new desk looks like. The first cut guessed six of twelve (GOLD SCALP for GOLDSCALP, GOLD SWING for GOLDSWING, GOLD ULTRA for GOLDULTRA, OI FLOW for OIFLOW, REVERSAL SNIPER for REVERSALSNIPER, and an OMNIGOLD that records nothing at all); the test now extracts the recorded names from the sources and fails on drift. Roster corrected and widened from 12 guessed desks to 26 verified ones, spanning both groups and including 80PERCENT, the one desk whose NAME states a rate. OMNIGOLD is deliberately left off: it renders gold setups but records none of its own. Five states, one sentence each - unwired, unmeasured, too thin to judge, below the floor, clears it. Nothing is hidden and nothing is re-ranked; a desk below the floor still shows its setups and is kept on the board, because hiding every card until 16 independent observations accumulate would leave an empty app for weeks. ALSO FIXED, FOUND ON THE WAY: the HG_SHELL parity guard matched ZERO scripts - its pattern anchored on .js\" while stamp-cachebusters.mjs writes ?v=NNN onto every tag - so it had been passing vacuously over 204 scripts, and 29 of them were missing from the offline shell, including the three tabs shipped this week. All 29 added and the pattern fixed, so the guard now checks all 204 and bites when one is dropped. Record only, nothing fetched or sent',
