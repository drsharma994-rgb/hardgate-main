...
var HG_BUILD = {
  version: 'hg-v980',
  pack: 'THE FORWARD LEDGER FILL MODEL NEVER RECEIVED A MARK FROM ELEVEN OF THE TWELVE GOLD DESKS THAT RECORD. hgFwdSettleFill settles a record as if the order had to fill first: a resting limit price never reached is unfilled, a fill bar that also touched an exit is unprovable, only a filled order is judged. It needs one field, mark (the price when the plan fired), to tell a resting order from a market one; hgFwdOrderType returns null without it and the pass stands aside. hgFwdRecordScan was fixed to forward it, and the record maps of GOLD SCALP, GOLD SWING, SUPER GOLD, GOLD DIRECTION, GOLD ULTRA, GOLD PRO, OPTI GOLD, NEW GOLD, OMNIGOLD 1, TAURIC, 80PERCENT and GOLD PINE never passed one; only OMNIGOLD stamps a mark. In GOLD SCALP committed walk 561 of 2,605 plans (21.5%) are resting orders and 90 of 2,193 settled plans never filled; in the live ledger every one was settled as a market order and could record a win it never opened for. Every gold desk forwards the mark it already has (the snapshot mark on the home desks and SUPER GOLD, the mint mark on GOLD DIRECTION and GOLD ULTRA, the last close of the series on GOLD PRO, TAURIC and 80PERCENT, the break bar close on OPTI GOLD, the setup mark on NEW GOLD, OMNIGOLD 1 and GOLD PINE), the normaliser refuses a mark at or below zero, and the gold desk note reads the fill split. Absent stays absent. Driven end to end: OPTI GOLD, whose orders rest by construction, records four SELL_LIMIT plans, the fill model settles three and sets one aside as never filled while the actual tally settles all four; the defect is reproduced first on a limit whose tape never reached its entry (a target hit with no mark, unfilled with one). The census is derived by call shape, aliases included: thirteen gold writers, none bare. No gate is loosened, no threshold moves; the actual tally is untouched by design and the fill-aware figures are counted beside it. Tests: tests/test-gold-ledger-fill-mark.mjs, 48 assertions, with 23/23 behavioural mutations caught across hg-forward.js, gold-forward-read.js, goldscalp.js, goldswing.js, super-gold.js, golddirection.js, goldultra.js, goldpro.js, optigold.js, newgold.js, omnigold1.js, tauric.js, eightypercent.js and goldpine.js, baseline verified green before every pass',
