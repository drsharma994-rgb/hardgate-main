...
var HG_BUILD = {
  version: 'hg-v890',
  pack: 'CRYPTO SCAN numbered a card #1 while stamping it weaker than the card below it. Every quality decision this tab makes is three-layer: the block a card is painted into is isHighQuality, the PROFESSIONAL-GRADE badge is isPro, the mechanic hg-forward pools the record by is voteTier - all three off hgComputeThreeLayerConfidence. The ranking was the one thing that was not. csSortSetups read layer-1 agreement alone, and layer 1 is 40% of the number the card stamps two lines below it. Driven through the real runScan over 87 multi-setup scans (870 contracts, 801 setups): the #1 card was NOT the strongest tier on screen in 37 of 87 scans (42.5%), and 100 adjacent pairs were ordered against their own tier - 23 of them at an IDENTICAL printed percentage, two cards both reading "92% agree" with the tie broken by structure and then by a float the reader cannot see, landing WEAK (confidence 0.57) above STANDARD (0.72). A second contradiction fell out of the same key: the #N badge is the index in the sorted list while renderCards PAINTS the high-quality block first, and because layer-1 ignores quality entirely a lower-quality card routinely outranked every high-quality one - on 24 of 31 mixed-block scans the first card on screen was not #1 and #1 sat further down among the lower-quality ones. The keys now run in the order the reader meets them: the block renderCards will paint it into, the tier stamped on the card, the printed whole percent of layer-1, SMC structure, then raw pct for determinism. The first key is not a new judgement - it is what renderCards already does with two filters, moved in front of the sort so the sorted list and the painted list are the same list and the badge cannot lie. Measured on the same 87 scans: #1-not-strongest 37 to 0, tier inversions 100 to 0, badge disagreements 24 to 0. The tier now also rides the card HEAD as a chip, because ranking by a key that hides in the collapsed body is the same defect as ranking against a visible one. CS_TIER_ORDER is pinned in tests against the tiers hgComputeThreeLayerConfidence actually returns, so a new tier cannot quietly arrive and sort as weak; an unrecognised stamp ranks BELOW weak and says so on the chip rather than printing itself as a tier. This is NOT a claim that three-layer ranks better by OUTCOME - nothing has measured that and the forward log is the instrument that eventually will. Inside one tier the tab makes no discrete claim, so layer-1 still orders there and the confidence float is deliberately not consulted: choosing a within-tier key is a ranking decision with no defect driving it',
