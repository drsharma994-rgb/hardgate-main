...
console.log('\n== S24 spent T1 cannot confirm ==');
{
  const src = fs.readFileSync(root + 'goldind.js', 'utf8');
  const fn = src.slice(src.indexOf('function hgGoldPart5ThreeDrive'), src.indexOf('function hgGoldPart5OpenType'));
  ok(/measured T1 already traded through/.test(fn), 'S24 names a spent measured T1');
  ok(/out\.grade = 'forming'/.test(fn) && /out\.t1 = NaN/.test(fn),
    'spent T1 clears the target and stays forming');
  ok(typeof W.hgGoldTakeEnginePlan === 'function', 'takeEnginePlan is the bind gate');
  const house = { dir: 'long', entry: 4422.3, stop: 4319.6, t1: 4576.4 };
  ok(W.hgGoldTakeEnginePlan(house, { t1: 4382.4 }) === false && house.t1 === 4576.4,
    'S24-style engine T1 below entry is refused');
}
