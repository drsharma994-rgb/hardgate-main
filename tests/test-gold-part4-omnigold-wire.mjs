...
console.log('\n== stamp ==');
{
  ...
  ok(m && m[1] === 'hg-v568', 'build-stamp version (got ' + (m && m[1]) + ')');
  ok(sw.indexOf("HG_CACHE = 'hg-v568'") >= 0, 'sw.js matches stamp');
  ok(/goldind\.js\?v=568/.test(html), 'index goldind ?v=');
  ok(/goldswing\.js\?v=568/.test(html), 'index goldswing ?v=');
  ok(/omnigold\.js\?v=568/.test(html), 'index omnigold ?v=');
}
