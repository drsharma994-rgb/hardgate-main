...
console.log('== check-production script ==');
{
  ...
  ok(fs.existsSync(root + 'scripts/sync-cache-bust.mjs'), 'scripts/sync-cache-bust.mjs exists');
  ok(pkg.scripts && pkg.scripts['sync:cache'], 'package.json exposes sync:cache');
}
