...
console.log('\n== stamp ==');
{
  ...
  ok(m && m[1] === 'hg-v567', 'build-stamp version (got ' + (m && m[1]) + ')');
  ok(sw.indexOf("HG_CACHE = 'hg-v567'") >= 0, 'sw.js matches stamp');
  ok(/goldind\.js\?v=567/.test(html), 'index goldind ?v=');
  ok(/omnigold\.js\?v=567/.test(html) || /omnigold\.js\?v=/.test(html), 'index omnigold ?v=');
}
