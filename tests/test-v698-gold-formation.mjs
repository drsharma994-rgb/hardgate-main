...
console.log('\n=================== SHARED CONTRACT (gold-formation.js) ===================');
{
  ...
  ok(W.hgGoldSessionEdge(Date.UTC(2026,5,1,9,0)).confirms === false, 'LONDON 07-11 (measured gross-negative) does not confirm');
  ...
  ok(W.hgGoldSessionEdge(Date.UTC(2026,5,1,3,0)).confirms === true, 'ASIA 00-06 (measured gross-positive) confirms');
...
console.log('\n=================== OMNIGOLD ===================');
{
  ...
  /* hg-v910: this pinned "-0.124R at n=1428" verbatim, so re-deriving the
     table from the bake it cites broke it — a legitimate re-bake failing a
     test that was only ever about the VERDICT. What matters here is that
     LONDON does not confirm and that the line says so with a measured number
     beside it; which number that is belongs to gold-formation.js, and
     test-gold-evidence-citations.mjs is what holds it to the source. */
  ok(!!sessionEntry && /LONDON 07-11 UTC measured gross -\d+\.\d+R at n=\d+/.test(sessionEntry.detail),
     'OMNIGOLD: a passing `session` gate in LONDON is NOT a confirmation, and says why — ' + (sessionEntry ? sessionEntry.detail.slice(0, 60) : ''));
  ok(!!sessionEntry && /the clock is not evidence for this trade/.test(sessionEntry.detail),
     'and it names the reason rather than only the number');
...
console.log('\n=================== OMNIGOLD 1 ===================');
{
  ...
  ok(cLon[0].ok === false && /LONDON/.test(cLon[0].detail),
     'OMNIGOLD 1: the session leg does NOT confirm in LONDON 07-11 (measured gross-negative; the magnitude lives in gold-formation.js)');
