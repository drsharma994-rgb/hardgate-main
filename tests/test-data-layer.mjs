...
{
  const raw = await j(`${FAPI}/fapi/v1/klines?symbol=BTCUSDT&interval=1h&limit=500`);
...
{
  const p = await j(`${FAPI}/fapi/v1/premiumIndex?symbol=BTCUSDT`);
  show('binanceFunding BTCUSDT', `fundingPct=${(+p.lastFundingRate)*100}%  markPrice=${p.markPrice}  nextFundingTime=${new Date(+p.nextFundingTime).toISOString()}`);
}

// 3) Long/short ETHUSDT 1h
{
  const raw = await j(`${FAPI}/futures/data/globalLongShortAccountRatio?symbol=ETHUSDT&period=1h&limit=30`);
...
{
  const f = await j(`${FRANK}/v1/latest?base=USD&symbols=EUR,JPY,GBP,CAD,SEK,CHF`);
  const r = f.rates;
...
{
  const raw = await j(`${FAPI}/fapi/v1/klines?symbol=XAUUSDT&interval=4h&limit=100`);
  const rows = raw.map(k => ({ t: Math.floor(k[0]/1000), o:+k[1], h:+k[2], l:+k[3], c:+k[4], v:+k[5] }))
                  .sort((a,b) => a.t - b.t);
...
{
  const p = await j(`${FAPI}/fapi/v1/premiumIndex?symbol=XAUUSDT`);
  show('binanceFunding XAUUSDT', `fundingPct=${(+p.lastFundingRate)*100}%  markPrice=${p.markPrice}  nextFundingTime=${new Date(+p.nextFundingTime).toISOString()}`);
  const raw = await j(`${FAPI}/futures/data/globalLongShortAccountRatio?symbol=XAUUSDT&period=1h&limit=5`);
  const d = raw[raw.length - 1];
...
{
  const raw = await j(`${FAPI}/fapi/v1/klines?symbol=PAXGUSDT&interval=4h&limit=100`);
  const rows = raw.map(k => ({ t: Math.floor(k[0]/1000), o:+k[1], h:+k[2], l:+k[3], c:+k[4], v:+k[5] }))
                  .sort((a,b) => a.t - b.t);
...
// 8) US Treasury daily yield-curve CSV -> US10Y ('10 Yr' column; newest-first rows)
{
  ...
  const newest = lines[1].split(',');                 // rows arrive newest-first
...
{
  const xau = await j(`${GOLDAPI}/price/XAU`);
  const xag = await j(`${GOLDAPI}/price/XAG`);
