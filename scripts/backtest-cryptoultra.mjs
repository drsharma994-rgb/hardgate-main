/* HARDGATE — CRYPTO ULTRA backtest harness (offline, node ESM).
   Run:  node scripts/backtest-cryptoultra.mjs [--smoke] [--bars=N] [--refresh]

   Replays cryptoultra.js's OWN engine per closed 15m bar on BTCUSDT,
   zero lookahead. Same file, same arithmetic — the VERIFIED panel prints
   what this code produced.

   RULE CHOSEN HONESTLY: IS = first 70%, OOS = last 30%.
   COSTS: Binance spot 0.10% maker+taker each side = 0.20% RT.
   DATA: BTCUSDT Binance spot (shared scripts/.bt-cache — run SERIALLY). */
...
const OUT_FILE = path.join(ROOT, 'scripts', SMOKE ? 'backtest-cryptoultra-smoke-results.json' : 'backtest-cryptoultra-results.json');
const SYMBOL = 'BTCUSDT';
const WIN_15M = 320, WIN_1H = 400, MIN_15M = 230;
const COST_FRAC = 2 * 0.0010;  // Binance spot 0.10% each side
...
async function jget(url){ const r = await fetch(url, { headers: { 'User-Agent': 'hardgate-cryptoultra-backtest/1.0' } }); if (!r.ok) throw new Error('HTTP ' + r.status + ' for ' + url); return r.json(); }
...
function boot(){
  ...
  if (typeof ctx.cryptoUltraEngine !== 'function') throw new Error('boot failed: cryptoUltraEngine missing');
  return ctx;
}
...
function walkRule(sigs, m15, minPct, gate, timeoutBars){
  ...
  const settle = (tr, exitIdx, outcome, rGross, bothTouch) => {
    ...
    tr.netR = rGross == null ? null : rGross - (tr.entry * COST_FRAC) / risk;
  ...
  for (let bi = MIN_15M; bi < m15.length; bi++){
    ...
    for (const dir of ['long', 'short']){
      ...
      if (tr.state === 'pending'){
        // market fill: entry at signal close, assume filled on bar i+1 at open
        tr.state = 'filled'; tr.fillIdx = bi;
      }
...
const eligible = grid.filter(g => g.ins.n >= MIN_N_PICK && g.ins.avgR_net != null);
let chosen = null;
for (const g of eligible) if (!chosen || g.ins.avgR_net > chosen.ins.avgR_net || (g.ins.avgR_net === chosen.ins.avgR_net && g.minPct > chosen.minPct)) chosen = g;
const chosenNote = chosen ? 'best in-sample avg R net @Binance at n>=' + MIN_N_PICK : 'NO rule reached n>=' + MIN_N_PICK + ' in-sample — nothing chosen, nothing baked';
...
const limitations = [
  'MARKET FILL AT THE SIGNAL CLOSE: the walk fills at the signal close, ignoring the next-bar open gap; BTC 15m gaps are typically small but can be large on liquidation cascades',
  'SAME-BAR FILL->TARGET OPTIMISM: ' + sameBarWins + ' of ' + oosSettled.filter(t => t.outcome === 'win').length + ' OOS wins settle on the fill bar; both-touch bars ARE losses (' + oosSettled.filter(t => t.bothTouch).length + ')',
  'PORTFOLIO STATS NOT ATTAINABLE: one live trade per side, unlimited overlap between sides; read per-trade expectancy only',
  'THE RULE WAS PICKED ON THE FIRST ' + Math.round(IS_SHARE * 100) + '% AND REPORTED ON THE LAST ' + Math.round((1 - IS_SHARE) * 100) + '%: one split, one regime of BTC history — a different quarter can measure differently; the grid rows show how sensitive the choice is',
  'BTCUSDT spot on Binance: 0.10% maker+taker each side = 0.20% round-trip; actual fees depend on VIP tier and BNB holdings',
...
const verdict = !chosen ? 'No rule reached the minimum in-sample sample; nothing is claimed.'
  ...
  : chosen.oos.avgR_net > 0 ? 'Out-of-sample the rule paid +' + chosen.oos.avgR_net.toFixed(3) + 'R per trade after Binance costs on n=' + chosen.oos.n + ' (win ' + (100 * chosen.oos.winRate).toFixed(1) + '%) — a small, positive edge. Size accordingly; it is not an ATM.'
  : 'Out-of-sample the rule did NOT pay after Binance costs (' + chosen.oos.avgR_net.toFixed(3) + 'R per trade on n=' + chosen.oos.n + ') — fires print for the record and are NOT tradable tickets.');
const tradable = !!(chosen && chosen.oos.n >= 50 && chosen.oos.avgR_net > 0);
...
const out = { meta: { generated: new Date().toISOString(), mode: SMOKE ? 'smoke' : 'full', symbol: SYMBOL, bars: { m15: m15.length, h1: h1.length },
              ...
              trades: chosen ? chosen._trades.map(t => ({ tISO: new Date(m15[t.sigIdx].t * 1000).toISOString(), dir: t.dir, pct: +t.pct.toFixed(3), decisive: t.decisive, regime: t.regime,
                ...
                rGross: t.rGross == null ? null : +t.rGross.toFixed(3), netR: t.netR == null ? null : +t.netR.toFixed(3),
...
console.log('minPct gate  | IS n  win%  net    | OOS n  win%  net    | fired merged');
for (const g of grid) console.log(rp((g.minPct * 100).toFixed(0) + '%', 6) + ' ' + rp(g.gate ? 'on' : 'off', 4) + '  | ' + rp(g.ins.n, 4) + ' ' + rp(g.ins.winRate == null ? '—' : (100 * g.ins.winRate).toFixed(0) + '%', 5) + ' ' + rp(g.ins.avgR_net, 6)
  + '  | ' + rp(g.oos.n, 5) + ' ' + rp(g.oos.winRate == null ? '—' : (100 * g.oos.winRate).toFixed(0) + '%', 5) + ' ' + rp(g.oos.avgR_net, 6) + ' | ' + rp(g.counters.fired, 5) + ' ' + rp(g.counters.merged, 6) + (chosen === g ? '   <== chosen' : ''));
console.log('\nchosen: ' + (chosen ? (chosen.minPct * 100) + '% · gate ' + (chosen.gate ? 'on' : 'off') + ' · ' + chosenNote : chosenNote));
if (cohorts){ for (const k of Object.keys(cohorts)){ console.log('\n--- OOS ' + k + ' ---'); for (const g of Object.keys(cohorts[k])){ const a = cohorts[k][g]; console.log(rp(g, 20) + '  n ' + rp(a.n, 4) + '  win ' + rp(a.winRate == null ? '—' : (100 * a.winRate).toFixed(0) + '%', 5) + '  net ' + rp(a.avgR_net, 7)); } } }
