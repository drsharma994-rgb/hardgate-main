...
function hgAtomicDeskPanelHtml(){
  ...
  var h = '<div class="hg-panel__legend" style="margin-top:10px">Atomic Agents · best setup · Delta + CoinDCX</div>';
  ...
  if (d.bestSetups && d.bestSetups.length){
    h += '<table class="hg-table" style="margin-top:6px"><thead><tr><th>sym</th><th>venue</th><th>dir</th><th>style</th><th>score</th></tr></thead><tbody>';
    for (var i = 0; i < Math.min(d.bestSetups.length, 8); i++){
      var s = d.bestSetups[i];
      h += '<tr><td>' + (s.sym || '—') + '</td><td>' + (s.bestVenue || s.exchange || '—') + '</td>'
        + '<td class="' + (s.dir === 'long' ? 'pos' : 'neg') + '">' + (s.dir || '—') + '</td>'
        + '<td>' + (s.style || '—') + '</td><td class="hg-num">' + (s.score != null ? s.score : '—') + '</td></tr>';
    }
    h += '</tbody></table>';
  }
  return h;
