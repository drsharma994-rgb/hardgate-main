...
- **PINE tab:** Background `pineWarm` runs every **15 min** with the alert cycle (after `edgeWarm` for universe). Telegram for NEW/RECENT signals via unified **tabalerts** batch (`collectPine*`); manual PINE scan still uses `pineFireAlerts` directly. Requires **Alerts ON** and Telegram configured (`sendTelegram` / `TELEGRAM_*` env).
...
- **Telegram setups (hg-v129):** Unified **15-min** batch via `tabalerts.js`. **Default (hg-v131):** only **7/7 CLEAN** tickets with **Entry · SL · TP1** (SWING/SCALP scans, EDGE/BEST/OI/SQUEEZE when gate-clean, BRAIN HIGH/PRIME with 7/7 swing evidence). Set `localStorage hgAlertCleanOnly=0` to restore all-tab alerts (PINE, GOLD tally, watches, etc.). Per-setup dedup in `hg_tabalert_keys`; cycle throttle in `hg_tabalert_last_run`. `HG_ALERT_CYCLE_MS` = 15 min. Requires **Alerts ON** + `TELEGRAM_TOKEN` / `TELEGRAM_CHAT_ID` (or browser-stored creds).
...
- **Star Trader tab:** `tests/test-startradertab.mjs` covers `startradertab.js` synthesis (`stSynthesize`, `stContextVotes`, `stDropForming`, EDGE scan fallback). `tests/test-startrader.mjs` covers `startrader.js` catalog + `xuniverse.js` merge legs.
