...
- **Telegram setups (hg-v129):** Unified **15-min** crypto batch via `tabalerts.js` (**7/7 CLEAN** default). **Gold (hg-v132):** separate **15-min** batch via `hgTabAlertsRunGold()` — **GOLD SCALP + GOLD SWING only**, **MOST PROBABLE** (`bestId`) or **grade-A locked** conviction; no GOLD PINE. Keys: `hgAlertGoldSeparate` (default ON), `hgAlertGoldConvictedOnly` (default ON), throttle `hg_tabalert_gold_last_run`. Legacy inline GOLD tab GS1–GS7 alerts off unless `hgAlertGoldLegacyInline=1`. Set `hgAlertCleanOnly=0` to widen the crypto batch.
