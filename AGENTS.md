...
- **GOLD DEEP inline engine:** 37-gate swing + 40-gate scalp ledger in `index.html` (`GOLD DEEP SCAN` markers). **`tests/test-gold-deep.mjs`** runs it offline; thin live gold feeds fall back to synthetic candles so `#goldDeepOut` always renders in CI.
