...
- **Non-live upgrades:** EDGE cards show **FLOW OK / PARTIAL / N/A** when Binance CVD/OBI legs missing; EDGE alerts run every **3 min**; tabalerts cover SMART $, OI FLOW, LIQS, SQUEEZE, CARRY, TERM BASIS watch; `hardRefreshAll` warms macro + layer tabs; static GitHub Pages shows proxy banner via `hghost.js`; gate funnel tab labeled **GATES** (id `execute`); scanner cards can show **IN BOOK** via `hgBookStampHTML`.
