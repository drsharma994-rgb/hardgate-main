...
- **PAPER BOOK:** Auto trails **ATR (2×4h ATR @0.5R+)**, **+0.5R @2R**, **+1R @3R**, BE @1R. **LIVE** when `EXECUTE_WEBHOOK_URL` set (`POST /api/book/live`). **WEEKLY DIGEST** + `LP_DIGEST_WEBHOOK_URL` (`GET/POST /api/book/digest`). Execution **blotter** + `GET /api/book/capabilities`.
