...
- **PAPER BOOK:** `book.js` + `/api/book` (Render only). Desk OMS on each row: MANAGE → TRADE PLAN, 50% scale, BE stop, CLOSE. Partial scale via `POST /api/book/scale`, stop moves via `POST /api/book/stop`. R-multiple column, bucket exposure bars, UTC daily PnL, NAV history.
