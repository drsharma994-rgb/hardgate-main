...
- **VP liquidity targets (hg-v557):** `goldVolumeProfile` now emits **POC / VAH / VAL / HVN / LVN / profile high-low** (70% value area). `hgGoldVpTargets` builds post-sweep TP1–TP3 ladder (external liquidity → profile → LVN edge); **PROFILE TARGET CONFIRMED** when MSS agrees and TP1 ≥1.5R. Thin-venue / non-COMEX note on panel. Wired into forming stack + `liqsweep`/`nyexh` cards. Tests: `tests/test-gold-vp-targets.mjs`.
