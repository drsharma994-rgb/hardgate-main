...
- **LOG costs + stats (hg-v170):** Setup log shows **ΣR gross** and **ΣR NET** (Delta taker/maker + 18% GST on fees); Wilson 95% CI on hit rate; per-row **net R** column. Override fees via `hg_fee_taker` / `hg_fee_maker` / `hg_fee_gst` in localStorage. **`tests/test-costs-stats.mjs`** guards fee model + directional G4.
