...
- **Gold venue (hg-v169):** `getXAUCandles` in `index.html` prefers **Delta XAUTUSD** candles (the tradable instrument); proxy fallback (Binance XAUUSDT chain) stamps **PROXY** on the DATA chip and records `S.goldBasisPct`. Use `hgGoldBasisNote()` when levels are proxy-sourced. **`tests/test-gold-venue.mjs`** guards the three network scenarios.
