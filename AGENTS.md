...
- **Fix Pack 21 (hg-v191):** **`CG_SWING_LOOK`** parameter (default still 30) — stop horizon was hardcoded at 30 bars (5 days on 4H) while G6 caps risk at ~1.75×ATR (~2.3 days). CALIBRATE adds **SWING LOOKBACK** panel sweeping 5/8/10/15/20/30 with cached candles. **`tests/test-swing-lookback.mjs`** (24 assertions).
