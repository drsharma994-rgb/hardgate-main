...
- **PAPER BOOK:** `book.js` + `/api/book` (Render only). Positions persist in `data/paperbook.json`. Auto-refreshes marks every 45s while the BOOK tab is open. `ADD TO BOOK` is on all major scanners including SMART $, BEST, and inline GOLD tabs. Daily PnL rolls at UTC midnight; NAV history snapshots on each mark refresh. Successful adds log to SCORECARD; closes trigger `hgScoreSettle` fire-and-forget.
