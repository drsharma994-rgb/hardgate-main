...
- **PAPER BOOK:** Multi-fund store (`?fund=` / `fund` body) — `GET/POST /api/book/funds`. Default **main**; presets gold/swing/macro. Scanner routing via `book-routing.js` / `lib/book-routing.mjs` (metals→gold, swing/scalp→swing, carry/termbasis/macro→macro). Consolidated LP: `GET /api/book/consolidated?period=month|week`. BOOK row **EXEC** uses `executeTrade` when `/api/execute` is configured. Auto trails ATR + R locks. LIVE / digest / email per active fund (`LP_DIGEST_FUND` env overrides cron fund).
