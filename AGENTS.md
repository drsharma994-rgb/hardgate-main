...
- **Fix Pack 9 (hg-v175):** **`hgFundingCostR`** in net R for LOG + scorecard (`fundingPct` at log/record time, hold from fill→settle); SCORECARD board shows **Wilson 95% CI** on win rate via `window.hgWilson`. **`tests/test-fix-pack-9.mjs`**.
