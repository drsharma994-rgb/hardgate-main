...
- **Chart vision (hg-v233):** **`/api/chart-vision/analyze`** — OHLCV → SVG + heuristic next-move (optional **`GEMINI_API_KEY`**). **`chart-vision-desk.js`** enriches top GOLD SCALP/SWING + EXECUTE cards with **VISION** chips; formation boost via **`hgChartVisionFormationBoost`**. **`tests/test-chart-vision.mjs`**.
