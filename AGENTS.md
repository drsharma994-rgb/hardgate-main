...
- **PAPER BOOK:** Auto trails **ATR (2×4h ATR @0.5R+)**, **+0.5R @2R**, **+1R @3R**, BE @1R. **LIVE** when `EXECUTE_WEBHOOK_URL` set (`POST /api/book/live`). **WEEKLY DIGEST** auto-sends Sun ~21:07 IST via `scripts/book-digest-watch.mjs` (Render) + `.github/workflows/book-digest.yml` backup. Channels: `LP_DIGEST_WEBHOOK_URL` and/or `TELEGRAM_TOKEN`+`TELEGRAM_CHAT_ID`; optional `BOOK_DIGEST_CRON_SECRET` for external cron. Status: `GET /api/book-digest-watch`.
