...
- **No live trading (hg-v171):** `HG_LIVE_TRADING_ENABLED = false` in `execute.js` — kills EXECUTE/LIVE buttons, auto-exec toggles, and all broker fetches. `bookLiveAllowed()` mirrors it. Re-arm only by flipping the flag. **`tests/test-no-live-trading.mjs`**.
