...
- **PAPER BOOK:** Multi-fund store (`?fund=` / `fund` body) — `GET/POST /api/book/funds`. Default **main**; presets gold/swing/macro. Auto trails ATR + R locks. LIVE / digest / email per active fund (`LP_DIGEST_FUND` env overrides cron fund).
