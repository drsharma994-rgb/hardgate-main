...
- **PAPER BOOK:** Auto trails **ATR (2×4h ATR @0.5R+)**, **+0.5R @2R**, **+1R @3R**, BE @1R. **LIVE** when `EXECUTE_WEBHOOK_URL` set. **WEEKLY DIGEST** Sun ~21:07 IST (`book-digest-watch.mjs` + GitHub backup). Channels: `LP_DIGEST_WEBHOOK_URL`, Telegram, or **email** (`LP_DIGEST_EMAIL_TO` + `RESEND_API_KEY` / `SENDGRID_API_KEY` / `SMTP_*`). Optional `BOOK_DIGEST_CRON_SECRET`. Status: `GET /api/book-digest-watch`.
