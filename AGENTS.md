...
- **Chart vision (hg-v236):** **`/api/chart-vision/analyze`** — OHLCV → SVG/PNG + heuristic **next-bar opinion** + outcome prediction (optional **`GEMINI_API_KEY`** multimodal PNG). **Vision veto on by default** (`CHART_VISION_VETO=0` to disable). **`chart-vision-desk.js`** enriches confirmed setups on SWING/SCALP, SMART $, SQUEEZE, TRENDMATRIX, GATES (EXECUTE), BEST, OIFLOW, EDGE, STARTRADER, and GOLD cards with **NEXT BAR** lines and **VISION** chips; formation boost via **`hgChartVisionFormationBoost`**. **`tests/test-chart-vision.mjs`**.
