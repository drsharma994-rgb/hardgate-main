...
- **Fix Pack 11 (hg-v177+):** **Vol mesh** (DVOL, spot-perp, stables) + **RS F10** — `hgRelStrength` 30×4H vs BTC (measured orthogonal family); BEST `famMax` scales denominator; selection bars 8/10 not 7/9. **`tests/test-fix-pack-11.mjs`**, **`tests/test-relative-strength.mjs`**.
