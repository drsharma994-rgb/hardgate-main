...
| `CHART_VISION_VETO` | When `1` / `true`, high-confidence vision conflicts can veto formation tickets |
...
- **Chart vision (hg-v234):** **`/api/chart-vision/analyze`** — OHLCV → SVG/PNG + heuristic next-move + **outcome prediction** (optional **`GEMINI_API_KEY`** multimodal PNG). **`chart-vision-desk.js`** enriches top GOLD SCALP/SWING + EXECUTE cards with **VISION** chips and predicted path/edge; formation boost via **`hgChartVisionFormationBoost`**. **`tests/test-chart-vision.mjs`**.
