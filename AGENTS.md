...
- **Chart vision (hg-v235):** **`/api/chart-vision/analyze`** — OHLCV → SVG/PNG + heuristic next-move + **outcome prediction** (optional **`GEMINI_API_KEY`** multimodal PNG). **Vision veto on by default** (`CHART_VISION_VETO=0` to disable). **`chart-vision-desk.js`** enriches top GOLD SCALP/SWING + EXECUTE cards with **VISION** chips and predicted path/edge; formation boost via **`hgChartVisionFormationBoost`**. **`tests/test-chart-vision.mjs`**.
